...
const TEXTURE_CACHE_MAX_SIZE = 200;
const HISTORY_MAX_DEPTH = 50;
enableMapSet();
function getFirstWidgetLookName(fileState) {
  if (!fileState) {
    return null;
  }
...
function syncSelectionToActiveFile(state, forceReset = false) {
  const activeFile = state.files.activeFilePath ? state.files.openedFiles.get(state.files.activeFilePath) : void 0;
  const nextWidgetLookName = activeFile && state.selection.widgetLookName && activeFile.loadedWidgetLooks.has(state.selection.widgetLookName) ? state.selection.widgetLookName : getFirstWidgetLookName(activeFile);
...
const useEditorStore = create()(
  immer((set2, get) => ({
    ...
    dirty: false,
    openFile: (filePath, fileIndex, widgetLooks) => set2((state) => {
    ...
    setActiveFile: (filePath) => set2((state) => {
    ...
    registerImagesets: (filePath, imagesetsData) => set2((state) => {
      const fileState = state.files.openedFiles.get(filePath);
      if (!fileState) return;
    ...
    setSchemePath: (filePath, schemePath) => set2((state) => {
      const fileState = state.files.openedFiles.get(filePath);
      if (fileState) {
        fileState.schemePath = schemePath;
      }
    }),
    selectWidgetLook: (name) => set2((state) => {
      state.selection.widgetLookName = name;
      state.selection.selectedNodeIds = [];
      state.selection.hoveredNodeId = null;
      state.preview.highlightedSection = null;
    }),
    selectNode: (nodeId, multiSelect) => set2((state) => {
      if (multiSelect) {
    ...
    hoverNode: (nodeId) => set2((state) => {
      state.selection.hoveredNodeId = nodeId;
    }),
    updateWidgetLook: (name, wl) => set2((state) => {
    ...
    markDirty: (dirty) => set2((state) => {
      ...
      state.dirty = dirty ? true : Array.from(state.files.openedFiles.values()).some((file) => file.dirty);
    })
  }))
);
function makeComponentNodeId(sectionName, componentType, componentIndex, look) {
  return look ? `section:${look}:${sectionName}:${componentType}:${componentIndex}` : `section:${sectionName}:${componentType}:${componentIndex}`;
}
function parseComponentNodeId(nodeId) {
  const parts = nodeId.split(":");
  if (parts[0] !== "section" || parts.length !== 4 && parts.length !== 5) {
    return null;
  }
  const [look, sectionName, componentType, componentIndexRaw] = parts.length === 5 ? [parts[1], parts[2], parts[3], parts[4]] : [void 0, parts[1], parts[2], parts[3]];
...
function evaluateDim(node, ctx) {
  switch (node.type) {
    ...
    case "WidgetDim": {
      const rect = node.widget ? ctx.widgetDimensions.get(node.widget) : ctx.widgetDimensions.get("__self__") || {
        x: 0,
        y: 0,
        w: ctx.parentWidth,
        h: ctx.parentHeight
...
function resolveDimBase(dimType, ctx) {
  switch (dimType) {
    case "LeftEdge":
    case "RightEdge":
    case "Width":
...
function resolveDimensionValue(dimType, width, height) {
  switch (dimType) {
    case "Width":
    case "LeftEdge":
    case "RightEdge":
...
function resolveRectDimValue(dimType, rect) {
  switch (dimType) {
    case "LeftEdge":
    case "XPosition":
      return rect.x;
...
function pixelToDim(pixelValue, dimType, parentSize, strategy = "hybrid") {
  switch (strategy) {
    case "absolute":
    ...
    case "unified-relative":
      return {
        type: "UnifiedDim",
        scale: parentSize > 0 ? Math.round(pixelValue / parentSize * 1e4) / 1e4 : 0,
        offset: 0,
        dimType
      };
    case "unified-offset":
    ...
    case "hybrid": {
      ...
      const scale = Math.round(pixelValue / parentSize * 1e3) / 1e3;
...
function resolveImagerySection(sectionRef, currentWidgetLookName, currentSections, availableLooks) {
...
function formatSectionRefName(sectionRef) {
  return sectionRef.look ? `${sectionRef.look}:${sectionRef.sectionName}` : sectionRef.sectionName;
}
function renderWidgetLook(ctx, sections, activeStates, evalCtx, textureCache2, viewport, currentWidgetLookName = "", availableLooks) {
  ctx.save();
...
function renderImagerySection(ctx, section, evalCtx, textureCache2, overrideColours) {
  for (const fc of section.frameComponents) {
    renderFrameComponent(ctx, fc, evalCtx, textureCache2, overrideColours);
  }
  for (const ic of section.imageryComponents) {
    renderImageryComponent(ctx, ic, evalCtx, textureCache2, overrideColours);
  }
  for (const tc of section.textComponents) {
    renderTextComponent(ctx, tc, evalCtx, overrideColours);
  }
}
function renderFrameComponent(ctx, frame, evalCtx, textureCache2, overrideColours) {
  const area = evaluateArea(frame.area, evalCtx);
  if (area.w <= 0 || area.h <= 0) return;
  const images = frame.images;
  const hasBorders = images.LeftEdge || images.RightEdge || images.TopEdge || images.BottomEdge || images.TopLeftCorner || images.TopRightCorner || images.BottomLeftCorner || images.BottomRightCorner;
  if (!hasBorders) {
    const bgRef = images.Background;
    if (bgRef) {
      const bgCanvas2 = textureCache2.get(bgRef.imageset, bgRef.image);
      if (bgCanvas2) {
        applyColours(ctx, frame.colours || overrideColours);
        drawStretched(ctx, bgCanvas2, area, frame.vertFormat, frame.horzFormat);
        ctx.globalAlpha = 1;
      }
    }
    return;
  }
  const topLeftCanvas = getCanvas(textureCache2, images.TopLeftCorner);
  const topRightCanvas = getCanvas(textureCache2, images.TopRightCorner);
  const bottomLeftCanvas = getCanvas(textureCache2, images.BottomLeftCorner);
  const bottomRightCanvas = getCanvas(textureCache2, images.BottomRightCorner);
  const leftCanvas = getCanvas(textureCache2, images.LeftEdge);
  const rightCanvas = getCanvas(textureCache2, images.RightEdge);
  const topCanvas = getCanvas(textureCache2, images.TopEdge);
  const bottomCanvas = getCanvas(textureCache2, images.BottomEdge);
  const bgCanvas = getCanvas(textureCache2, images.Background);
...
function renderImageryComponent(ctx, comp, evalCtx, textureCache2, overrideColours) {
  const area = evaluateArea(comp.area, evalCtx);
  if (area.w <= 0 || area.h <= 0) return;
  if (comp.image) {
    const canvas = textureCache2.get(comp.image.imageset, comp.image.image);
    if (canvas) {
      applyColours(ctx, comp.colours || overrideColours);
      drawStretched(ctx, canvas, area, comp.vertFormat, comp.horzFormat);
      ctx.globalAlpha = 1;
    }
  }
}
function renderTextComponent(ctx, comp, evalCtx, overrideColours) {
  const area = evaluateArea(comp.area, evalCtx);
  if (area.w <= 0 || area.h <= 0) return;
...
function drawStretched(ctx, src, area, vertFormat, horzFormat) {
...
function drawTiled(ctx, src, area, tileVert, tileHorz) {
...
function applyColours(ctx, colours, area) {
  if (!colours) return;
...
function getCanvas(textureCache2, ref) {
  if (!ref) {
    return null;
  }
  return textureCache2.get(ref.imageset, ref.image);
}
function mergeColourRects(base, override) {
...
function renderSelectionHandles(ctx, rect, viewport) {
  ctx.save();
  ctx.translate(viewport.offsetX, viewport.offsetY);
  ctx.scale(viewport.scale, viewport.scale);
...
function hitTestDragMode(mouseX, mouseY, rect, viewport, handleRadius = 6) {
...
function applyDrag(mode, dx, dy, original, minSize = 4) {
...
function zoomViewport(viewport, delta, pivotX, pivotY, minScale = 0.1, maxScale = 10) {
...
class TextureCacheManager {
  ...
  atlasImages = /* @__PURE__ */ new Map();
  /** 纹理加载状态 */
  loadStates = /* @__PURE__ */ new Map();
  /** 加载完成回调列表 */
  loadCallbacks = /* @__PURE__ */ new Map();
  /** 注册 imageset 资源 */
  registerImageset(resource) {
    this.imagesets.set(resource.name, resource);
  }
  /** 批量注册 imageset 资源 */
  registerImagesets(resources) {
    for (const r of resources) {
      this.imagesets.set(r.name, r);
    }
  }
  /** 请求加载纹理图集（异步） */
  async loadAtlas(imagesetName) {
  ...
  /** 预裁剪所有子图像到缓存 */
  preClipSubImages(imagesetName, resource, atlasImg) {
  ...
  /** 获取 imageset 的加载状态 */
  getLoadState(imagesetName) {
    return this.loadStates.get(imagesetName) || "idle";
  }
  /** 获取子图像尺寸信息 */
  getSubImageSize(imageset, image) {
    const resource = this.imagesets.get(imageset);
  ...
  getSubImageDataUrl(imageset, image) {
  ...
  extractRequiredImagesets(widgetLook, availableLooks) {
  ...
  async loadRequiredTextures(widgetLook, availableLooks) {
    const required = this.extractRequiredImagesets(widgetLook, availableLooks);
    const promises = required.filter((name) => {
  ...
  /** 清除指定 imageset 的缓存 */
  invalidateImageset(imagesetName) {
...
const textureCache = new TextureCacheManager();
function computeWriteback(originalArea, newPixelRect, dragMode, ctx, strategy = "hybrid") {
  const changedFields = [];
  const newArea = { ...originalArea };
  const usesWidth = originalArea.width !== void 0;
  const usesHeight = originalArea.height !== void 0;
  const usesRight = originalArea.right !== void 0;
  const usesBottom = originalArea.bottom !== void 0;
  ...
  const description = buildDescription(dragMode, changedFields);
  return { newArea, changedFields, description };
}
function writebackDim(originalDim, newPixelValue, dimType, parentSize, strategy) {
...
function writebackUnifiedDim(original, newPixelValue, dimType, parentSize) {
  if (parentSize <= 0) {
    return { type: "AbsoluteDim", value: Math.round(newPixelValue) };
  }
  const newOffset = Math.round(newPixelValue - original.scale * parentSize);
  if (Math.abs(newOffset) > parentSize * 0.5) {
    const scale = Math.round(newPixelValue / parentSize * 1e3) / 1e3;
...
function writebackDimOperator(original, newPixelValue, dimType, parentSize, strategy) {
  return pixelToDim(newPixelValue, dimType, parentSize, strategy);
}
function isWidthChanging(mode) {
  return mode === "resize-tl" || mode === "resize-tr" || mode === "resize-bl" || mode === "resize-br" || mode === "resize-l" || mode === "resize-r";
}
function isRightChanging(mode) {
  return mode === "move" || mode === "resize-tr" || mode === "resize-br" || mode === "resize-r";
}
function isHeightChanging(mode) {
  return mode === "resize-tl" || mode === "resize-tr" || mode === "resize-bl" || mode === "resize-br" || mode === "resize-t" || mode === "resize-b";
}
function isBottomChanging(mode) {
  return mode === "move" || mode === "resize-bl" || mode === "resize-br" || mode === "resize-b";
}
function buildDescription(mode, changedFields, _originalArea, _newArea) {
  const modeLabel = {
    "move": "Move",
...
function applyDragWriteback(widgetLook, payload, ctx, strategy) {
  const newWl = JSON.parse(JSON.stringify(widgetLook));
...
function Canvas() {
  ...
  const [canvasSize, setCanvasSize] = reactExports.useState({ w: 800, h: 600 });
  const [dragState, setDragState] = reactExports.useState(EMPTY_DRAG_STATE);
  const [texturesReady, setTexturesReady] = reactExports.useState(false);
  ...
  const currentWl = React$2.useMemo(() => {
  ...
  const availableLooks = React$2.useMemo(() => activeFile ? openedFiles.get(activeFile)?.loadedWidgetLooks : void 0, [activeFile, openedFiles]);
  ...
  reactExports.useEffect(() => {
    ...
    const selectedArea = dragState.active && dragState.currentArea ? dragState.currentArea : selectedNodeIds.length > 0 ? findSelectedArea(currentWl, selectedNodeIds[0], evalCtx, availableLooks) : null;
    if (selectedArea) {
  ...
  const handleMouseDown = reactExports.useCallback((e) => {
    ...
    const selectedTarget = selectedNodeIds.length > 0 ? renderTargets.find((target) => target.id === selectedNodeIds[0]) ?? buildTargetFromNodeId(currentWl, selectedNodeIds[0], evalCtx, availableLooks) : null;
    const selectedHitMode = selectedTarget ? hitTestDragMode(e.nativeEvent.offsetX, e.nativeEvent.offsetY, selectedTarget.area, viewport) : null;
    const hitTarget = selectedHitMode ? selectedTarget : renderTargets.slice().reverse().find((target) => hitTestDragMode(e.nativeEvent.offsetX, e.nativeEvent.offsetY, target.area, viewport));
    if (!hitTarget) {
      return;
    }
    const dragMode = hitTarget === selectedTarget && selectedHitMode ? selectedHitMode : hitTestDragMode(e.nativeEvent.offsetX, e.nativeEvent.offsetY, hitTarget.area, viewport);
    if (!dragMode) {
      return;
    }
  ...
  const handleMouseUp = reactExports.useCallback(() => {
    if (dragState.active && dragState.mode && dragState.currentArea && dragState.sectionName && dragState.componentType && currentWl && wlName) {
      ...
      if (dx > 1 || dy > 1 || dw > 1 || dh > 1) {
        const targetWidgetLook = dragState.lookName ? availableLooks?.get(dragState.lookName) : currentWl;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: containerRef, className: "canvas-container", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
    "canvas",
    {
      ref: canvasRef,
      width: canvasSize.w,
...
function drawGrid(ctx, size, viewport) {
...
function findSelectedArea(wl, nodeId, evalCtx, availableLooks) {
...
function findFirstVisibleArea(wl, matchedStates, evalCtx, availableLooks) {
...
function buildEvaluationContext(wl, availableLooks, parentWidth, parentHeight) {
  const evalCtx = {
    parentWidth,
    parentHeight,
    widgetDimensions: /* @__PURE__ */ new Map([
      ["__self__", { x: 0, y: 0, w: parentWidth, h: parentHeight }]
    ]),
    imageDimensions: /* @__PURE__ */ new Map(),
    fontMetrics: /* @__PURE__ */ new Map()
  };
...
function resolveMatchedStates(wl, activeStates) {
...
function collectRenderableComponents(wl, matchedStates, evalCtx, availableLooks) {
...
function appendSectionTargets(targets, section, evalCtx, lookName) {
  section.frameComponents.forEach((component, index2) => {
    targets.push({
      id: lookName ? `section:${lookName}:${section.name}:frame:${index2}` : `section:${section.name}:frame:${index2}`,
  ...
  section.imageryComponents.forEach((component, index2) => {
    targets.push({
      id: lookName ? `section:${lookName}:${section.name}:imagery:${index2}` : `section:${section.name}:imagery:${index2}`,
      area: evaluateArea(component.area, evalCtx),
      lookName: lookName ?? null,
      sectionName: section.name,
  ...
  section.textComponents.forEach((component, index2) => {
    targets.push({
      id: lookName ? `section:${lookName}:${section.name}:text:${index2}` : `section:${section.name}:text:${index2}`,
      area: evaluateArea(component.area, evalCtx),
      lookName: lookName ?? null,
      sectionName: section.name,
      componentType: "text",
...
function buildTargetFromNodeId(wl, nodeId, evalCtx, availableLooks) {
...
function findComponentArea(wl, lookName, sectionName, componentType, componentIndex, evalCtx, availableLooks) {
  const targetLook = lookName && lookName !== wl.name ? availableLooks?.get(lookName) : wl;
  ...
  switch (componentType) {
    case "frame":
      return section.frameComponents[componentIndex] ? evaluateArea(section.frameComponents[componentIndex].area, evalCtx) : null;
    case "imagery":
      return section.imageryComponents[componentIndex] ? evaluateArea(section.imageryComponents[componentIndex].area, evalCtx) : null;
    case "text":
      return section.textComponents[componentIndex] ? evaluateArea(section.textComponents[componentIndex].area, evalCtx) : null;
  }
}
const zhCN = {
  app: {
...
class I18nManager {
  ...
  /** 获取所有可用语言 */
  getAvailableLocales() {
    return Object.keys(MESSAGE_MAPS);
  }
}
...
function useTranslation() {
  const [, forceUpdate] = React$2.useState(0);
  React$2.useEffect(() => {
...
function StateBar() {
  ...
  const [autoCycling, setAutoCycling] = reactExports.useState(false);
  const cycleRef = reactExports.useRef(null);
  const cycleIndexRef = reactExports.useRef(0);
  const toggleState = reactExports.useCallback((state) => {
  ...
  const handleAutoCycle = reactExports.useCallback(() => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "state-bar", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "state-bar-left", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "state-bar-label", children: [
        t("panel.buttonStates"),
        ":"
      ] }),
      BUTTON_STATES.map((state) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "state-bar-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "state-bar-label", children: [
        t("panel.windowStates"),
        ":"
      ] }),
      FRAME_WINDOW_STATES.slice(0, 4).map((state) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: `state-toggle state-toggle-sm ${activeStates.includes(state) ? "active" : ""}`,
          onClick: () => toggleState(state),
          title: state,
          children: abbreviateState(state)
        },
        state
      ))
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "state-bar-right", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "size-input-group", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "state-bar-label", children: "W:" }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "size-separator", children: "×" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "size-input-group", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "state-bar-label", children: "H:" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            type: "number",
...
function abbreviateState(state) {
  return state.replace(/Active/g, "A").replace(/Inactive/g, "I").replace(/Disabled/g, "D").replace(/WithTitle/g, "T").replace(/NoTitle/g, "NT").replace(/WithFrame/g, "F").replace(/NoFrame/g, "NF");
}
function LiveViewport() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "live-viewport", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "viewport-canvas-area", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Canvas, {}) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(StateBar, {})
  ] });
}
const ICON_MAP = {
  widgetlook: "📦",
...
function buildLayerTree(wl, activeStateNames = [], availableLooks) {
  ...
  root2.children.push(sectionNode);
  const stateNode = {
  ...
  root2.children.push(stateNode);
  return root2;
}
function buildSectionNode(wlName, section) {
  return {
...
function buildStateNode(wlName, state, index2, sectionMap, activeStates, availableLooks) {
  const children = [];
  state.layers.forEach((layer, li) => {
    const layerChildren = layer.sections.map((sec, si) => {
      const resolvedSection = resolveImagerySection(sec, wlName, sectionMap, availableLooks);
      return {
        id: `wl-${wlName}-state-${index2}-layer-${li}-sec-${si}`,
        type: "section",
    ...
    children.push({
      id: `wl-${wlName}-state-${index2}-layer-${li}`,
      type: "layer",
      name: `Layer ${li}`,
      icon: ICON_MAP.layer,
      visible: true,
...
function buildSectionComponentNodes(section, lookName) {
  if (!section) {
    return [];
  }
  return [
    ...section.frameComponents.map((component, index2) => ({
      id: makeComponentNodeId(section.name, "frame", index2, lookName),
      type: "component",
      name: "FrameComponent",
      icon: "🖼️",
      visible: true,
    ...
    ...section.imageryComponents.map((component, index2) => ({
      id: makeComponentNodeId(section.name, "imagery", index2, lookName),
      type: "component",
    ...
    ...section.textComponents.map((component, index2) => ({
      id: makeComponentNodeId(section.name, "text", index2, lookName),
      type: "component",
      name: "TextComponent",
      icon: "🖼️",
      visible: true,
...
function VirtualList({
  ...
  const handleKeyDown = reactExports.useCallback((e) => {
    ...
    const currentIdx = selectedItemKey ? filteredItems.findIndex((item) => item.key === selectedItemKey) : -1;
    if (e.key === "ArrowDown") {
      e.preventDefault();
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "virtual-list-container", children: [
    searchable && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "virtual-list-search", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "text",
      ...
      searchQuery && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "search-clear", onClick: () => setSearchQuery(""), children: "✕" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "virtual-list-stats", children: filteredItems.length !== items.length ? `${filteredItems.length} / ${items.length}` : `${items.length} 项` }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        tabIndex: 0,
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { height: totalHeight, position: "relative" }, children: visibleItems.map((item, i) => {
            ...
            return /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                children: [
                  item.icon && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "item-icon", children: item.icon }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "item-label", title: item.label, children: item.label })
                ]
              },
              item.key
            );
          }) }),
          filteredItems.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "virtual-list-empty", children: emptyHint })
        ]
      }
    )
  ] });
...
function ExplorerPanel() {
  const { t } = useTranslation();
  const [treeSearch, setTreeSearch] = reactExports.useState("");
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "explorer-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "explorer-section explorer-wl-list", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "panel-header", children: t("panel.widgetLooks") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(WidgetLookList, {})
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "explorer-divider" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "explorer-section explorer-tree", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "panel-header", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t("panel.layerTree") }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tree-search-bar", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "text",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(LayerTree, { searchQuery: treeSearch })
    ] })
  ] });
}
function WidgetLookList() {
  const { t } = useTranslation();
  const activeFile = useEditorStore((s) => s.files.activeFilePath);
  const openedFiles = useEditorStore((s) => s.files.openedFiles);
...
function LayerTree({ searchQuery }) {
  const { t } = useTranslation();
  const wlName = useEditorStore((s) => s.selection.widgetLookName);
  const activeFile = useEditorStore((s) => s.files.activeFilePath);
  const openedFiles = useEditorStore((s) => s.files.openedFiles);
  ...
  if (!tree) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "panel-content empty-hint", children: t("panel.selectWidgetLook") });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "panel-content layer-tree-content", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
    TreeNode,
    {
      node: tree,
...
function TreeNode({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tree-node", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          ...
          isStateNode && stateName ? /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              type: "checkbox",
          ...
          ) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tree-icon", children: node.icon }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tree-label", children: node.name })
        ]
      }
    ),
    expanded && hasChildren && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tree-children", children: node.children.map((child) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      TreeNode,
      {
        node: child,
        selectedIds,
...
const OPERATOR_TYPES = ["Add", "Subtract", "Multiply", "Divide"];
const FONT_METRICS = ["LineSpacing", "Baseline", "HorzExtent"];
...
function DimEditor({
  ...
  const exprPreview = reactExports.useMemo(() => formatDimExpr(value), [value]);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-editor", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-editor-header", onClick: () => !disabled && setExpanded(!expanded), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dim-collapse-icon", children: expanded ? "▼" : "▶" }),
      label && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dim-label", children: label }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dim-expr-preview", title: exprPreview, children: exprPreview.length > 40 ? exprPreview.slice(0, 37) + "..." : exprPreview }),
      !disabled && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dim-type-badge", children: value.type })
    ] }),
    expanded && !disabled && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dim-editor-body", children: /* @__PURE__ */ jsxRuntimeExports.jsx(DimNodeTypeSwitcher, { value, onChange, parentSize }) })
  ] });
}
function DimNodeTypeSwitcher({
  ...
  const { t } = useTranslation();
  const [switchingType, setSwitchingType] = reactExports.useState(null);
  const handleTypeSwitch = reactExports.useCallback((newType) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-type-switcher", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dim-type-buttons", children: DIM_NODE_TYPES.map((dt) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        className: `dim-type-btn ${dt === value.type ? "active" : ""}`,
        onClick: () => handleTypeSwitch(dt),
        title: t("dim.switchTo", { type: dt }),
        children: dt.replace("Dim", "")
      },
      dt
    )) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-type-editor", children: [
      value.type === "AbsoluteDim" && /* @__PURE__ */ jsxRuntimeExports.jsx(AbsoluteDimEditor, { value, onChange }),
      value.type === "UnifiedDim" && /* @__PURE__ */ jsxRuntimeExports.jsx(UnifiedDimEditor, { value, onChange, parentSize }),
      value.type === "ImageDim" && /* @__PURE__ */ jsxRuntimeExports.jsx(ImageDimEditor, { value, onChange }),
      value.type === "WidgetDim" && /* @__PURE__ */ jsxRuntimeExports.jsx(WidgetDimEditor, { value, onChange }),
      value.type === "FontDim" && /* @__PURE__ */ jsxRuntimeExports.jsx(FontDimEditor, { value, onChange }),
      value.type === "PropertyDim" && /* @__PURE__ */ jsxRuntimeExports.jsx(PropertyDimEditor, { value, onChange }),
      value.type === "DimOperator" && /* @__PURE__ */ jsxRuntimeExports.jsx(DimOperatorEditor, { value, onChange, parentSize })
    ] })
  ] });
}
function AbsoluteDimEditor({
  ...
  const { t } = useTranslation();
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dim-fields", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.value") }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "input",
      {
        type: "number",
...
function UnifiedDimEditor({
  ...
  const { t } = useTranslation();
  const computedValue = parentSize !== void 0 ? value.scale * parentSize + value.offset : null;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-fields", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.scale") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "number",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.offset") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "number",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.dimType") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "select",
        {
          value: value.dimType,
          onChange: (e) => onChange({ ...value, dimType: e.target.value }),
          children: DIM_TYPES.map((dt) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: dt, children: dt }, dt))
        }
      )
    ] }),
    computedValue !== null && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-computed", children: [
      "≈ ",
...
function ImageDimEditor({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-fields", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.imageset") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "text",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.image") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "text",
...
function WidgetDimEditor({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-fields", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.widget") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "text",
          value: value.widget || "",
          onChange: (e) => {
            const newWidget = e.target.value || void 0;
...
function FontDimEditor({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-fields", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.fontDim") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "text",
          value: value.font || "",
          onChange: (e) => {
            const newFont = e.target.value || void 0;
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.metric") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "select",
        {
          value: value.metric,
          onChange: (e) => onChange({ ...value, metric: e.target.value }),
          children: FONT_METRICS.map((fm) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: fm, children: fm }, fm))
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.padding") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "number",
...
function PropertyDimEditor({
  ...
  const { t } = useTranslation();
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dim-fields", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.propertyName") }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "input",
      {
        type: "text",
...
function DimOperatorEditor({
  ...
  const { t } = useTranslation();
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-operator-editor", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: t("dim.operatorLabel") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "select",
        {
          value: value.op,
          onChange: (e) => onChange({ ...value, op: e.target.value }),
          children: OPERATOR_TYPES.map((op) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: op, children: op }, op))
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-operator-children", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-operator-left", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-operator-label", children: [
          t("dim.left"),
          ":"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-operator-right", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dim-operator-label", children: [
          t("dim.right"),
          ":"
        ] }),
...
function AreaEditor({
  ...
  const { t } = useTranslation();
  const handleDimChange = reactExports.useCallback(
    (field, newDim) => {
      onChange({ ...area, [field]: newDim });
    },
    [area, onChange]
  );
  const hasWidth = area.width !== void 0;
  const hasRight = area.right !== void 0;
  const hasHeight = area.height !== void 0;
  const hasBottom = area.bottom !== void 0;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "area-editor", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "area-editor-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "area-section-title", children: t("dim.position") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        DimEditor,
        {
          label: "Left",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "area-editor-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "area-section-title", children: t("dim.horizontalSize") }),
      hasWidth && /* @__PURE__ */ jsxRuntimeExports.jsx(
        DimEditor,
        {
          label: "Width",
          value: area.width,
      ...
      hasRight && /* @__PURE__ */ jsxRuntimeExports.jsx(
        DimEditor,
        {
          label: "Right",
          value: area.right,
      ...
      !hasWidth && !hasRight && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dim-field-empty", children: t("dim.noHorizontalSize") })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "area-editor-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "area-section-title", children: t("dim.verticalSize") }),
      hasHeight && /* @__PURE__ */ jsxRuntimeExports.jsx(
        DimEditor,
        {
          label: "Height",
          value: area.height,
      ...
      hasBottom && /* @__PURE__ */ jsxRuntimeExports.jsx(
        DimEditor,
        {
          label: "Bottom",
          value: area.bottom,
      ...
      !hasHeight && !hasBottom && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dim-field-empty", children: t("dim.noVerticalSize") })
    ] })
  ] });
}
function InspectorPanel() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inspector-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inspector-tabs", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: `inspector-tab ${activeTab === "properties" ? "active" : ""}`,
          onClick: () => setActiveTab("properties"),
          children: t("panel.properties")
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          children: t("panel.images")
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "inspector-content", children: activeTab === "properties" ? /* @__PURE__ */ jsxRuntimeExports.jsx(PropertiesSection, {}) : /* @__PURE__ */ jsxRuntimeExports.jsx(ResourcesSection, {}) })
  ] });
}
function PropertiesSection() {
  ...
  const parentWidth = useEditorStore((s) => s.preview.parentWidth);
  const parentHeight = useEditorStore((s) => s.preview.parentHeight);
  const fileState = reactExports.useMemo(() => activeFile ? openedFiles.get(activeFile) ?? null : null, [activeFile, openedFiles]);
  const wl = reactExports.useMemo(() => {
    if (!fileState || !wlName) return null;
    return fileState.loadedWidgetLooks.get(wlName) || null;
  }, [fileState, wlName]);
  const selectedComponent = reactExports.useMemo(() => {
    if (!fileState || !wl || selectedNodeIds.length === 0) return null;
    const componentRef = parseComponentNodeId(selectedNodeIds[0]);
    if (!componentRef) return null;
    const targetWidgetLook = componentRef.look ? fileState.loadedWidgetLooks.get(componentRef.look) : wl;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "properties-content", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(PropertySection, { title: t("property.widgetLook"), children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      EditableProperty,
      {
    ...
    wl.propertyDefinitions.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(PropertySection, { title: t("property.propertyDefinitions"), children: wl.propertyDefinitions.map((pd, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      EditableProperty,
      {
        label: pd.name,
    ...
    wl.properties.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(PropertySection, { title: t("property.properties"), children: wl.properties.map((p, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      EditableProperty,
      {
        label: p.name,
        value: p.value,
    ...
    selectedComponent && selectedComponent.type === "frame" && selectedComponent.component && /* @__PURE__ */ jsxRuntimeExports.jsx(
    ...
    selectedComponent && selectedComponent.type === "imagery" && selectedComponent.component && /* @__PURE__ */ jsxRuntimeExports.jsx(
    ...
    selectedComponent && selectedComponent.type === "text" && selectedComponent.component && /* @__PURE__ */ jsxRuntimeExports.jsx(
    ...
    wl.imagerySections.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(PropertySection, { title: t("property.imagerySections"), children: wl.imagerySections.map((sec, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "property-label", children: [
        "🎨 ",
        sec.name
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "property-value-readonly", children: [
        sec.frameComponents.length,
        t("property.frame"),
        " ",
        sec.imageryComponents.length,
        t("property.imagery"),
        " ",
        sec.textComponents.length,
        t("property.textComp")
      ] })
    ] }, `sec-${i}`)) }),
    wl.stateImagerys.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(PropertySection, { title: t("property.stateImagery"), children: wl.stateImagerys.map((si, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "property-label", children: [
        "⚡ ",
        si.name
      ] }),
...
function FrameComponentProperties({
  ...
  const handleAreaChange = reactExports.useCallback((newArea) => {
    updateComponentArea(wl, wlName, sectionName, "frame", index2, newArea);
  }, [wl, wlName, sectionName, index2]);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(PropertySection, { title: `Frame #${index2} (${sectionName})`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      AreaEditor,
      {
        area: component.area,
    ...
    component.vertFormat && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "VertFormat" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.vertFormat })
    ] }),
    component.horzFormat && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "HorzFormat" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.horzFormat })
    ] }),
    Object.entries(component.images).map(([type, ref]) => {
      if (!ref) return null;
      return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row image-ref", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: type }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "property-value-readonly", children: [
          ref.imageset,
          "/",
          ref.image
    ...
    Object.entries(component.imageProperties ?? {}).map(([type, propertyName]) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "property-label", children: [
        type,
        " ImageProperty"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: propertyName })
    ] }, `prop-${type}`)),
    component.colourRectProperty && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "ColourRectProperty" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.colourRectProperty })
    ] })
  ] });
}
function ImageryComponentProperties({
  ...
  const handleAreaChange = reactExports.useCallback((newArea) => {
    updateComponentArea(wl, wlName, sectionName, "imagery", index2, newArea);
  }, [wl, wlName, sectionName, index2]);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(PropertySection, { title: `Imagery #${index2} (${sectionName})`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      AreaEditor,
      {
        area: component.area,
    ...
    component.image && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row image-ref", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "Image" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "property-value-readonly", children: [
        component.image.imageset,
        "/",
        component.image.image
      ] })
    ] }),
    component.imageProperty && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "ImageProperty" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.imageProperty })
    ] }),
    component.colourRectProperty && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "ColourRectProperty" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.colourRectProperty })
...
function TextComponentProperties({
  ...
  const handleAreaChange = reactExports.useCallback((newArea) => {
    updateComponentArea(wl, wlName, sectionName, "text", index2, newArea);
  }, [wl, wlName, sectionName, index2]);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(PropertySection, { title: `Text #${index2} (${sectionName})`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      AreaEditor,
      {
        area: component.area,
    ...
    component.font && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "Font" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.font })
    ] }),
    component.text && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "Text" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.text })
    ] }),
    component.borderEnableProperty && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "BorderEnable" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.borderEnableProperty })
    ] }),
    component.borderColourProperty && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "BorderColour" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.borderColourProperty })
    ] }),
    component.colourRectProperty && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: "ColourRectProperty" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: component.colourRectProperty })
...
function ResourcesSection() {
  ...
  const openedFiles = useEditorStore((s) => s.files.openedFiles);
  const [searchQuery, setSearchQuery] = reactExports.useState("");
  const [selectedImageset, setSelectedImageset] = reactExports.useState(null);
  const [viewMode, setViewMode] = reactExports.useState("grid");
  const fileState = activeFile ? openedFiles.get(activeFile) : null;
  const imagesets = fileState?.imagesets;
  const imagesetNames = reactExports.useMemo(
    () => imagesets ? Array.from(imagesets.keys()) : [],
    [imagesets]
  );
  const imageEntries = reactExports.useMemo(() => {
    const entries = [];
    if (!imagesets) return entries;
    const targetSets = selectedImageset ? [[selectedImageset, imagesets.get(selectedImageset) ?? null]] : Array.from(imagesets.entries()).map(([name, imageset]) => [name, imageset]);
  ...
  const filteredEntries = reactExports.useMemo(() => {
    ...
    const q = searchQuery.toLowerCase();
    return imageEntries.filter(
      (e) => e.name.toLowerCase().includes(q) || e.imageset.toLowerCase().includes(q)
    );
  }, [imageEntries, searchQuery]);
  const totalImages = reactExports.useMemo(() => {
    if (!imagesets) return 0;
    let count = 0;
    for (const is2 of imagesets.values()) {
      count += is2.subImages.size;
    }
    return count;
  }, [imagesets]);
  const handleSelectImage = reactExports.useCallback((entry) => {
    const ref = `${entry.imageset}/${entry.name}`;
    navigator.clipboard.writeText(ref).catch(() => {
    });
  }, []);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "resources-content", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "resources-toolbar", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "text",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "view-mode-toggle", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "imageset-selector compact", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `image-list ${viewMode}`, children: [
      filteredEntries.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-hint", children: imagesetNames.length === 0 ? t("imageBrowser.noImagesets") : t("imageBrowser.noMatch") }),
      viewMode === "grid" ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "image-grid", children: filteredEntries.map((entry) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
...
${entry.def.width}×${entry.def.height}`,
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "image-thumbnail", children: entry.dataUrl ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: entry.dataUrl, alt: entry.name }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "image-placeholder", children: "?" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "image-grid-label", title: entry.name, children: entry.name })
          ]
        },
        `${entry.imageset}/${entry.name}`
      )) }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "image-list-view", children: filteredEntries.map((entry) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          className: "image-list-item",
          onClick: () => handleSelectImage(entry),
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "image-list-thumb", children: entry.dataUrl ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: entry.dataUrl, alt: entry.name }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "?" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "image-list-info", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "image-list-name", children: entry.name }),
...
function PropertySection({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-section", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section-title", onClick: () => setCollapsed(!collapsed), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "collapse-icon", children: collapsed ? "▶" : "▼" }),
      title
    ] }),
    !collapsed && children
  ] });
...
function EditableProperty({
  ...
  if (readOnly || !onChange) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: label }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-value-readonly", children: value })
    ] });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "property-row", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "property-label", children: label }),
...
function updatePropertyDefinition(wl, wlName, index2, newValue) {
  const store = useEditorStore.getState();
  const newWl = JSON.parse(JSON.stringify(wl));
  newWl.propertyDefinitions[index2].initialValue = newValue;
  store.pushHistory({
    description: `Change PropertyDefinition "${wl.propertyDefinitions[index2].name}"`,
    widgetLookName: wlName,
    snapshot: JSON.stringify(wl),
    affectedPaths: [`propertyDefinitions.${index2}.initialValue`]
  });
  store.updateWidgetLook(wlName, newWl);
}
function updateProperty(wl, wlName, index2, newValue) {
  const store = useEditorStore.getState();
  const newWl = JSON.parse(JSON.stringify(wl));
  newWl.properties[index2].value = newValue;
  store.pushHistory({
    description: `Change Property "${wl.properties[index2].name}"`,
    widgetLookName: wlName,
    snapshot: JSON.stringify(wl),
    affectedPaths: [`properties.${index2}.value`]
  });
  store.updateWidgetLook(wlName, newWl);
}
function updateComponentArea(wl, wlName, sectionName, componentType, index2, newArea) {
  const store = useEditorStore.getState();
  const newWl = JSON.parse(JSON.stringify(wl));
  ...
  switch (componentType) {
    case "frame":
      if (section.frameComponents[index2]) {
        section.frameComponents[index2].area = newArea;
      }
      break;
    case "imagery":
      if (section.imageryComponents[index2]) {
        section.imageryComponents[index2].area = newArea;
      }
      break;
    case "text":
      if (section.textComponents[index2]) {
        section.textComponents[index2].area = newArea;
      }
      break;
  }
  store.pushHistory({
    description: `Edit Area: ${sectionName}/${componentType}#${index2}`,
    widgetLookName: wlName,
    snapshot: JSON.stringify(wl),
    affectedPaths: [`${sectionName}.${componentType}[${index2}].area`]
  });
  store.updateWidgetLook(wlName, newWl);
}
function App() {
  ...
  if (!ready) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "loading", children: t("app.loading") });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-layout", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "top-bar", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Toolbar, {}) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "main-content", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "left-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ExplorerPanel, {}) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "center-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsx(LiveViewport, {}) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "right-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsx(InspectorPanel, {}) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(StatusBar, {})
  ] });
}
async function saveCurrentFile(store) {
...
function Toolbar() {
  ...
  const openFile = reactExports.useCallback(async () => {
    ...
    try {
      ...
      if (resources && resources.imagesets) {
        ...
        const imagesetResources = [];
        for (const data of Object.values(resources.imagesets)) {
          const subImages = /* @__PURE__ */ new Map();
          if (data.subImages) {
            for (const [subName, subDef] of Object.entries(data.subImages)) {
              subImages.set(subName, subDef);
  ...
  const resetZoom = reactExports.useCallback(() => {
    useEditorStore.getState().setViewport({ scale: 1, offsetX: 0, offsetY: 0 });
  }, []);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "toolbar", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "toolbar-group", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: openFile, title: t("toolbar.openTitle"), children: t("toolbar.open") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: saveFile, title: t("toolbar.saveTitle"), disabled: !dirty, children: t("toolbar.save") })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "toolbar-group", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleUndo, title: t("toolbar.undoTitle"), children: "↩" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleRedo, title: t("toolbar.redoTitle"), children: "↪" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "toolbar-group", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: zoomOut, title: t("toolbar.zoomOutTitle"), children: "−" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "zoom-label", children: [
        Math.round(scale * 100),
        "%"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: zoomIn, title: t("toolbar.zoomInTitle"), children: "+" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: resetZoom, title: t("toolbar.resetZoomTitle"), children: "⊡" })
    ] })
  ] });
}
function StatusBar() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "status-bar", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: activeFile ? activeFile.split(/[\\/]/).pop() : t("status.noFile") }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: wlName || t("status.noSelection") }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: dirty ? `● ${t("status.modified")}` : `○ ${t("status.saved")}` }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
      t("status.undoCount", { count: undoCount }),
      " | ",
      t("status.redoCount", { count: redoCount })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t("status.textures", { cached: cacheStats.cachedCount, max: cacheStats.maxSize }) })
  ] });
}
