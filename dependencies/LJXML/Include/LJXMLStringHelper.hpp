...
namespace LJXMLStringHelper
{
	...
	inline bool reallyLoadFromMemory(unsigned char* buffer, std::size_t size, std::wstring& ws, EncodeLJ& bufferCode)
	{
		...
		if (bufferCode == LJ_UTF_16)
		{
			...
			m_buffer = new unsigned char[(characterCount + 1)*sizeof(wchar_t)];
			...
			dst[characterCount] = 0;
			ws.assign(dst, characterCount);
		}
		else if (bufferCode == LJ_UTF_16_BIG_ENDIAN)
		{
			...
			*dst = 0;
			ws.assign(reinterpret_cast<wchar_t*>(m_buffer), characterCount);
		}
		else if (bufferCode == LJ_UTF_8 || bufferCode == LJ_UTF_8_NO_MARK || (bufferCode == LJ_ANSI && !multiBytes))
		{
			m_buffer = new unsigned char[(size + 1) * sizeof(wchar_t)];
			...
			ws.assign(text, characterCount);
		}
		else if (bufferCode == LJ_ANSI && multiBytes)
		{
			std::string bytes(reinterpret_cast<const char*>(buffer), size);
			ws = LJXMLStringHelper::s2ws(bytes);
