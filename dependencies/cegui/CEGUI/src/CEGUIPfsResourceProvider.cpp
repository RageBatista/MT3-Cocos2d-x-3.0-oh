...
namespace CEGUI
{
	...
	void PFSResourceProvider::loadRawDataContainer(const String& filename, LJFM::LJFMID& output, const String& resourceGroup)
	{
		...
		// 日志：方法入口 - 记录请求加载的资源路径和资源组
		CEGUI::Logger::getSingleton().logEvent(
			"[PFSResourceProvider::loadRawDataContainer] 请求加载资源 - 文件名: " + filename +
			", 资源组: " + (resourceGroup.empty() ? "(默认)" : resourceGroup),
			CEGUI::Informative);
		
		if (filename.empty())
		{
			CEGUI::Logger::getSingleton().logEvent(
				"[PFSResourceProvider::loadRawDataContainer] 错误 - 文件名为空",
				CEGUI::Errors);
		...
		// 日志：完整路径
		CEGUI::Logger::getSingleton().logEvent(
			"[PFSResourceProvider::loadRawDataContainer] 完整路径: " + final_filename,
			CEGUI::Informative);
		...
		if( !Afile.Open(pfsname, LJFM::FM_EXCL, LJFM::FA_RDONLY ))
		{
			// 日志：文件打开失败
			CEGUI::Logger::getSingleton().logEvent(
				"[PFSResourceProvider::loadRawDataContainer] 文件打开失败 - 路径: " + final_filename,
				CEGUI::Errors);
			return ;
		}
		...
		CEGUI::Logger::getSingleton().logEvent(
			"[PFSResourceProvider::loadRawDataContainer] 文件打开成功 - 大小: " +
			PropertyHelper::intToString(file_size) + " 字节",
			CEGUI::Informative);
		
		if( file_size == 0 )
		{
			CEGUI::Logger::getSingleton().logEvent(
				"[PFSResourceProvider::loadRawDataContainer] 文件大小为0 - 路径: " + final_filename,
				CEGUI::Errors);
			return ;
		}
		...
		// 日志：文件读取成功
		CEGUI::Logger::getSingleton().logEvent(
			"[PFSResourceProvider::loadRawDataContainer] 文件读取成功 - 读取字节数: " +
