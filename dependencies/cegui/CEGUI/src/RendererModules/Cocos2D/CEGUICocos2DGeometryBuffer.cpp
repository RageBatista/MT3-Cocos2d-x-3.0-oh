...
void Cocos2DGeometryBuffer::draw() const
{
	...
	for (int pass = 0; pass < pass_count; ++pass)
	{
		...
		for (; i != d_batches.end(); ++i)
		{
			...
			//cocos2d::ccGLBindTexture2D( pTexture->getTextureName() );
			if (pTexture->isEtc())
			{
				cocos2d::CCGLProgram* etcProgram = cocos2d::CCShaderCache::sharedShaderCache()->programForKey(kCCShader_PositionTextureColorEtc);
				if (!etcProgram)
				{
					pos += batchVertexCount;
					d_RenderSuccess = false;
				...
				cocos2d::ccGLBindTexture2D(pTexture->getAlphaName());
			}
			else
			{
