...
class COCOS2D_GUIRENDERER_API Cocos2DRenderer : public Renderer
{
    struct StateManager
    {
        GLboolean d_ScissorEnabled;
        GLboolean d_BlendEnabled;
        GLboolean d_DepthEnabled;
        GLboolean d_StencilEnabled;
        GLboolean d_CullEnabled;
        GLboolean d_ColorMask[4];
        GLboolean d_VertexAttribEnabled[3];
        GLint d_ScissorBox[4];
        GLint d_Viewport[4];
        GLint d_CurrentProgram;
		std::string d_ShaderKey;
        GLint d_ActiveTexture;
        GLint d_TextureBindings[2];
        GLint d_BlendSrcRGB;
        GLint d_BlendDstRGB;
        GLint d_BlendSrcAlpha;
        GLint d_BlendDstAlpha;
        GLint d_UnpackAlignment;
        GLint d_ArrayBuffer;
        GLint d_ElementArrayBuffer;
        GLint d_VertexAttribSize[3];
        GLint d_VertexAttribType[3];
        GLint d_VertexAttribNormalized[3];
        GLint d_VertexAttribStride[3];
        GLint d_VertexAttribBuffer[3];
        void* d_VertexAttribPointer[3];
    };
public:
...
public:
    bool OnFileLoaded(ITask* aPTask);
    bool OnImageParsed(ITask* aPTask);
    bool OnImageLoadFailed(ITask* aPTask, Cocos2DTexture* texture, const String& filename);
...
private:
    ...
    TextureList d_pendingDestroyTextures;
    ...
    std::vector<StateManager> d_stateStack;
    std::vector<StateManager> d_externalStateStack;
	unsigned int d_externalPassDepth;

    void captureRenderState(StateManager& state);
    void restoreRenderState(const StateManager& state);
    void detachTexture(Cocos2DTexture* texture, bool deleteLoadedTask);
    void collectPendingTextureDeletes();
    void notifyGeometryBuffersTextureReleased(Cocos2DTexture* texture);
