...
namespace LJFMX
{
	...
	bool CLJFSZipFile::Open(void* FileInfo)
	{
		...
		if (FI->m_FileArea == 0)
		{
			...
			if (pData == NULL)
			{
				std::string LegacyPFN = ws2s(FI->m_PathFileName);
				if (LegacyPFN != PFN)
				{
					pData = cocos2d::CCFileUtils::sharedFileUtils()->getFileData(LegacyPFN.c_str(), "r", &ulSize);
				}
			}
		}
		else
		{
			...
			Close();
			fsize_t dataSize = pLJFSFile->GetSize();
			pData = dataSize > 0 ? (unsigned char*)malloc(dataSize) : NULL;
			...
			if (pData != NULL)
			{
				...
				pLJFSFile->ReadAll(pData, dataSize);
			}
			delete pLJFSFile;
		}
		...
		if (FI->m_CodeType == 1)
		{
			if (!DeCode(FI, m_pDataBegin, m_DataDeCode))
			{
				Close();
				return false;
			}
		...
		{
			if (!UnCompress(FI, m_pDataBegin, m_DataUnCompress))
			{
				Close();
				return false;
			}
	...
	{
		return LJUnCompress((LJFP_FileInfo*)pFI, pDataSrc, pDataDst) == 0;
	}
	bool CLJFSZipFile::DeCode(void* pFI, unsigned char* pDataSrc, unsigned char*& pDataDst)
	{		
		return LJDeCode((LJFP_FileInfo*)pFI, pDataSrc, pDataDst) == 0;
	}
