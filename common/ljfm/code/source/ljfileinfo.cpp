...
{
	if (FI == NULL || (pSrc == NULL && FI->m_Size > 0))
	{
		pDst = NULL;
		return -1;
	}
	if (FI->m_SizeOriginal == 0)
	{
		pDst = NULL;
		return 0;
	}
...
bool MergerPackInfo(LJFP_PackInfo* pPIBase, LJFP_PackInfo* pPINew, LJFP_PackInfo*& pPIResult)
{
	...
	// 先放入 base 全量映射，再用 new 覆盖变更项。
	// 这样可兼容服务端仅下发增量 fl.ljpi 的场景，避免丢失未变更资源映射。
	for (unsigned int i = 0; i < pPIBase->GetFileCount(); i++)
	{
		LJFP_FileInfo* pFIBaseClone = pPIBase->GetFileInfo(i)->Clone();
		pPIResult->AddFileInfo(pFIBaseClone);
	}
	...
	for (unsigned int i = 0; i < pPINew->GetFileCount(); i++)
	{
		...
		if (pFIBase == NULL)
		{
			pPIResult->AddFileInfo(pFINew->Clone());
		}
		else
		{
			if (pFIBase->m_CRC32 != pFINew->m_CRC32 || pFIBase->m_Size != pFINew->m_Size)
			{
				// AddFileInfo 会覆盖同 CRC32 的映射到 new 条目。
				pPIResult->AddFileInfo(pFINew->Clone());
			}
		}
	}
