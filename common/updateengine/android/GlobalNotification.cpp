...
void onNotifyFail()
{
	...
	if (MT3::JniHelper::getStaticMethodInfo(t, 
				...
				"()V"))
	{   
		LOGD("onNotifyFail method found");
		t.env->CallStaticVoidMethod(t.classID, t.methodID);
		t.env->DeleteLocalRef(t.classID);
	}
	else
	{
		LOGD("onNotifyFail method not found");
	}
}
...
{
	LOGD("onNotifyShowForm invoke type=%d size=%d url=%s",
		FormType,
		DownloadSize,
		SHARE_Wstring2String(AppURL).c_str());
    MT3::JniMethodInfo t;
    if (MT3::JniHelper::getStaticMethodInfo(t,
                "com/locojoy/mini/mt3/GameApp",
                "NotifyShowForm",
    ...
    {
		LOGD("onNotifyShowForm method found");
	...
	{
		LOGD("onNotifyShowForm method not found");
	}
}
