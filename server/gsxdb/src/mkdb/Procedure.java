...
public class Procedure {
	...
	protected void lock(mkdb.TTable<?, ?> table, Collection<?> keys) {
		Lockeys.lock(table, keys);
	}

	/**
	 * 获得单条记录锁，对锁的进行顺序并且加入当前事务。
	 * @param lockey 锁
	 */
	protected void lock(mkdb.Lockey lockey) {
		Lockeys.lock(lockey);
	}
	 ...
	 * @param lockeys 锁
	 */
	protected void lock(mkdb.Lockey[] lockeys) {
	 ...
	 * @param keys 键集合
	 */
	protected <K, V> void lockGeneric(mkdb.TTable<K, V> table, Collection<V> keys) {
		Lockeys.lock(table, keys);
	}
