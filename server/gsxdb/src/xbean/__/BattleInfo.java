...
public final class BattleInfo extends mkdb.XBean implements xbean.BattleInfo {
	...
	private mkdb.util.SetX<Long> feedbacks; // 角色id
	...
	private java.util.LinkedList<Long> quitteamroleids; // 战斗中有退队角色,战后自动退队，作者 changhao
	private java.util.LinkedList<Long> absentteamroleids; // 战斗中有暂离角色,战后自动暂离，作者 changhao
	private java.util.LinkedList<Long> teamidlist; // 战斗里所有队伍id，作者 changhao
	...
	@Override
	public java.util.Set<Long> getFeedbacks() { // 角色id
		_xdb_verify_unsafe_();
		return mkdb.Logs.logSet(new mkdb.LogKey(this, "feedbacks"), feedbacks);
	}

	public java.util.Set<Long> getFeedbacksAsData() { // 角色id
		_xdb_verify_unsafe_();
	...
	@Override
	public java.util.List<Long> getQuitteamroleids() { // 战斗中有退队角色,战后自动退队，作者 changhao
		_xdb_verify_unsafe_();
		return mkdb.Logs.logList(new mkdb.LogKey(this, "quitteamroleids"), quitteamroleids);
	}

	public java.util.List<Long> getQuitteamroleidsAsData() { // 战斗中有退队角色,战后自动退队，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public java.util.List<Long> getAbsentteamroleids() { // 战斗中有暂离角色,战后自动暂离，作者 changhao
		_xdb_verify_unsafe_();
		return mkdb.Logs.logList(new mkdb.LogKey(this, "absentteamroleids"), absentteamroleids);
	}

	public java.util.List<Long> getAbsentteamroleidsAsData() { // 战斗中有暂离角色,战后自动暂离，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public java.util.List<Long> getTeamidlist() { // 战斗里所有队伍id，作者 changhao
		_xdb_verify_unsafe_();
		return mkdb.Logs.logList(new mkdb.LogKey(this, "teamidlist"), teamidlist);
	}

	public java.util.List<Long> getTeamidlistAsData() { // 战斗里所有队伍id，作者 changhao
		_xdb_verify_unsafe_();
	...
	public static final class Data implements xbean.BattleInfo {
		...
		private long scriptplaytime; // 脚本播放时间（ms），根据脚本计算出来的播放时间，是脚本播放的最短时间，最短1秒
		private java.util.HashMap<Integer, xbean.Decision> decisions; // 战斗单元决策结果
		private java.util.HashSet<Long> feedbacks; // 角色id
		private int battleresult; // 战斗是否结束及战斗结果，参考BattleField.BATTLE_XXX,默认为2（BATTLE_NOT_END）
		private java.util.HashMap<Integer, Integer> roundresult; // 战斗单位：战斗结果
		private java.util.HashMap<Integer, xbean.Protectors> protectormap; // 被保护人：保护者列表
		private java.util.concurrent.ScheduledFuture<?> timeout; // 超时检测
