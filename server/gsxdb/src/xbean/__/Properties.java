...
public final class Properties extends mkdb.XBean implements xbean.Properties {
	...
	private java.util.HashMap<Integer, xbean.Jingmai> jingmai; // 拥有的称谓列表
	...
	private java.util.LinkedList<Long> jblb; // 结拜列表，存储结拜角色的ID
	private int exitstate; // 角色退出公会方式，0主动退出    1被踢出    2解散          key，作者 hzl
	...
	private int energy; // 活力，作者 changhao
	private int playtimetimes; // 战斗播放时长倍数 10为1倍速 liuxinhua
	private int rolezonghemaxscore; // 人物综合实力历史最高评分，作者 changhao
	private int skillscore; // 技能评分，作者 changhao
	private int petscore; // 最强单宠物评分，作者 changhao
	private int manypetscore; // 多宠物评分，作者 changhao
	private int levelscore; // 等级评分，作者 changhao
	private int xiulianscroe; // 修炼评分，作者 changhao
	private int rolescore; // 人物评分，作者 changhao
	private int equipscore; // 装备评分，作者 changhao
	private long historymaxprofessioncontribute; // 历史最大职业贡献度，作者 changhao
	private int ride; // 当前坐骑，作者 changhao
	...
	private java.util.HashMap<Integer, Integer> shizhuang; // 时装衣橱，key=时装id value=时装id
	private int cruise; // 巡游状态，默认0没有巡游，否则为巡游id
	private int cruise2; // 巡游2，作者 changhao
	private int cruise3; // 巡游3，作者 changhao
	...
	private long expiretime; // 点卡结束时间(点卡服专用)，作者 changhao
	private int firstprompt; // 还没提示过就是0(点卡服专用)，作者 changhao
	...
	private int bsitem; // 
	private int zqitem; // 
	private java.util.HashMap<Integer, Integer> zuoqi; // 坐骑信息
	private java.util.LinkedList<String> benefitcode; // 
	private long partnerid; // 
	private java.util.HashMap<Integer, Integer> qlgmap; // 潜灵果
	private int qlgcurrextra; // 潜灵果额外属性表当前使用的数量
	private java.util.HashMap<Integer, xbean.FunOpenClose> qlgextrapromap; // 潜灵果额外属性表 key 存储的开启潜灵果数量 value 存储的额外属性点
	...
	Properties(int __, mkdb.XBean _xp_, String _vn_) {
		...
		jingmai = new java.util.HashMap<Integer, xbean.Jingmai>();
		scheme = 1;
		schemechanges = 0;
		fightpetkey = -1;
		...
		clankey = -1;
		oldclankey = -1;
		jblb = new java.util.LinkedList<Long>();
		exitstate = -1;
		onlinetime = -1;
		offlinetime = -1;
		logintimesofday = 0;
		...
		shizhuang = new java.util.HashMap<Integer, Integer>();
		cruise = 0;
		cruise2 = 0;
		cruise3 = 0;
		...
		zuoqi = new java.util.HashMap<Integer, Integer>();
		benefitcode = new java.util.LinkedList<String>();
		qlgmap = new java.util.HashMap<Integer, Integer>();
		qlgcurrextra = 0;
		qlgextrapromap = new java.util.HashMap<Integer, xbean.FunOpenClose>();
	}
	...
	private void assign(Properties _o_) {
		...
		jingmai = new java.util.HashMap<Integer, xbean.Jingmai>();
		for (java.util.Map.Entry<Integer, xbean.Jingmai> _e_ : _o_.jingmai.entrySet())
			jingmai.put(_e_.getKey(), new Jingmai(_e_.getValue(), this, "jingmai"));
		scheme = _o_.scheme;
		...
		shizhuang = new java.util.HashMap<Integer, Integer>();
		for (java.util.Map.Entry<Integer, Integer> _e_ : _o_.shizhuang.entrySet())
			shizhuang.put(_e_.getKey(), _e_.getValue());
		cruise = _o_.cruise;
		cruise2 = _o_.cruise2;
		cruise3 = _o_.cruise3;
		fighteai = new java.util.LinkedList<xbean.BattleAI>();
		...
		zhuansheng = _o_.zhuansheng;
		bsitem = _o_.bsitem;
		zqitem = _o_.zqitem;
		zuoqi = new java.util.HashMap<Integer, Integer>();
		for (java.util.Map.Entry<Integer, Integer> _e_ : _o_.zuoqi.entrySet())
			zuoqi.put(_e_.getKey(), _e_.getValue());
		benefitcode = new java.util.LinkedList<String>();
		benefitcode.addAll(_o_.benefitcode);
		partnerid = _o_.partnerid;
		qlgmap = new java.util.HashMap<Integer, Integer>();
		for (java.util.Map.Entry<Integer, Integer> _e_ : _o_.qlgmap.entrySet())
			qlgmap.put(_e_.getKey(), _e_.getValue());
		qlgcurrextra = _o_.qlgcurrextra;
		qlgextrapromap = new java.util.HashMap<Integer, xbean.FunOpenClose>();
		for (java.util.Map.Entry<Integer, xbean.FunOpenClose> _e_ : _o_.qlgextrapromap.entrySet())
			qlgextrapromap.put(_e_.getKey(), new FunOpenClose(_e_.getValue(), this, "qlgextrapromap"));
	}
	...
	public final OctetsStream marshal(OctetsStream _os_) {
		...
		_os_.compact_uint32(jingmai.size());
		for (java.util.Map.Entry<Integer, xbean.Jingmai> _e_ : jingmai.entrySet())
		{
			_os_.marshal(_e_.getKey());
		...
		_os_.compact_uint32(shizhuang.size());
		for (java.util.Map.Entry<Integer, Integer> _e_ : shizhuang.entrySet())
		{
			_os_.marshal(_e_.getKey());
			_os_.marshal(_e_.getValue());
		}
		...
		_os_.marshal(zhuansheng);
		_os_.marshal(bsitem);
		_os_.marshal(zqitem);
		_os_.compact_uint32(zuoqi.size());
		for (java.util.Map.Entry<Integer, Integer> _e_ : zuoqi.entrySet())
		{
			_os_.marshal(_e_.getKey());
			_os_.marshal(_e_.getValue());
		}
		_os_.compact_uint32(benefitcode.size());
		for (String _v_ : benefitcode) {
			_os_.marshal(_v_, mkdb.Const.IO_CHARSET);
		}
		_os_.marshal(partnerid);
		_os_.compact_uint32(qlgmap.size());
		for (java.util.Map.Entry<Integer, Integer> _e_ : qlgmap.entrySet())
		{
			_os_.marshal(_e_.getKey());
			_os_.marshal(_e_.getValue());
		}
		_os_.marshal(qlgcurrextra);
		_os_.compact_uint32(qlgextrapromap.size());
		for (java.util.Map.Entry<Integer, xbean.FunOpenClose> _e_ : qlgextrapromap.entrySet())
		{
			_os_.marshal(_e_.getKey());
			_e_.getValue().marshal(_os_);
		}
	...
	public final OctetsStream unmarshal(OctetsStream _os_) throws MarshalException {
		...
		}
		{
			int size = _os_.uncompact_uint32();
			if (size >= 12) { // {java.util.HashMap} 16 * 0.75 = 12
				jingmai = new java.util.HashMap<Integer, xbean.Jingmai>(size * 2);
			}
			for (; size > 0; --size)
			{
				int _k_ = 0;
				_k_ = _os_.unmarshal_int();
				xbean.Jingmai _v_ = new Jingmai(0, this, "jingmai");
				_v_.unmarshal(_os_);
				jingmai.put(_k_, _v_);
			}
		}
		...
		}
		{
			int size = _os_.uncompact_uint32();
			if (size >= 12) { // {java.util.HashMap} 16 * 0.75 = 12
				shizhuang = new java.util.HashMap<Integer, Integer>(size * 2);
			}
			for (; size > 0; --size)
			{
				int _k_ = 0;
		...
		zhuansheng = _os_.unmarshal_int();
		bsitem = _os_.unmarshal_int();
		zqitem = _os_.unmarshal_int();
		{
			int size = _os_.uncompact_uint32();
			if (size >= 12) { // {java.util.HashMap} 16 * 0.75 = 12
				zuoqi = new java.util.HashMap<Integer, Integer>(size * 2);
			}
			for (; size > 0; --size)
			{
				int _k_ = 0;
		...
		partnerid = _os_.unmarshal_long();
		{
			int size = _os_.uncompact_uint32();
			if (size >= 12) { // {java.util.HashMap} 16 * 0.75 = 12
				qlgmap = new java.util.HashMap<Integer, Integer>(size * 2);
			}
			for (; size > 0; --size)
			{
				int _k_ = 0;
		...
		qlgcurrextra = _os_.unmarshal_int();
		{
			int size = _os_.uncompact_uint32();
			if (size >= 12) { // {java.util.HashMap} 16 * 0.75 = 12
				qlgextrapromap = new java.util.HashMap<Integer, xbean.FunOpenClose>(size * 2);
			}
			for (; size > 0; --size)
			{
				int _k_ = 0;
				_k_ = _os_.unmarshal_int();
				xbean.FunOpenClose _v_ = new FunOpenClose(0, this, "qlgextrapromap");
				_v_.unmarshal(_os_);
				qlgextrapromap.put(_k_, _v_);
			}
		}
		return _os_;
	}
	...
	@Override
	public java.util.Map<Integer, xbean.Jingmai> getJingmai() { // 拥有的称谓列表
		_xdb_verify_unsafe_();
		return mkdb.Logs.logMap(new mkdb.LogKey(this, "jingmai"), jingmai);
	}

	@Override
	public java.util.Map<Integer, xbean.Jingmai> getJingmaiAsData() { // 拥有的称谓列表
		_xdb_verify_unsafe_();
		java.util.Map<Integer, xbean.Jingmai> jingmai;
		Properties _o_ = this;
		jingmai = new java.util.HashMap<Integer, xbean.Jingmai>();
		for (java.util.Map.Entry<Integer, xbean.Jingmai> _e_ : _o_.jingmai.entrySet())
			jingmai.put(_e_.getKey(), new Jingmai.Data(_e_.getValue()));
		return jingmai;
	}
	...
	@Override
	public java.util.List<Long> getJblb() { // 结拜列表，存储结拜角色的ID
		_xdb_verify_unsafe_();
		return mkdb.Logs.logList(new mkdb.LogKey(this, "jblb"), jblb);
	}

	public java.util.List<Long> getJblbAsData() { // 结拜列表，存储结拜角色的ID
		_xdb_verify_unsafe_();
		java.util.List<Long> jblb;
		Properties _o_ = this;
		jblb = new java.util.LinkedList<Long>();
		jblb.addAll(_o_.jblb);
		return jblb;
	...
	@Override
	public int getExitstate() { // 角色退出公会方式，0主动退出    1被踢出    2解散          key，作者 hzl
		_xdb_verify_unsafe_();
		return exitstate;
	}
	...
	@Override
	public int getEnergy() { // 活力，作者 changhao
		_xdb_verify_unsafe_();
		return energy;
	}
	...
	@Override
	public int getRolezonghemaxscore() { // 人物综合实力历史最高评分，作者 changhao
		_xdb_verify_unsafe_();
		return rolezonghemaxscore;
	}

	@Override
	public int getSkillscore() { // 技能评分，作者 changhao
		_xdb_verify_unsafe_();
		return skillscore;
	}

	@Override
	public int getPetscore() { // 最强单宠物评分，作者 changhao
		_xdb_verify_unsafe_();
		return petscore;
	}

	@Override
	public int getManypetscore() { // 多宠物评分，作者 changhao
		_xdb_verify_unsafe_();
		return manypetscore;
	}

	@Override
	public int getLevelscore() { // 等级评分，作者 changhao
		_xdb_verify_unsafe_();
		return levelscore;
	}

	@Override
	public int getXiulianscroe() { // 修炼评分，作者 changhao
		_xdb_verify_unsafe_();
		return xiulianscroe;
	}

	@Override
	public int getRolescore() { // 人物评分，作者 changhao
		_xdb_verify_unsafe_();
		return rolescore;
	}

	@Override
	public int getEquipscore() { // 装备评分，作者 changhao
		_xdb_verify_unsafe_();
		return equipscore;
	}

	@Override
	public long getHistorymaxprofessioncontribute() { // 历史最大职业贡献度，作者 changhao
		_xdb_verify_unsafe_();
		return historymaxprofessioncontribute;
	}

	@Override
	public int getRide() { // 当前坐骑，作者 changhao
		_xdb_verify_unsafe_();
		return ride;
	}
	...
	@Override
	public java.util.Map<Integer, Integer> getShizhuang() { // 时装衣橱，key=时装id value=时装id
		_xdb_verify_unsafe_();
		return mkdb.Logs.logMap(new mkdb.LogKey(this, "shizhuang"), shizhuang);
	}

	@Override
	public java.util.Map<Integer, Integer> getShizhuangAsData() { // 时装衣橱，key=时装id value=时装id
		_xdb_verify_unsafe_();
		java.util.Map<Integer, Integer> shizhuang;
		Properties _o_ = this;
		shizhuang = new java.util.HashMap<Integer, Integer>();
		for (java.util.Map.Entry<Integer, Integer> _e_ : _o_.shizhuang.entrySet())
			shizhuang.put(_e_.getKey(), _e_.getValue());
	...
	@Override
	public int getCruise2() { // 巡游2，作者 changhao
		_xdb_verify_unsafe_();
		return cruise2;
	}

	@Override
	public int getCruise3() { // 巡游3，作者 changhao
		_xdb_verify_unsafe_();
		return cruise3;
	}
	...
	@Override
	public long getExpiretime() { // 点卡结束时间(点卡服专用)，作者 changhao
		_xdb_verify_unsafe_();
		return expiretime;
	}

	@Override
	public int getFirstprompt() { // 还没提示过就是0(点卡服专用)，作者 changhao
		_xdb_verify_unsafe_();
		return firstprompt;
	}
	...
	@Override
	public java.util.Map<Integer, Integer> getZuoqi() { // 坐骑信息
		_xdb_verify_unsafe_();
		return mkdb.Logs.logMap(new mkdb.LogKey(this, "zuoqi"), zuoqi);
	}

	@Override
	public java.util.Map<Integer, Integer> getZuoqiAsData() { // 坐骑信息
		_xdb_verify_unsafe_();
		java.util.Map<Integer, Integer> zuoqi;
		Properties _o_ = this;
		zuoqi = new java.util.HashMap<Integer, Integer>();
		for (java.util.Map.Entry<Integer, Integer> _e_ : _o_.zuoqi.entrySet())
			zuoqi.put(_e_.getKey(), _e_.getValue());
	...
	@Override
	public java.util.List<String> getBenefitcode() { // 
		_xdb_verify_unsafe_();
		return mkdb.Logs.logList(new mkdb.LogKey(this, "benefitcode"), benefitcode);
	}

	public java.util.List<String> getBenefitcodeAsData() { // 
		_xdb_verify_unsafe_();
		java.util.List<String> benefitcode;
		Properties _o_ = this;
		benefitcode = new java.util.LinkedList<String>();
		benefitcode.addAll(_o_.benefitcode);
		return benefitcode;
	...
	@Override
	public java.util.Map<Integer, Integer> getQlgmap() { // 潜灵果
		_xdb_verify_unsafe_();
		return mkdb.Logs.logMap(new mkdb.LogKey(this, "qlgmap"), qlgmap);
	}

	@Override
	public java.util.Map<Integer, Integer> getQlgmapAsData() { // 潜灵果
		_xdb_verify_unsafe_();
		java.util.Map<Integer, Integer> qlgmap;
		Properties _o_ = this;
		qlgmap = new java.util.HashMap<Integer, Integer>();
		for (java.util.Map.Entry<Integer, Integer> _e_ : _o_.qlgmap.entrySet())
			qlgmap.put(_e_.getKey(), _e_.getValue());
	...
	@Override
	public int getQlgcurrextra() { // 潜灵果额外属性表当前使用的数量
		_xdb_verify_unsafe_();
		return qlgcurrextra;
	}

	@Override
	public java.util.Map<Integer, xbean.FunOpenClose> getQlgextrapromap() { // 潜灵果额外属性表 key 存储的开启潜灵果数量 value 存储的额外属性点
		_xdb_verify_unsafe_();
		return mkdb.Logs.logMap(new mkdb.LogKey(this, "qlgextrapromap"), qlgextrapromap);
	}

	@Override
	public java.util.Map<Integer, xbean.FunOpenClose> getQlgextrapromapAsData() { // 潜灵果额外属性表 key 存储的开启潜灵果数量 value 存储的额外属性点
		_xdb_verify_unsafe_();
		java.util.Map<Integer, xbean.FunOpenClose> qlgextrapromap;
		Properties _o_ = this;
		qlgextrapromap = new java.util.HashMap<Integer, xbean.FunOpenClose>();
		for (java.util.Map.Entry<Integer, xbean.FunOpenClose> _e_ : _o_.qlgextrapromap.entrySet())
			qlgextrapromap.put(_e_.getKey(), new FunOpenClose.Data(_e_.getValue()));
		return qlgextrapromap;
	}
	...
	@Override
	public void setExitstate(int _v_) { // 角色退出公会方式，0主动退出    1被踢出    2解散          key，作者 hzl
		_xdb_verify_unsafe_();
	...
	@Override
	public void setEnergy(int _v_) { // 活力，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setRolezonghemaxscore(int _v_) { // 人物综合实力历史最高评分，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setSkillscore(int _v_) { // 技能评分，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setPetscore(int _v_) { // 最强单宠物评分，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setManypetscore(int _v_) { // 多宠物评分，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setLevelscore(int _v_) { // 等级评分，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setXiulianscroe(int _v_) { // 修炼评分，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setRolescore(int _v_) { // 人物评分，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setEquipscore(int _v_) { // 装备评分，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setHistorymaxprofessioncontribute(long _v_) { // 历史最大职业贡献度，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setRide(int _v_) { // 当前坐骑，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setCruise2(int _v_) { // 巡游2，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setCruise3(int _v_) { // 巡游3，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setExpiretime(long _v_) { // 点卡结束时间(点卡服专用)，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setFirstprompt(int _v_) { // 还没提示过就是0(点卡服专用)，作者 changhao
		_xdb_verify_unsafe_();
	...
	@Override
	public void setZhuansheng(int _v_) { // 
		_xdb_verify_unsafe_();
		mkdb.Logs.logIf(new mkdb.LogKey(this, "zhuansheng") {
			protected mkdb.Log create() {
				return new mkdb.logs.LogInt(this, zhuansheng) {
					public void rollback() { zhuansheng = _xdb_saved; }
				};}});
		zhuansheng = _v_;
	}

	@Override
	public void setBsitem(int _v_) { // 
		_xdb_verify_unsafe_();
		mkdb.Logs.logIf(new mkdb.LogKey(this, "bsitem") {
			protected mkdb.Log create() {
				return new mkdb.logs.LogInt(this, bsitem) {
					public void rollback() { bsitem = _xdb_saved; }
				};}});
		bsitem = _v_;
	}

	@Override
	public void setZqitem(int _v_) { // 
		_xdb_verify_unsafe_();
		mkdb.Logs.logIf(new mkdb.LogKey(this, "zqitem") {
			protected mkdb.Log create() {
				return new mkdb.logs.LogInt(this, zqitem) {
					public void rollback() { zqitem = _xdb_saved; }
				};}});
		zqitem = _v_;
	}

	@Override
	public void setPartnerid(long _v_) { // 
		_xdb_verify_unsafe_();
		mkdb.Logs.logIf(new mkdb.LogKey(this, "partnerid") {
			protected mkdb.Log create() {
				return new mkdb.logs.LogLong(this, partnerid) {
					public void rollback() { partnerid = _xdb_saved; }
				};}});
		partnerid = _v_;
	}

	@Override
	public void setQlgcurrextra(int _v_) { // 潜灵果额外属性表当前使用的数量
		_xdb_verify_unsafe_();
		mkdb.Logs.logIf(new mkdb.LogKey(this, "qlgcurrextra") {
			protected mkdb.Log create() {
				return new mkdb.logs.LogInt(this, qlgcurrextra) {
					public void rollback() { qlgcurrextra = _xdb_saved; }
				};}});
		qlgcurrextra = _v_;
	}
	...
	public final boolean equals(Object _o1_) {
		...
		if (!jingmai.equals(_o_.jingmai)) return false;
		...
		if (!jblb.equals(_o_.jblb)) return false;
		...
		if (!shizhuang.equals(_o_.shizhuang)) return false;
		...
		if (zhuansheng != _o_.zhuansheng) return false;
		if (bsitem != _o_.bsitem) return false;
		if (zqitem != _o_.zqitem) return false;
		if (!zuoqi.equals(_o_.zuoqi)) return false;
		if (!benefitcode.equals(_o_.benefitcode)) return false;
		if (partnerid != _o_.partnerid) return false;
		if (!qlgmap.equals(_o_.qlgmap)) return false;
		if (qlgcurrextra != _o_.qlgcurrextra) return false;
		if (!qlgextrapromap.equals(_o_.qlgextrapromap)) return false;
		return true;
	}
	...
	public final int hashCode() {
		...
		_h_ += zqitem;
		_h_ += zuoqi.hashCode();
		_h_ += benefitcode.hashCode();
		_h_ += partnerid;
		_h_ += qlgmap.hashCode();
		_h_ += qlgcurrextra;
		_h_ += qlgextrapromap.hashCode();
		return _h_;
	}
	...
	public String toString() {
		...
		_sb_.append(zuoqi);
		_sb_.append(",");
		_sb_.append(benefitcode);
		_sb_.append(",");
		_sb_.append(partnerid);
		_sb_.append(",");
		_sb_.append(qlgmap);
		_sb_.append(",");
		_sb_.append(qlgcurrextra);
		_sb_.append(",");
		_sb_.append(qlgextrapromap);
		_sb_.append(")");
	...
	public mkdb.logs.Listenable newListenable() {
		...
		lb.add(new mkdb.logs.ListenableMap().setVarName("jingmai"));
		...
		lb.add(new mkdb.logs.ListenableChanged().setVarName("jblb"));
		...
		lb.add(new mkdb.logs.ListenableMap().setVarName("shizhuang"));
		...
		lb.add(new mkdb.logs.ListenableChanged().setVarName("zhuansheng"));
		lb.add(new mkdb.logs.ListenableChanged().setVarName("bsitem"));
		lb.add(new mkdb.logs.ListenableChanged().setVarName("zqitem"));
		lb.add(new mkdb.logs.ListenableMap().setVarName("zuoqi"));
		lb.add(new mkdb.logs.ListenableChanged().setVarName("benefitcode"));
		lb.add(new mkdb.logs.ListenableChanged().setVarName("partnerid"));
		lb.add(new mkdb.logs.ListenableMap().setVarName("qlgmap"));
		lb.add(new mkdb.logs.ListenableChanged().setVarName("qlgcurrextra"));
		lb.add(new mkdb.logs.ListenableMap().setVarName("qlgextrapromap"));
		return lb;
	}
	...
	private class Const implements xbean.Properties {
		...
		public java.util.Map<Integer, xbean.Jingmai> getJingmai() { // 拥有的称谓列表
			_xdb_verify_unsafe_();
			return mkdb.Consts.constMap(jingmai);
		}
		...
		public java.util.List<Long> getJblb() { // 结拜列表，存储结拜角色的ID
			_xdb_verify_unsafe_();
			return mkdb.Consts.constList(jblb);
		}
		...
		public java.util.Map<Integer, Integer> getShizhuang() { // 时装衣橱，key=时装id value=时装id
			_xdb_verify_unsafe_();
			return mkdb.Consts.constMap(shizhuang);
		}
		...
		public java.util.Map<Integer, Integer> getZuoqi() { // 坐骑信息
			_xdb_verify_unsafe_();
			return mkdb.Consts.constMap(zuoqi);
		}
		...
		public java.util.List<String> getBenefitcode() { // 
			_xdb_verify_unsafe_();
			return mkdb.Consts.constList(benefitcode);
		}
		...
		public java.util.Map<Integer, Integer> getQlgmap() { // 潜灵果
			_xdb_verify_unsafe_();
			return mkdb.Consts.constMap(qlgmap);
		}
		...
		public java.util.Map<Integer, xbean.FunOpenClose> getQlgextrapromap() { // 潜灵果额外属性表 key 存储的开启潜灵果数量 value 存储的额外属性点
			_xdb_verify_unsafe_();
			return mkdb.Consts.constMap(qlgextrapromap);
		}
		...
		public final OctetsStream unmarshal(OctetsStream _os_) throws MarshalException {
			...
			}
			{
				...
				for (; size > 0; --size)
				{
					int _k_ = 0;
					_k_ = _os_.unmarshal_int();
					xbean.Jingmai _v_ = xbean.Pod.newJingmaiData();
					_v_.unmarshal(_os_);
					jingmai.put(_k_, _v_);
				}
			}
			...
			qlgcurrextra = _os_.unmarshal_int();
			{
				...
				for (; size > 0; --size)
				{
					int _k_ = 0;
					_k_ = _os_.unmarshal_int();
					xbean.FunOpenClose _v_ = xbean.Pod.newFunOpenCloseData();
					_v_.unmarshal(_os_);
					qlgextrapromap.put(_k_, _v_);
				}
			}
