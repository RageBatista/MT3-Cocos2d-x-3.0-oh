...
public class HuoBanInfo implements Marshal, Comparable<HuoBanInfo> {
    ...
    public boolean equals(Object _o1_) {
        if (_o1_ == this) {
            return true;
        } else if (_o1_ instanceof HuoBanInfo) {
    ...
    public int compareTo(HuoBanInfo _o_) {
        if (_o_ == this) {
            ...
            int _c_ = 0;
            _c_ = this.huobanid - _o_.huobanid;
            if (0 != _c_) {
                return _c_;
            } else {
                _c_ = this.infight - _o_.infight;
                if (0 != _c_) {
                    return _c_;
                } else {
                    _c_ = this.weekfree - _o_.weekfree;
                    if (0 != _c_) {
                        return _c_;
                    } else {
                        _c_ = Long.signum(this.state - _o_.state);
