...
import fire.pb.PetBean; // 添加这行导入语句
...
public class Pet {
	...
	public static Pet getPet(long roleId, PetColumn petColumn, int petKey,
		...
		try {
			...
			ExceptionHandler.handleException(e, "Pet.getPet1");
			return null;
		}
	}
	...
	public static Pet getPet(long roleId, int petColumnType, int petKey,
		...
		try {
			...
			ExceptionHandler.handleException(e, "Pet.getPet2");
			return null;
		}
	}
	...
	public fire.pb.PetBean getProtocolPet() { // 修改返回类型为 fire.pb.PetBean
		final fire.pb.PetBean pet = new fire.pb.PetBean(); // 修改变量类型为 fire.pb.PetBean
		pet.id = getBaseId();
		pet.ownername = "";
		...
		if (isLocked() != -1)
			pet.flag = (byte) (pet.flag | fire.pb.PetBean.FLAG_LOCK); // 修改这里
		if (isBind())
			pet.flag = (byte) (pet.flag | fire.pb.PetBean.FLAG_BIND); // 修改这里
	...
	public xbean.PetEquipItem addPetEquipHuFu(fire.pb.item.PetEquipItem equipItem) {
		if (equipItem == null) {
			return null;
		}

		List<xbean.PetEquipItem> equips = petInfo.getPetequipbag();
		if (equips == null) {
			return null;
		}

		int pos = equipItem.getEquipAttr().getPos();
		xbean.PetEquipItem newEquip = xbean.Pod.newPetEquipItem();
		newEquip.setItemid(equipItem.getItemId());
		newEquip.setPos(pos);
		newEquip.setTaozhuangid(equipItem.getEquipAttr().getTaozhuangid());
		newEquip.getPro().putAll(equipItem.getEquipAttr().getPro());
		newEquip.getSkill().putAll(equipItem.getEquipAttr().getSkill());

		for (int i = 0; i < equips.size(); i++) {
			xbean.PetEquipItem oldEquip = equips.get(i);
			if (oldEquip != null && oldEquip.getPos() == pos) {
				equips.set(i, newEquip);
				return oldEquip;
			}
		}
	...
	public List<xbean.PetSkill> getInternals() {
		return petInfo.getInternals();
	}

	public List<Integer> getBattleInternalIds() {
		List<Integer> skillIds = new LinkedList<Integer>();
		for (xbean.PetSkill skill : getInternals())
			skillIds.add(skill.getSkillid());
		return skillIds;
	}

	public boolean hasInternal(int skillId) {
		List<xbean.PetSkill> skills = getInternals();
		for (xbean.PetSkill skill : skills) {
			if (skill.getSkillid() == skillId) {
				return true;
			}
	...
	public List<xbean.PetSkill> getBattleInternals() {
		return petInfo.getInternals();
	}
	...
	public boolean addInternal(int skillId, long expireTime, int skillExp, int skillType) {
		List<xbean.PetSkill> skills = petInfo.getInternals();
		if (skills.size() >= getInternalMaxNum()) {
			return false;
		}
		xbean.PetSkill newSkill = xbean.Pod.newPetSkill();
		newSkill.setSkillid(skillId);
		newSkill.setSkilltype(skillType);
		newSkill.setCertification(0);
		skills.add(newSkill);
		return true;
	}
	...
	public int insertInternal(int index, int skillId, long expireTime, int skillType) {
		...
		xbean.PetSkill oldSkill = skills.get(index);
		int oldSkillId = oldSkill.getSkillid();
		xbean.PetSkill newSkill = xbean.Pod.newPetSkill();
		newSkill.setSkillid(skillId);
		newSkill.setSkilltype(skillType);
		newSkill.setCertification(0);
	...
	public int LevelUpInternal(int skillId, int newSkillId) {
		List<xbean.PetSkill> skills = petInfo.getInternals();
		for (xbean.PetSkill skill : skills) {
			if (skill.getSkillid() == skillId) {
	...
	public void fillSRefreshPetInternal(SRefreshPetInternal send) {
		send.internals = new LinkedList<Petskill>();
		for (xbean.PetSkill skill : petInfo.getInternals()) {
			Petskill petSkill = new Petskill();
			petSkill.skillid = skill.getSkillid();
			petSkill.skillexp = 0;
			petSkill.skilltype = (byte) skill.getSkilltype();
		...
		send.expiredtimes = new HashMap<Integer, Long>();
	}
}
