...
public class PExchangeCodeAuReturn extends Procedure {
	...
	protected boolean process() throws Exception {
		...
		JsonNode jobj = JsonUtil.parseJson(result);
		...
		int count = jobj.get("Count").asInt();
		ArrayNode awards = (ArrayNode) jobj.get("Awards");
		...
		for (int i = 0; i < awards.size(); i++) {
			jobj = awards.get(i);
			String[] award = jobj.get("Award").asText().split("\\|");
			if (award.length < 4) {
				continue;
			}
