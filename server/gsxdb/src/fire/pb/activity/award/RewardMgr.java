...
public class RewardMgr {
	...
	// 奖励二级表的原始数据
	private java.util.NavigableMap<Integer, SActivityAwardItems> award2Map = new java.util.TreeMap<Integer, SActivityAwardItems>();
	...
	// 特定时间的额外奖励
	private java.util.Map<Integer, ExAward> externalAwardMap = new HashMap<Integer, ExAward>();

	// 给予的物品是否绑定
	private java.util.Map<Integer, Boolean> bindMap = new HashMap<Integer, Boolean>();

	// 如果背包栏满了是否放入临时背包
	private java.util.Map<Integer, Boolean> tempBagAvailableMap = new HashMap<Integer, Boolean>();

	// 给予的物品是否需要发消息
	private java.util.Map<Integer, MsgInfos> msgInfosMap = new HashMap<Integer, MsgInfos>();
...
//					MessageMgr.psendMsgNotifyWhileCommit(roleId, 180034, para);
	...
	void distributeCreditValueAward(long roleId, int awardID, Map<String, Object> parameters,
		...
		if (creditBili.containsKey(awardID)) {
			...
			// MoneyType.MoneyType_EreditPoint, "奖励ID:" + 奖励ID,
			// getType, 0);
			// }
		}
		// 信用值额外
