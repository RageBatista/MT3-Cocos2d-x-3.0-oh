...
{
	public static int ITEM_ID_LIANSHENG_BOX = 331403; //连胜宝箱，作者 changhao
	public static int ONE_PEOPLE_SCROE = 60; //赢一个人积分，作者 changhao
	public static int ATTACK_SUB_ACT = 12; //单人扣除行动力，作者 changhao
	...
	public final static int WIN_NUM = 5; //连胜数量，作者 changhao
	public final static int REFRESH_BOX_TIME = 60; //多少后开始刷宝箱，作者 changhao
	public final static int BOX_TIME = 60 * 10;//宝箱存留多少秒，作者 changhao
	...
	public java.util.concurrent.atomic.AtomicInteger curweek = new java.util.concurrent.atomic.AtomicInteger(-1);//当前轮数 从0开始，作者 changhao
	...
	/***
	 * 取公会综合实力排名，作者 changhao
	 * @参数tmp
	 * @参数排序图
	 */
	void GetClanZongheList(java.util.ArrayList<xbean.ClanInfo> tmp, java.util.Map<Long, Integer> sortmap)
	{
		xbean.ClanZongHeRankList list = xtable.Clanzonghelist.select(1); //取出综合实力榜，作者 changhao
		if (list != null)
		{
	...
	java.util.ArrayList<xbean.ClanInfo> CalcQualificationClan(int which)
	{
		...
		if (curweek == 0 && which == 0) //第一轮的第一场看公会综合实力匹配，作者 changhao
		{
		...
		else
		{
			...
			xbean.ClanFightRaceRankList ranlListBean = xtable.Clanfightracelist.select(datetime); //得到这周的轮排名，作者 changhao
			if (ranlListBean == null)
			{
				ranlListBean = xtable.Clanfightracelist.select(datetime - 7 * day1); //得到上周的轮排名，作者 changhao				
			}
			...
			{
				GetClanZongheList(tmp, sortmap);// 没有轮排行数据就按公会综合实力榜匹配，作者 changhao	
			}
		}
		...
		for (xbean.ClanInfo e : tmp)
		{
			...
			if (claninfo.getClanlevel() < ActivityClanFightManager.CLAN_FIGHT_CLAN_LEVEL) //公会等级>=2，作者 changhao
			{
				continue;
			}
			
			if (claninfo.getMembers().size() < ActivityClanFightManager.CLAN_FIGHT_PEOPLE_NUM) //会员至少20人，作者 changhao
			{
				continue;
			}
			...
			for (Long roleid : claninfo.getMembers().keySet())
			{
				...
				if (prop != null)
				{
					...
					{						
						if (ActivityClanFightManager.CLAN_FIGHT_LOGIN_TIME == 0 || cur - day1 * ActivityClanFightManager.CLAN_FIGHT_LOGIN_TIME < prop.getOnlinetime()) //7天内登陆过，作者 changhao
						{
							count ++;
		...
		int minscroe = 6000000; //找出最弱公会，作者 changhao
		...
		if (l.size() % 2 == 1) //移除奇数个，作者 changhao
		{
			if (minscroeclan != null) //移除最弱的，作者 changhao
			{
	...
	void CreateClanFightMatch(int which, java.util.ArrayList<xbean.ClanInfo> list, boolean force)
	{
		...
		if (clanfights == null || clanfights.getCreatetime() != t || force == true) //需要重新生成，作者 changhao
		{
			if (clanfights == null)
			{
			...
			for (int i = 0; i < size / 2; i ++)
			{
				...
				if (c1 != null && c2 != null)
				{
					...
					long onlyid = t + i; //生成公会战唯一id，作者 changhao
					...
					else
					{
						...
						for (Long o : ids1.getIds()) //遍历清除老的副本id，作者 changhao
						{
					...
					{
						java.util.List<Long> removelist = new java.util.ArrayList<Long>();
						for (Long o : ids2.getIds()) //遍历清除老的副本id，作者 changhao
						{
							if (o < GetMonday1500ByTime(cur))
							{
								removelist.add(o);
	...
	{
		//计算出当前的轮数，作者 changhao
		curweek.set(CalcCurWeekNum());
		
		long cur = System.currentTimeMillis();
		
		java.util.ArrayList<xbean.ClanInfo> list1 = CalcQualificationClan(0); //计算有资格的公会，作者 changhao
		java.util.ArrayList<xbean.ClanInfo> list2 = CalcQualificationClan(1); //计算有资格的公会，作者 changhao
		...
		mkdb.Lockeys.lock(mkdb.Lockeys.get(xtable.Locks.CLANS, setclanid)); //锁定公会id，作者 changhao
		...
		{
			CreateClanFightMatch(0, list1, force); //创建2组比赛，作者 changhao			
		}
				...
				if (clanfights != null)
				{
					for (xbean.ClanFight e : clanfights.getClan1vschan2().values())
					{
						if (e != null)
						{
							...
							if (e.getWinner() != -1) //这场工会战已经结束，作者 changhao
							{
								info1.fightcount ++;
	...
	/***
	 * 检查这个玩家是否可以公会战不考虑组队，作者 changhao
	 * lock clan -> clanfightid，作者 changhao
	 * @参数角色ID
	 * @返回
	 */
	public static boolean checkClanFight(long roleid, int level)
	{
		...
		long day1 = 24 * 60 * 60 * 1000;
		if (cur < mi.getJointime() + day1 * CLAN_FIGHT_JOIN_CLAN_TIME) //有人加入公会时间不足2天，作者 changhao
		{			
			return false;
		}		
		...
		mkdb.Lockeys.lock(mkdb.Lockeys.get(xtable.Locks.CLANFIGHTIDLOCK, ids.getIds())); //锁定公会战id，作者 changhao
