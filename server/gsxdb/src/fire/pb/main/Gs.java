...
public class Gs {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			ExceptionHandler.handleException(e, "fire.pb.main.Gs.shutdown - 服务器关闭");
		} finally {
			System.exit(1);
		}
	}
	...
	private static void onShutdown() throws Exception {
		...
		for (Role r : roles) {
			try {
				...
				ExceptionHandler.handleException(e, "fire.pb.main.Gs.onShutdown - 踢玩家下线");
			}
	...
	private static void stopJMXServer() {
		try {
			...
			ExceptionHandler.handleException(ex, "fire.pb.main.Gs.stopJMXServer");
			return;
		}
			...
			public void run() {
				...
				while (true) {
					...
					try {
						...
						ExceptionHandler.handleSilently(e, "fire.pb.main.Gs.printMemoryLogThread");
			...
			public void run() {
				...
				while (true) {
					try {
						...
						ExceptionHandler.handleSilently(e, "fire.pb.main.Gs.roleNumLogThread");
						Thread.currentThread().interrupt();
					}
				}
			}
