...
public class CGetEquipTips extends __CGetEquipTips__ {
	...
	protected void process() {
		...
		if (roleId < 0) {
			Module.logger.warn("[CGetEquipTips] roleId < 0, 无效角色");
			return;
		}

		Module.logger.info("[CGetEquipTips] 收到装备Tips请求: roleId=" + roleId
			+ ", packid=" + packid + ", keyinpack=" + keyinpack);

		ItemMaps column = Module.getInstance().getItemMaps(roleId, packid, true);
		ItemBase bi = column.getItem(keyinpack);
		if (null == bi) {
			Module.logger.warn("[CGetEquipTips] 物品不存在: roleId=" + roleId
				+ ", packid=" + packid + ", keyinpack=" + keyinpack);
			return;
		}

		if (!(bi instanceof EquipItem)) {
			Module.logger.warn("[CGetEquipTips] 物品不是装备类型: roleId=" + roleId
				+ ", itemId=" + bi.getItemId() + ", 类型=" + bi.getClass().getSimpleName());
			return;
		}

		EquipItem equipItem = (EquipItem) bi;
		Module.logger.info("[CGetEquipTips] 获取装备Tips: itemId=" + equipItem.getItemId()
			+ ", itemName=" + equipItem.getName() + ", equipPos=" + equipItem.getEquipPos());

		com.locojoy.base.Octets tips = equipItem.getTips();
		Module.logger.info("[CGetEquipTips] Tips数据大小: " + (tips != null ? tips.size() : "null") + " 字节");

		SGetEquipTips equipTips = new SGetEquipTips(packid, keyinpack,
				key2inpack, tips);

		gnet.link.Onlines.getInstance().send(roleId, equipTips);
		Module.logger.info("[CGetEquipTips] 已发送SGetEquipTips到客户端");
	}
