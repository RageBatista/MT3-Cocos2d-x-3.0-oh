...
import xbean.DiscardItem;
import xbean.Item;
import xbean.ItemRecycleDate;
import xbean.ItemRecycleIndex;
import xbean.Itemrecoverlist;
import xbean.Pod;
import xtable.Itemrecover;
import xtable.Itemrecyclebin;
import xtable.Itemrecycleidx;
 ...
 * <p>本类提供了物品系统的核心功能，包括：
 * <ul>
 *   <li>物品基本属性管理（ID、类型、数量、标志位等）</li>
 *   <li>物品绑定机制（绑定后不可交易）</li>
 *   <li>物品使用逻辑（支持对角色、宠物、其他物品使用）</li>
 *   <li>物品回收和恢复（删除后可从回收站恢复）</li>
 *   <li>物品超时管理（有效期物品的自动失效）</li>
 *   <li>物品交易控制（摆摊、交易权限检查）</li>
 * </ul>
 *
 * <h2>继承体系</h2>
 * <前>
 * ItemBase (抽象基类)
 *   ├─ EquipItem (装备类物品 - 武器、防具、饰品)
 *   ├─ GroceryItem (杂货类物品 - 消耗品、材料)
 *   ├─ PetItem (宠物类物品 - 召唤兽)
 *   ├─ TaskItem (任务类物品 - 任务道具)
 *   └─ ... (其他物品类型)
 * </前>
 ...
 * <h2>核心字段说明</h2>
 * <ul>
 *   <li>{@link #itemAttr} - 物品静态属性（来自配置表 ItemShuXing）</li>
 *   <li>{@link #itemData} - 物品动态数据（存储在数据库 xbean.Item）</li>
 *   <li>{@link #packid} - 所属背包ID（0=主背包，其他值为仓库等）</li>
 *   <li>{@link #keyinpack} - 在背包中的位置（格子编号）</li>
 *   <li>{@link #roleid} - 物品所有者角色ID</li>
 * </ul>
 *
 * <h2>物品标志位（Flags）</h2>
 * <ul>
 *   <li>{@link fire.pb.Item#BIND} - 绑定标志（不可交易）</li>
 *   <li>{@link fire.pb.Item#ONSTALL} - 摆摊中（锁定状态）</li>
 *   <li>{@link fire.pb.Item#TIMEOUT} - 有超时设置</li>
 *   <li>{@link fire.pb.Item#CANNOTONSTALL} - 禁止摆摊</li>
 * </ul>
 *
 * <h2>使用示例</h2>
 * <前>{@代码
 * // 使用物品
 * ItemBase item = pack.getItem(keyinpack);
 * UseResult result = item.onUse(1); // 使用1个
 *
 * // 绑定物品
 * item.bindAndNotify(); // 绑定并通知客户端
 *
 * // 检查物品状态
 * if (item.isTimeout()) {
 *     // 物品已过期
 ...
 * }</pre>
 *
 * @see EquipItem 装备物品实现
 * @see GroceryItem 杂货物品实现
 * @see Pack 背包管理类
 * @see ItemMgr 物品管理器接口
 * @author 游戏服务器团队
 * @版本2.0
...
public abstract class ItemBase {
	...
	public boolean canTrans() {
		return ((itemData.getFlags() & (fire.pb.Item.BIND | fire.pb.Item.ONSTALL | fire.pb.Item.CANNOTONSTALL)) == 0);
	}
