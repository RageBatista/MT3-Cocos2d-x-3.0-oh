...
public class Module implements ModuleInterface {
	...
	private int getBasicScore(EquipItem item) {
		...
		Iterator<Entry<Integer, Integer>> extraAttr = item.getEquipAttr()
				.getExtraattr().entrySet().iterator();
		while (extraAttr.hasNext()) {
			Entry<Integer, Integer> current = extraAttr.next();
			int id = current.getKey() - current.getKey() % 10;
			switch (id) {
			case Commontext.CONS:
			case Commontext.IQ:
		...
		Iterator<Entry<Integer, Integer>> extraAddAttr = item.getEquipAttr()
				.getExtraaddattr().entrySet().iterator();
		while (extraAddAttr.hasNext()) {
			Entry<Integer, Integer> current = extraAddAttr.next();
			int id = current.getKey() - current.getKey() % 10;
			switch (id) {
			case Commontext.CONS:
			case Commontext.IQ:
