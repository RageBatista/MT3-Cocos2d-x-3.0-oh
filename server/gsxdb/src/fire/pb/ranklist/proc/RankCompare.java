...
public class RankCompare {
	public static boolean CompareTo(mkdb.Bean current, mkdb.Bean other)
	{
		...
		if (current instanceof xbean.ClanRankRecord) { // 公会等级比较，作者 changhao
		...
		if (current instanceof xbean.RoleRankRecord) { // 人物评分比较，作者 changhao
		...
		if (current instanceof xbean.RedPackRecord) { //红包排行榜，作者 changhao
			return compareToRedPackRankRecord(current, other);
		}
		
		if (current instanceof xbean.GiveFlowerRecord) { //送花排行榜，作者 changhao
			return compareToGiveFlowerRankRecord(current, other);
		}

		if (current instanceof xbean.ReceFlowerRecord) { //收花排行榜，作者 changhao
