...
public class PRoleZongheRankProc extends Procedure {
	...
	public boolean process(){
		...
		record.setShape(prole.getProperties().getShape()); // 模型数据
		record.setColor1(prole.getProperties().getRolecolor1());
		record.setColor2(prole.getProperties().getRolecolor2());
		fire.pb.map.Role.fillPlayerComponentsInt(roleId, record.getComponents()); // 武器头饰坐骑等
		...
		professionrecord.setShape(prole.getProperties().getShape()); // 职业榜模型数据
		professionrecord.setColor1(prole.getProperties().getRolecolor1());
		professionrecord.setColor2(prole.getProperties().getRolecolor2());
		fire.pb.map.Role.fillPlayerComponentsInt(roleId, professionrecord.getComponents()); // 武器头饰坐骑等
		...
		rolerecord.setShape(prole.getProperties().getShape()); // 人物榜模型数据
		rolerecord.setColor1(prole.getProperties().getRolecolor1());
		rolerecord.setColor2(prole.getProperties().getRolecolor2());
		fire.pb.map.Role.fillPlayerComponentsInt(roleId, rolerecord.getComponents()); // 武器头饰坐骑等
		...
		//if(prole.getLevel() < 30) return true; 不要等级限制，作者 changhao
		...
		if (clanInfo != null) //如果玩家有公会更新公会的综合实力，作者 changhao
		{
	...
	public int ProfessionToRankType(int school)
	{
		...
		else if (school == 20) // 女儿村
		{
			return RankType.PROFESSION_NUERCUN_RANK;
		}
		else if (school == 21) // 小雷音
		{
			return RankType.PROFESSION_XIAOLEIYIN_RANK;
		}
		else if (school == 22) // 花果山
		{
			return RankType.PROFESSION_HUAGUOSHAN_RANK;
		}
		else if (school == 23) // 须弥海
		{
			return RankType.PROFESSION_XUMIHAI_RANK;
		}
		else if (school == 24) // 盘丝洞
		{
			return RankType.PROFESSION_PANSI_RANK;
		}
