	...
	public List<Octets> getMO(SRequestRankList response, int page) {
		...
		for(int i=0; i<records.size(); i++){
			...
			if(i >= start && i < end) {
				...
				long uniquePetId = marshalRecord.getUniquepetid();
				if (uniquePetId > 0L) {
					...
					if (pet != null) {
						...
						if (petInfo != null) {
							...
							marshalRecord.setColour(color);
							marshalRecord.setShape(petInfo.getId());
							marshalRecord.setPetname(petInfo.getName());
							String nickName = xtable.Properties.selectRolename(petInfo.getOwnerid());
							if (nickName == null) {
								nickName = "";
							}
							marshalRecord.setNickname(nickName);
						}
					}
				}
