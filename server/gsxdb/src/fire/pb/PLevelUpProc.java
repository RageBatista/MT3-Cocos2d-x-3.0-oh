		...
		if(level >= 30){
			mkdb.Procedure.pexecuteWhileCommit(new mkdb.Procedure(){
				...
				public boolean process(){
					...
					// 填充模型相关数据，用于排行榜显示角色模型
					xbean.Properties prop = pRole.getProperties();
					marshRecord.setShape(prop.getShape());
					marshRecord.setColor1(prop.getRolecolor1());
					marshRecord.setColor2(prop.getRolecolor2());
					// 填充换装组件（武器、头饰、坐骑等）
					fire.pb.map.Role.fillPlayerComponentsInt(roleId, marshRecord.getComponents());
