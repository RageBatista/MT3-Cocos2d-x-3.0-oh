...
public class PAfterEnterWorld extends Procedure {
	...
	protected boolean process() {
		...
		try {				
			if (fire.pb.fushi.Module.GetPayServiceType() == 1) //点卡服务器，作者 changhao
			{
...
//					mkdb.Procedure.psendWhileCommit(roleId, msg);	
			...
			// 登录时同步 MySQL 中的货币、VIP、日卡与封禁信息
			MysqlSyncManager.syncFushinum(userId);
			MysqlSyncManager.syncBitcoinnum(userId);
			MysqlSyncManager.syncVipinfo(roleId);
			MysqlSyncManager.syncRoledaypay(userId);
			MysqlSyncManager.syncUserpunish(userId);
		}
		catch(Exception e){
		...
		if (fire.pb.fushi.Module.GetPayServiceType() == 0) //普通服务器，作者 changhao
		{
