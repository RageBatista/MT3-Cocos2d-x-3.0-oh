...
public class SkillMonster extends SkillAgent
{
	...
	private final long roleid; // 老大的ID 当这个怪物是伙伴的时候,能用到，作者 changhao
	...
	public int getLevel(int skillId)
	{
		...
		if (roleid != 0)
		{
			...
			if (skillType == SkillConstant.TYPE_ROLE_PRACTICE_SKILL) //修炼技能等级，作者 changhao
			{
	...
	public List<Integer> getAllBattleSkills()
	{
		...
		//如果是伙伴，作者 changhao
		if (monsterbean.getFightnpctype() == xbean.Fighter.FIGHTER_PARTNER && roleid != 0) //队长的修炼技能 应用到伙伴身上，作者 changhao
		{
			//增加修炼技能对应的被动技能，作者 changhao
			SkillRole skillrole = new SkillRole(roleid, true);
