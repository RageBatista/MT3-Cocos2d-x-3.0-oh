	...
	protected void process() {
		...
		mkdb.Procedure requestlearnliveskill = new mkdb.Procedure()
		{
			...
			protected boolean process()
			{
				...
				int requireLevel = 1; //需要学习到的等级，作者 changhao
				...
				if (requireLevel > config.skillLevelMax) //是否已经达到最大等级，作者 changhao
				{
				...
				if (config.studyLevelRule > 0)
				{
					...
					if (roleidlevel < needLevelLimit) //等级不够，作者 changhao
					{
				...
				if (config.skillId != 0) //是被动技能，作者 changhao
				{
