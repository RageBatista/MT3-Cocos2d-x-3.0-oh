...
public class CLiveSkillMakeDrug extends __CLiveSkillMakeDrug__ {
	...
	protected void process() {
		...
		mkdb.Procedure liveskillmakedrug = new mkdb.Procedure()
		{
			...
			protected boolean process()
			{
				...
				xbean.LiveSkill liveskill = skillrole.getLiveSkills().get(LiveSkillManager.LIVE_SKILL_TYPE_MAKE_DRUG);	//得到生活技能等级，作者 changhao			
				if (liveskill == null)
				{
				...
				if (energyenough == false) //消耗活力，作者 changhao
				{
				...
				if (makingslist.size() == 0) //使用银币炼药，作者 changhao
				{
				...
				else if (makingslist.size() < 2) //一个材料不能炼药，作者 changhao
				{
					...
					for (Integer makings : makingslist)
					{
						...
						if (stuff != null)
						{
							...
							if (classid != null)
							{
								...
								if (drugstufftype == true) //如果是炼药材料，作者 changhao
								{
				...
				if (count < 2) //一个材料不能炼药，作者 changhao
				{
					psend(roleid, new SSkillError(SkillConstant.SkillError.OneStuffCannot));
					return true;					
				}
				...
				if (index <= prob) //制作开始，作者 changhao
				{
					...
					int drug = LiveSkillManager.ITEM_TYPE_DRUG_JIN_CHUANG; //最终产出啥药，作者 changhao
					if (randomValue < jinchuangweight) //是否产生金疮药，作者 changhao
					{
	...
	public java.util.LinkedList<Integer> makingslist; // 材料链，作者 changhao
