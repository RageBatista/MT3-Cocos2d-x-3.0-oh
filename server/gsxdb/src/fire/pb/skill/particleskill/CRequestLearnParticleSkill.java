...
public class CRequestLearnParticleSkill extends __CRequestLearnParticleSkill__ {
	...
	protected void process() {
		...
		mkdb.Procedure requestlearnparticleskill = new mkdb.Procedure()
		{
			...
			protected boolean process()
			{
				...
				int curskilllevel = practiceskill.getLevel(); //需要学习到的等级，作者 changhao
				
				if (curskilllevel >= config.skillLevelMax) //是否已经达到最大等级，作者 changhao
				{
				...
				if (config.needGuild == 1 && roleproperty.getClankey() <= 0) //是否需要公会，作者 changhao
				{
					return true;	
				}
				...
				int which = LiveSkillManager.getInstance().GetPracticleSkillPlayerOrPet(id); //得到这个修炼技能对应的对象，作者 changhao
			
				if (itemid != 0) //消耗道具升级，作者 changhao
				{
				...
				AddExp(curskilllevel, practiceskill.getExp() + addexp, practiceskill, maxlevel); //升级，作者 changhao
				...
				if (msg.skill.level > 0)
				{
					...
					if (practiceskill.getLevel() > curskilllevel) //升级刷新属性，作者 changhao
					{
						...
						if(!result.getChangedAttrs().isEmpty())//通知属性改动，作者 changhao
						{
	...
	{
		int maxexp = LiveSkillManager.getInstance().GetPracticleSkillNeedExp(id, curskilllevel); //得到升这级需要的经验，作者 changhao
		int exp = curexp;
		while (exp >= maxexp)
		{
			exp -= maxexp;
			...
			if (maxexp == -1) //升级到最大了，作者 changhao
			{
	...
	public int id; // 技能ID，作者 changhao
	public int times; // 学习次数，作者 changhao
	public int itemid; // 使用的道具，作者 changhao
