...
public class SkillRole extends SkillAgent
{
	...
	public void addEquipments(long roleId) {
		fire.pb.item.Equip equip = new fire.pb.item.Equip(roleId, true);
		SceneSkillRole sceneRole = SkillManager.getSceneSkillRole(roleId);
		for (fire.pb.item.ItemBase item : equip) {
			if (item instanceof fire.pb.item.EquipItem) {
				fire.pb.item.EquipItem equipItem = (fire.pb.item.EquipItem) item;
				if (equipItem.getExtInfo().getEndure() > 0) {
					sceneRole.unequip(equipItem.getEquipType(), null);
					sceneRole.addEquipEffectAndSkillWithSP(equipItem);
				}
			}
		}
	}

	public Result UpdateExtBuff(Map<Integer, Integer> extSkills, boolean send) {
		if (extSkills == null) {
			return new Result(false);
		}
		srole.getExtskills().putAll(extSkills);
		if (send) {
			sendExtSkills();
		}
		return addJingmaiSkillBuff();
	}

	public Result UpdateExtBuff1(Map<Integer, Integer> extSkills, boolean send) {
		return UpdateExtBuff(extSkills, send);
	}

	public Result addJingmaiSkillBuff() {
		return addExtSkillBuff();
	}

	public Result addJingmaiSkillBuff1() {
		return addExtSkillBuff();
	}

	private Result addExtSkillBuff() {
		Result result = new Result(true);
		FightJSEngine fightJSEngine = null;
		boolean binitJsEngine = false;
		BuffAgent buffpet = new BuffRoleImpl(roleId);

		for (int skillId : srole.getExtskills().keySet()) {
			FightSkillConfig sconf = fire.pb.skill.Module.getInstance().getFightSkillConfig(skillId);
			if (sconf == null) {
				continue;
			}
					...
					for (BuffUnit buffarg : subskill.getBuffUnits()) {
						...
						if (buffarg.effectJavascriptMap.size() > 0) {
							for (Map.Entry<Integer, JavaScript> entry : buffarg.effectJavascriptMap.entrySet()) {
								buff.getEffects().put(entry.getKey(),
										entry.getValue().eval(fightJSEngine, null, null).floatValue());
							}
						}
	...
	public Result addLiveSkillBuff()
	{
		...
		for (java.util.Map.Entry<Integer, xbean.LiveSkill> e : skillrole.getLiveSkills().entrySet())
		{
			...
			if (config.skillId != 0) //这是被动技能，作者 changhao
			{
				...
				if(skillconf.isActiveSkill())//是否是被动技能，作者 changhao
				{
					continue;
				}
				if(bInitjsEngine == false)
		...
		else
		{
			...
			case SkillConstant.TYPE_ROLE_SPECIAL_SKILL:
				if(getSpecialSkill(skillId)!= null)
					...
					return 0;
/*			案例 SkillConstant.TYPE_ROLE_OTHER_SKILL：
