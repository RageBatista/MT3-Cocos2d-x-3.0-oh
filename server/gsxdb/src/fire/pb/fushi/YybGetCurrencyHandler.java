		...
		if (ret.equals("0")) {
        	new mkdb.Procedure() {
				...
				protected boolean process() {
					int balance = Integer.valueOf(json.get("balance").asText()); //游戏币个数，包含赠送游戏币
					int genBalance = Integer.valueOf(json.get("gen_balance").asText()); //赠送游戏币个数
					int firstSave = Integer.valueOf(json.get("first_save").asText()); //是否满足首次充值，1满足，0不满足
					int saveAmt = Integer.valueOf(json.get("save_amt").asText()); //累计充值金额
