...
public class PSendRedPack extends Procedure {
	...
	protected boolean process() throws Exception {
		...
		if(modeltype==RedPackType.TYPE_WORLD){
			//发送跑马灯提示
			MessageMgr.sendSystemMsg(172011,Arrays.<String>asList(rolename,redpackinfo.getRedpackdes()));
			//发送链接
			MessageMgr.sendSystemMsg(172012,Arrays.<String>asList(rolename,redpackinfo.getRedpackdes(),redpackinfo.getRedpackid(),redpackinfo.getModeltype()+""));
		}else if(modeltype==RedPackType.TYPE_CLAN){
			//发送公会消息
			if (clanInfo != null ) {
		...
		if (modeltype==RedPackType.TYPE_WORLD) //红包排行榜，作者 changhao
		{
