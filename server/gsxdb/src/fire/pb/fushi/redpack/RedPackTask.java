...
//								MessageMgr.psendSystemMessageToRole(roleid, 172007,Arrays.<String>asList(RedPackManage.getTypeStringByType(redpackinfo.getModeltype()),money+""));
//							}别的{
								//添加金币
								fire.pb.item.Pack bag = new fire.pb.item.Pack(roleid, false);
								bag.addSysGold(money, "退回没有领取的红包", fire.log.enums.YYLoggerTuJingEnum.tujing_Value_redpack, 0);
								logger.info("角色id"+"\t"+roleid+"退回没有领取的红包,红包类型\t"+redpackinfo.getModeltype()+"红包金额\t"+money+"\t红包id"+redpackinfo.getRedpackid());
								MessageMgr.psendSystemMessageToRole(roleid, 172008,Arrays.<String>asList(RedPackManage.getTypeStringByType(redpackinfo.getModeltype()),money+""));
