...
public class PReqPay extends Procedure {
	...
	protected boolean process() throws Exception {
		...
		String platString = "locojoy";//auUserInfo.getNickname().substring(0, 4); //(test)登陆里面有个和平台相关的名字，作者 changhao
		...
		LinkedList<GoodInfo> commonGoods = platGoods.get(0); //服务器ID，作者 changhao
		if (commonGoods != null){
			for (GoodInfo goodInfo : commonGoods) {
				if(fire.pb.fushi.Module.GetPayServiceType() == 0){
					if (ConfigManager.getInstance().getConf(SAddCash.class).get(goodInfo.goodid).gameshow==1) //加入所用充值商品，作者 changhao 
						sReqCharge.goods.add(new GoodInfo(goodInfo.goodid, goodInfo.price, goodInfo.fushi, goodInfo.present, goodInfo.beishu));
				}else{
					if (ConfigManager.getInstance().getConf(SAddCashPCard.class).get(goodInfo.goodid).gameshow==1) //加入所用充值商品，作者 changhao 
						sReqCharge.goods.add(new GoodInfo(goodInfo.goodid, goodInfo.price, goodInfo.fushi, goodInfo.present, goodInfo.beishu));
				}
			}
		}
		...
		xbean.RoleDoubleChareInfo multiCharge = xtable.Roledoublecharge.select(roleid); //加倍充值返回记录，作者 changhao
		...
		if(multiCharge == null || multiCharge.getActivetime() < FushiManager.MULTICHARGE_CHARGE_START_TIME){
			...
			FushiManager.getInstance().initRoleDoubleChareInfo(multiCharge, sReqCharge.goods); //初始化玩家的多倍 返还充值情况，作者 changhao
		}
