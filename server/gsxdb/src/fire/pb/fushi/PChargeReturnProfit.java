...
public class PChargeReturnProfit extends Procedure {
	...
	protected boolean process() throws Exception {
		...
		else 
		{
			...
			{
				xbean.EChargeReturnProfit returnprofit = xtable.Roleid2chargereturnprofit.get(roleid); //得到返利数据，作者 changhao
				if (returnprofit == null)
				{
					xbean.EChargeReturnProfit profit = xbean.Pod.newEChargeReturnProfit();
			        for (SChargeReturnProfit e : map.values()) //初始化玩家的返利数据，作者 changhao
			        {
				...
				{
					if (profit.getStatus() == 2) //如果还未达标，作者 changhao
					{						
						if (fushi >= profit.getMaxvalue())
						{
							profit.setStatus(1);  //设置为可领取状态，作者 changhao
						}
					}
