...
public class PAddCoin extends Procedure {
	...
	protected boolean process() throws Exception {
		...
		xbean.YbNum ybNum = ybNums.getRoleyb().get(roleid); //得到角色对应的符石数量，作者 changhao
		if (ybNum == null) {
			try {
		...
		if (sn > 0 && ybNum.getSnlist().contains(sn)) {// 如果sn小于等于xdb里保存的sn,说明可能是重复的消息,不加符石,直接返回成功即可
			...
			ybNum.setNum(newNum);
			ybNum.setNopresentnum(sellnum + ybNum.getNopresentnum()); //没有赠送的实际符石，作者 changhao
		...
		pexecuteWhileCommit(new PChargeReturnProfit(user.getPrevloginroleid())); //更新充值返利状态，作者 changhao
