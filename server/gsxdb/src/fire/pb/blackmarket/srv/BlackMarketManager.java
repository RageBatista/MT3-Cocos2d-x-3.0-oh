...
import fire.pb.mysql.MysqlSyncManager;
...
public class BlackMarketManager {
	...
	public boolean goldOrderUp(long roleId, long gold, long rmb) {
		...
		// 同步到MySQL
		MysqlSyncManager.syncBlackMarket(roleId);
	...
	public void goldOrderTrade(final long buyerRoleId, final long sellerRoleId, final long pid) {
		...
		new mkdb.Procedure() {
			...
			protected boolean process() throws Exception {
				...
				// 同步到MySQL
				MysqlSyncManager.syncBlackMarket(buyerRoleId);
				MysqlSyncManager.syncBlackMarket(sellerRoleId);
	...
	public boolean goldOrderBrowse(final long roleId) {
		...
		// 优化：根据实际数据量设置初始容量，减少扩容
		int buyListSize = Math.min(goldOrderBuyMap.size(), BlackMarketUtils.getMaxOrderLstNumber());
		int saleListSize = Math.min(goldOrderSaleMap.size(), BlackMarketUtils.getMaxOrderLstNumber());
		ArrayList<GoldOrder> buyList = new ArrayList<>(buyListSize);
		ArrayList<GoldOrder> saleList = new ArrayList<>(saleListSize);
