...
public class GMHttpExecuteHandler implements HttpHandler {
			...
			if (oper != null) {
				...
				if (!sign.equalsIgnoreCase(compareSign)) {
					...
					if (!"".equals(messageBody)) {
						JsonNode json = JsonUtil.parseJson(messageBody);
					...
					if (data == null) {
						...
						for (Map.Entry<String, Object> entry : data.entrySet()) {
							...
							if (value instanceof String) {
								jsonObj.put(entry.getKey(), (String) value);
							} else if (value instanceof Integer) {
								jsonObj.put(entry.getKey(), (Integer) value);
							} else if (value instanceof Long) {
								jsonObj.put(entry.getKey(), (Long) value);
							} else if (value instanceof Double) {
								jsonObj.put(entry.getKey(), (Double) value);
							} else if (value instanceof Boolean) {
								jsonObj.put(entry.getKey(), (Boolean) value);
							}
						}
					}
	...
	private void setResponseInfo(ObjectNode jsonobj, int code) {
		if (jsonobj.has("Code")) {
			return;
		}
	...
	private static boolean parseAward(int itemCount, String sAwards, List<xbean.SingleCompensationAward> outAwards, Map<Integer, Integer> needDecCurrency) {
		ArrayNode jsAwards = (ArrayNode) JsonUtil.parseJson(sAwards);
		...
		for (int i = 0; i < jsAwards.size(); i++) {
			jobj = jsAwards.get(i);
			String[] awardString = jobj.get("Award").asText().split("\\|");
