...
public class ChatChannel {
	...
	public boolean verifyClanChat(final long senderId) {
		...
		if (member.getIsbannedtalk()) {
			...
			if (leveltime > 0) {
				...
				MessageMgr.sendMsgNotify(senderId, 141161,Arrays.<String>asList(leveltime + ""));
				return false;
			}
	...
	/***
	 * subType == 1 的时候忽略等级检测 时间频率检测，作者 changhao
		...
		mkdb.Procedure procedure = new mkdb.Procedure() {
			...
			protected boolean process() {
				...
				if (subType != 2) {
					...
					if (getCostMoney() > 0)
					{
						MessageMgr.psendMsgNotifyWhileCommit(senderId, 160375,Arrays.<String>asList(Integer.toString(getCostMoney())));						
					}
				}
	...
	/**
	 * @return 世界频道发言 活力消耗，作者 changhao
	 */
...
//		}
		...
		case ChannelType.CHANNEL_TEAM_APPLY:
			// subType === 1 表示跳过检查，上层已经做了时间检测了，作者 changhao
