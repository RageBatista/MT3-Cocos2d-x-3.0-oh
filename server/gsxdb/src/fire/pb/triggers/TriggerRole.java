...
public class TriggerRole
{
	...
	private void addQuest(TriggerEvent event)
	{
		if(event.addquest == 0)
			return;
		final int addQuestId = event.addquest;
		final int triggerId = event.id;
		mkdb.Procedure.pexecuteWhileCommit(
			new mkdb.Procedure() {
				@Override
				protected boolean process() {
					TeamManager.getTeamByRoleId(roleId);//强行锁队伍
					if (MissionManager.getInstance().getMissionConfig(addQuestId) == null) {
						Module.logger.warn("角色[" + roleId + "]触发器[" + triggerId + "]添加任务失败: 任务配置不存在 taskid=" + addQuestId);
						return true;
					}
					new PAcceptMajorMission(roleId, addQuestId, true, true).call();
					return true;
				}
			} );
	}
