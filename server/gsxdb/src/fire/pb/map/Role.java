...
public class Role extends Body {
	...
	public static void getPlayerComponents(final long roleid, final java.util.Map<Byte, Integer> info) {
		...
		if (prop == null) {
			info.put((byte)SpriteComponents.SPRITE_HORSEDRESS, 0);
			info.put((byte)SpriteComponents.ROLE_COLOR1, 0);
			info.put((byte)SpriteComponents.ROLE_COLOR2, 0);
			info.put((byte)SpriteComponents.SPRITE_EQUIP_EFFECT, 0);
			return;
		}
		...
		info.put((byte)SpriteComponents.SPRITE_HORSEDRESS, ride); //坐骑，作者 changhao
	...
	// 支持 Map<Integer, Integer> 类型（用于 FighterInfo.components 等）
	public static void getPlayerComponentsInt(final long roleid, final java.util.Map<Integer, Integer> info) {
		Equip equip = new Equip(roleid, true);
		ItemBase bi = equip.getWeapon();
		if (bi != null && bi instanceof EquipItem && ((EquipItem)bi).getEndure() > 0) {
			info.put(SpriteComponents.SPRITE_WEAPON, bi.getItemId());
			info.put(SpriteComponents.SPRITE_WEAPONCOLOR,
					((EquipItem)bi).getItemAttr().equipcolor);
		} else {
			info.put(SpriteComponents.SPRITE_WEAPON, 0);
			info.put(SpriteComponents.SPRITE_WEAPONCOLOR, 0);
		}
		bi = equip.getHeaddress();
		if (bi != null && bi instanceof EquipItem && ((EquipItem)bi).getEndure() > 0) {
			info.put(SpriteComponents.SPRITE_HEADDRESS, bi.getItemId());
		}

		xbean.Properties prop = xtable.Properties.select(roleid);
		if (prop == null) {
			info.put(SpriteComponents.SPRITE_HORSEDRESS, 0);
			info.put(SpriteComponents.ROLE_COLOR1, 0);
			info.put(SpriteComponents.ROLE_COLOR2, 0);
			info.put(SpriteComponents.SPRITE_EQUIP_EFFECT, 0);
			return;
		}
		int rideitemid = prop.getRide();
		...
		info.put(SpriteComponents.SPRITE_HORSEDRESS, ride);
		info.put(SpriteComponents.ROLE_COLOR1, prop.getRolecolor1());
		info.put(SpriteComponents.ROLE_COLOR2, prop.getRolecolor2());
		info.put(SpriteComponents.SPRITE_EQUIP_EFFECT, prop.getEquipeffect());
	}

	public static void fillPlayerComponents(final long roleid, final java.util.Map<Byte, Integer> info) {
		getPlayerComponents(roleid, info);
	}

	// 支持排行榜等使用 Character 类型的场景（避免类型擦除冲突）
	public static void fillPlayerComponentsChar(final long roleid, final java.util.Map<Character, Integer> info) {
		java.util.Map<Byte, Integer> byteMap = new java.util.HashMap<>();
		getPlayerComponents(roleid, byteMap);
		for (java.util.Map.Entry<Byte, Integer> entry : byteMap.entrySet()) {
			info.put((char)entry.getKey().byteValue(), entry.getValue());
		}
	}

	// 支持 xbean 自动生成的使用 Integer 类型的场景（兼容旧代码，避免类型擦除冲突）
	public static void fillPlayerComponentsInt(final long roleid, final java.util.Map<Integer, Integer> info) {
		java.util.Map<Byte, Integer> byteMap = new java.util.HashMap<>();
		getPlayerComponents(roleid, byteMap);
		for (java.util.Map.Entry<Byte, Integer> entry : byteMap.entrySet()) {
			info.put((int)entry.getKey().byteValue(), entry.getValue());
		}
	}
