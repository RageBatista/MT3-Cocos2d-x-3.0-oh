...
public class PEnterClanFightBattleField extends mkdb.Procedure
{
	...
	@Override
	/***
	 * 锁定 ->team-> clanfightid -> roleid，作者 changhao
	 */
	...
	{		
		...
		if (cid == null) //没有公会，作者 changhao
		{
			MessageMgr.sendMsgNotify(roleid, 410049, null);					
			return false;
		}
		
		xbean.ClanInfo c = xtable.Clans.get(cid); //先锁公会，作者 changhao
		if (c != null)
		{				
			xbean.TeamInfo teaminfo = null;
			if (teamId != null)
			{
				teaminfo = xtable.Team.get(teamId); //锁定队伍，作者 changhao
			}
			...
			if (ids != null)
			{
				lock(mkdb.Lockeys.get(xtable.Locks.CLANFIGHTIDLOCK, ids.getIds())); //锁定公会战id，作者 changhao
				...
				{	
					...
					if (haveabsorleave == true) //队伍中有暂离的队员，作者 changhao
					{
				...
				lock(mkdb.Lockeys.get(xtable.Locks.ROLELOCK, roleids)); //锁定角色，作者 changhao
				
				for (Long r : roleids)
				{
					xbean.ClanMemberInfo mi = c.getMembers().get(r);
					if (mi == null) //有不在同一公会，作者 changhao
					{
					...
					if (cur < mi.getJointime() + day1 * fire.pb.activity.clanfight.ActivityClanFightManager.CLAN_FIGHT_JOIN_CLAN_TIME) //有人加入公会时间不足2天，作者 changhao
					{
						if (roleids.size() > 1)
						{
					...
					if (prop.getLevel() < fire.pb.activity.clanfight.ActivityClanFightManager.CLAN_FIGHT_ROLE_LEVEL) //等级不够，作者 changhao
					{
						if (roleids.size() > 1)
						{
