...
public class CStartClanFightBattle extends __CStartClanFightBattle__ {
	...
	protected void process() {
		...
		mkdb.Procedure startclanfightbattle = new mkdb.Procedure()
		{
			...
			protected boolean process()  throws Exception
			{
				...
				Long c1 = xtable.Roleid2clanfightid.select(roleid); //检测是否在战场中，作者 changhao
				if (c1 == null)
				{
					String srcname = xtable.Properties.selectRolename(roleid);
					String targetname = xtable.Properties.selectRolename(targetid);
				...
				Long c2 = xtable.Roleid2clanfightid.select(targetid); //检测是否在战场中，作者 changhao
				if (c2 == null)
				{
					String srcname = xtable.Properties.selectRolename(roleid);
					String targetname = xtable.Properties.selectRolename(targetid);
				...
				if (!c1.equals(c2)) //是否在同一公会战里，作者 changhao
				{
				...
				if (team != null)
				{
					if (team.isNormalMember(targetid)) //如果是正常队员，作者 changhao
					{
	...
	public long targetid; // 目标id，作者 changhao
