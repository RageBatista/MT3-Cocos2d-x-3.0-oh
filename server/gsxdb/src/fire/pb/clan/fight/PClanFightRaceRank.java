...
{
	java.util.List<Long> timelist; //计算哪天的战场，作者 changhao
	...
	{	
		...
		//计算出本赛季的公会战 胜率等统计，作者 changhao
		java.util.Map<Long, ActivityClanFightManager.ClanFightStatisticsInfo> infos = ActivityClanFightManager.getInstance().CalcClanStatistics(8);//一轮的数据，作者 changhao
		...
		if (timelist.size() == 2)
		{
			...
			this.lock(mkdb.Lockeys.get(xtable.Locks.CLANFIGHTRACELIST, locks)); //锁排行榜，作者 changhao
			this.lock(mkdb.Lockeys.get(xtable.Locks.CLANFIGHTS, locks)); //锁，作者 changhao			
		}
		...
		for (Long t : timelist)
		{
			if (timelist.size() == 2) //计算的是2场比赛的总排名，作者 changhao
			{
			...
			else
			{
				...
				if (clanfights != null)
				{
					this.lock(mkdb.Lockeys.get(xtable.Locks.CLANFIGHTIDLOCK, clanfights.getClan1vschan2().keySet())); //锁所有公会战id，作者 changhao
