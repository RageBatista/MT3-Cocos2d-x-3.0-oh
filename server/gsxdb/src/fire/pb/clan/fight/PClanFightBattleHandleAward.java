...
public class PClanFightBattleHandleAward extends mkdb.Procedure
{
	...
	@Override
	/**
	 * lock clanid -> clanfightid -> roleids，作者 changhao
	 */
	protected boolean process() throws Exception
	{
		...
		this.lock(mkdb.Lockeys.get(xtable.Locks.CLANS, setclanids)); //锁定2个公会，作者 changhao	
		...
		if (bf.getClanfightBean().getState() == xbean.ClanFightBattleField.STATE_CLAN_FIGHT_OVER) //结束了就不算了，作者 changhao
		{
			return true;
		}
		...
		this.lock(mkdb.Lockeys.get(xtable.Locks.ROLELOCK, roleids)); //锁所有角色，作者 changhao
		
		//处理赢的 一边 奖励，作者 changhao
		if (winners.size() != 0)
		{
			int totalscroe = losers.size() * ActivityClanFightManager.ONE_PEOPLE_SCROE; //获得总积分，作者 changhao
			
			int whichwin = bf.getSideByRoleId(winners.get(0));
			
			boolean attackerside = false; //当前赢的这边是主动发起攻击，作者 changhao
			...
			for (Long f : winners)
			{
				...
				statistics.setWinnum(statistics.getWinnum() + 1); //连胜加1，作者 changhao
				
				if (attackerside == true) //攻击方扣行动力，作者 changhao
				{
				...
				bf.addClanScore(whichwin, addscroe); //赢的一边加公会积分，作者 changhao
				...
				if (statistics.getWinnum() == ActivityClanFightManager.WIN_NUM) //连胜宝箱，作者 changhao
				{
				...
				if (statistics.getWinnum() >= ActivityClanFightManager.WIN_NUM)
				{
					...
					if (first == true)
					{
						...
						else
						{
							if (attackerclan != null)
							{
								//<T t="敌方" c="ff261407"/><T t="$parameter1$" c="ff261407"/><T t="的队伍获得了" c="ff261407"/><T t="$parameter2$" c="ff261407"/><T t="连胜" c="ff261407"/>，作者 changhao
				...
				if (statistics.getAct() <= 0) //行动力小于0离开战场，作者 changhao
				{	
		...
		//处理输的一边奖励，作者 changhao
		if (losers.size() != 0)
		{
			int whichlose = bf.getSideByRoleId(losers.get(0));
			
			boolean attackerside = false; //当前输的这边是主动发起攻击，作者 changhao
			...
			for (Long f : losers)
			{
				...
				fire.pb.talk.MessageMgr.psendMsgNotifyWhileCommit(roleid, 410039, 0, p);//扣除行动力，作者 changhao
				...
				if (onepeople == true)
				{
					...
					bf.addClanScore(whichlose, addscroe); //赢的一边加公会积分，作者 changhao
