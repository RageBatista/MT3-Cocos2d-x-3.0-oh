...
public class PClanFightOver extends mkdb.Procedure
{
	...
	@Override
	/***
	 * locs clanid -> clanfights -> clanfightid，作者 changhao
	 */
	...
	{	
		...
		if (bf != null)
		{
			...
			mkdb.Executor.getInstance().schedule(new Runnable() //60秒后开始刷宝箱，作者 changhao
			{
				@Override
				public void run()
				{
			...
			mkdb.Executor.getInstance().schedule(new Runnable() //10分钟后开始销毁宝箱，作者 changhao
			{
				@Override
				public void run()
				{
			...
			//清理玩家统计信息，作者 changhao
			PClanFightClearRolesStatistics clear = new PClanFightClearRolesStatistics(clanfightid);	
			mkdb.Executor.getInstance().schedule(new Runnable() //12分钟后清理玩家信息，作者 changhao
			{
				@Override
				public void run()
				{
			...
			mkdb.Executor.getInstance().schedule(new Runnable() //20秒后离开战场，作者 changhao
			{
				@Override
				public void run()
				{
