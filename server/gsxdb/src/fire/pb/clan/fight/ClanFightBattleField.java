...
{
	static long DELETE_TIMER = 5000; //战场删除定时器，作者 changhao
	static long COMMON_TIMER = 5001; //普通定时器 （加经验，加积分 等)，作者 changhao
	static long CLANFIGHT_OVER = 5002; //战场结束定时器，作者 changhao
	static long CLANFIGHT_ADDSCROE = 5003; //战场积分定时器，作者 changhao
	...
	{
		//初始化活动时间相关，作者 changhao
		initTime();
		
		//创建动态地图，作者 changhao
		initMap();
	
		//创建战场需要的定时器，作者 changhao
		createTimer();
	...
	private void initMap()
	{
		...
		if (mapcfg != null)
		{
			...
			if (mapcfg.dynamic == 1)
			{
				DuplicateHelper.createDynamicMap(mapId, clanfightid, "",true, -1, DynamicSceneType.NORMAL_SCENE, null); //修复我
			}
		}
	}
	...
	/***
	 * 创建公会战场用的定时器，作者 changhao
	 */
	public void createTimer() throws Exception
	{
		long cur = System.currentTimeMillis();
				
		//注册公会战场结束定时器，作者 changhao
	 ...
	 * @参数角色Id
	 * @参数哪个
	 * lock teamid -> roleids，作者 changhao
	 * @返回
	 */
	public int enterBattleField(long roleId, int which)
	{
		...
		for (Long eroleid : roleids)
		{
			...
			if (statistics.getAct() <= 0) //行动力不足不能进了，作者 changhao
			{
			...
			mkdb.Procedure.psendWhileCommit(eroleid, msg); //发送行动力数据，作者 changhao
	 ...
	 * 离开战场，作者 changhao
	 * @参数角色Id
	 * lock teamid -> roleid，作者 changhao
	 * @返回
	 */
	...
	{	
		...
		List<Long> quitteamroleids = new LinkedList<Long>(); //需要离队的角色，作者 changhao
		...
		if (team != null)
		{
			...
			if (num != team.getAllMemberIds().size() || ((fightperiod == true) && dismiss == true) || dismiss == true) //如果有非正常队员，作者 changhao
			{
				if (team.isTeamLeader(roleId))
				{
		...
		for (Long e : quitteamroleids) //执行离队操作，作者 changhao
		{
	...
	/***
	 * 给公会加积分，作者 changhao
	 * which == 0 是左边公会 which == 1 是右边公会，作者 changhao
	 * @参数哪个
	 * @参数分数
	 */
	...
	/***
	 * 通过公会id 得到是战场那边（左边0右边1 中立-1)，作者 changhao
	 * @帕拉克兰德
	 * @返回
	 */
	...
	/***
	 * 通过role id 得到是战场那边（左边0右边1 中立-1)，作者 changhao
	 * @帕拉克兰德
	 * @返回
	 */
	...
	public void scroeSort(int which, boolean focre)
	{
		...
		if (cur < stamp + 60000 * 1 && focre == false) //至少1分钟才能排一次，作者 changhao
		{
			return;
		}
	...
	{	
		//胜利的公会有庆贺宝箱，作者 changhao
		if (whichclanwin != -1) //获胜方才刷庆贺宝箱，作者 changhao
		{
			long winclanid = 0;
			int num = 0;
			if (whichclanwin == 0)
			...
			{
				final long npcKey = fire.pb.map.SceneNpcManager.getInstance().getNextId();		
				SceneNpcManager.createNpcByMap(npcKey, ActivityClanFightManager.CELEBRATE_BOX, "", ClanUtils.MAPID, winclanid, 3, 0); //公会领地刷新庆贺宝箱，作者 changhao
	...
	public int checkWhichWinByPeopleNum()
	{
		if (clanfightbean != null)
		{
			...
			if (cur < getClanfightBean().getActivitypreparetime()) //至少过了准备时间，作者 changhao
			{
				return -1;
			}
