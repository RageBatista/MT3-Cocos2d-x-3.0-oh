...
public class ClanBaseManager extends ClanManage {
	...
	public void addNewClanMember(long masterid, long applyroleid,xbean.Properties properties,xbean.ClanInfo claninfo,boolean flag){
		...
		MessageMgr.psendSystemMessageToRole(applyroleid, 160186,Arrays.<String>asList(claninfo.getClanname()));
		//通过申请加入公会后屏幕中间没有系统提示160332
		MessageMgr.psendMsgNotifyWhileCommit(applyroleid, 160332,Arrays.<String>asList(claninfo.getClanname()));
	...
	public boolean applyClan(long roleId, long clankey, boolean isOneKey) {
		...
		if (properties.getLevel() < getJoinClanLevel()) {
			MessageMgr.sendMsgNotify(roleId, 150137,Arrays.<String>asList(getJoinClanLevel()+""));
			return false;
		}
	...
	public boolean createClan(long roleId, String clanname, String clanaim) {
		...
		factionInfo.getHouse().put(ClanManage.GoldBank, 0); //金库 初始0级，作者 changhao
		factionInfo.getHouse().put(ClanManage.DrugStone, 0); //药房 初始0级，作者 changhao
		factionInfo.getHouse().put(ClanManage.Hotel, 0); //旅馆 初始0级，作者 changhao
		factionInfo.setBonus(0); //公会分红，作者 changhao
		factionInfo.setAutostate(0);//1表示自动开启   0表示关闭
		factionInfo.setRequestlevel((short) 1);
		factionInfo.setTotalscore(propRole.getProperties().getRolezonghemaxscore()); //刚创建公会评分就是会长的，作者 changhao
		...
		if (teamId != null) //先把team 锁住 ,后面公会创建任务完成后,会锁定队伍，作者 changhao
		{
			fire.pb.team.Team team = fire.pb.team.TeamManager.getTeamByTeamID(teamId);
			mkdb.Lockeys.lock(mkdb.Lockeys.get(xtable.Locks.ROLELOCK,team.getAllMemberIds())); //把所有队员锁掉，作者 changhao			
		}		
...
//				Procedure.psendWhileCommit(masterid,ClanManage.packagingToApplicantMemberList(clanInfo));
			...
			if (ClanUtils.getClanInfoById(applyroleid, true) != null) {
				// 已经入了其他帮了
				MessageMgr.psendMsgNotify(masterid, 145116,Arrays.<String>asList(new PropRole(applyroleid, true).getName()));
				//这里把玩家从申请列表中删除
			...
			clanInfo.getApplicants().remove(applyroleid);
			cancelApply(applyroleid, clanInfo.getKey());
			MessageMgr.psendSystemMessageToRole(applyroleid, 160213,Arrays.<String>asList(clanInfo.getClanname()));
			MessageMgr.sendMsgNotify(applyroleid, 150134,Arrays.<String>asList(clanInfo.getClanname()));
			Procedure.psendWhileCommit(masterid, new SRefuseApply(applyroleid));
		}
		return true;
	}
		...
		for(Map.Entry<Long, xbean.ClanMemberInfo> entry : claninfo.getMembers().entrySet()){
			mkdb.Procedure proc=new mkdb.Procedure(){
				...
				protected boolean process() {
					...
					MessageMgr.psendSystemMessageToRole(roleId, 160233,Arrays.<String>asList(Integer.toString(ClanUtils.getCurrentClanPoint(roleId)/2)));
	...
	public boolean invitationClan(long guestroleid, long hostroleid) {
		...
		if (properties.getLevel() < getJoinClanLevel()) {// 判断被邀请入会者等级
			MessageMgr.sendMsgNotify(guestroleid, 150137,Arrays.<String>asList(getJoinClanLevel()+""));
			return false;
		}
	...
	public boolean leaveClan(long roleId) {
		...
		if (fmi != null)
		{
			Long clanfightid = xtable.Roleid2clanfightid.select(roleId); //在公会战场中不能退会，作者 changhao
			if (clanfightid != null)
			{
		...
		//在好友频道发送消息160257
		MessageMgr.psendSystemMessageToRole(roleId, 160257,Arrays.<String>asList(Integer.toString(ClanUtils.getCurrentClanPoint(roleId)/2)));
