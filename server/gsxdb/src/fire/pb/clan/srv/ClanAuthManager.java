...
public class ClanAuthManager extends ClanManage {
	...
	public boolean bannedTalk(long masterId, long memberId, int flag) {
		...
		if (flag == 1) {
			...
			MessageMgr.sendMsgNotify(memberId, 141161,Arrays.<String>asList(ClanManage.getLeaveStopTalkMin(memberInfo)+""));
			//<T t="你已被"  c="FF693F00"></T><T t="$parameter1$" c="ff06cc11"></T><T t="禁言，无法在公会频道发言，2小时后将自动解除。" c="FF693F00"></T>
			MessageMgr.psendSystemMessageToRole(memberId, 160241,Arrays.<String>asList(mastername));
			...
			MessageMgr.psendSystemMessageToRole(memberId, 160242,Arrays.<String>asList(mastername));
...
//	}
	...
	public boolean changePosition(long masterId, long memberId, int position) {
		...
		if (isPositionFull(masterId, position, clanInfo)) {
			...
			MessageMgr.psendMsgNotify(masterId, 160427,Arrays.<String>asList(ClanUtils.retNameByPosition(position)));
			return false;
		}
		...
		if (position == memberInfo.getClanposition()) {
			...
			MessageMgr.psendMsgNotify(masterId, 145105,Arrays.<String>asList(paras));
			return false;
		}
		// 职位从这里开始变化
	...
	public void changePosition(long masterId, long memberId, int position,int oldposition, xbean.ClanMemberInfo memberInfo,PropRole memberProp,xbean.ClanInfo clanInfo){
		...
		if(masterId>0){
			MessageMgr.psendMsgNotifyWhileCommit(masterId, 145092, 0,Arrays.<String>asList(parasToOperator));
		}
	...
	public boolean fireMemeberClan(long masterId, long memberId, int reasonType) {
		...
		Long clanfightid = xtable.Roleid2clanfightid.select(memberId); //是否在公会战中，作者 changhao
		if (clanfightid != null) //如果此人在公会战中不能踢出，作者 changhao
		{
	...
	public void fireMemeberClanResult(long masterId,long memberId,int reasonType,xbean.ClanInfo clanInfo,int position){
		...
		MessageMgr.psendSystemMessageToRole(memberId, 160188,Arrays.<String>asList(masterroleName, getReasonContent(reasonType), Integer.toString(ClanUtils.getCurrentClanPoint(memberId)/2)));
		if(masterId>0){
