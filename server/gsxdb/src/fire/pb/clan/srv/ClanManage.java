...
public abstract class ClanManage {
	...
	public static int Lobby = 1; //公会大厅，作者 changhao
	public static int GoldBank = 2; //金库，作者 changhao
	public static int DrugStone = 3; // 药店，作者 changhao
	public static int Hotel = 4; //旅店，作者 changhao
	...
	public static boolean addClanBonus(long roleId,int addvalue,String reason,boolean showMsg){
		...
		if(showMsg){
			//提示获得公会分红
			MessageMgr.psendMsgNotifyWhileCommit(roleId, 160158,Arrays.<String>asList(addvalue+""));
		}
	...
	public static boolean addContribution(long roleId,int addvalue,String reason, YYLoggerTuJingEnum way, boolean showMsg){
		...
		if(System.currentTimeMillis()-member.getJointime()>FREERED_TIME){
			...
			if(showMsg){
				//发送客户端提示
				if(tipvalue>0){
					MessageMgr.psendMsgNotifyWhileCommit(roleId, 160065,Arrays.<String>asList(tipvalue+""));
				}
			}
		}
		if(tipvalue>0){
	...
	public static boolean AddClanMoney(int money, xbean.ClanInfo faction)
	{
		...
		if (v > bank.limitmoney) //超过资金上限，作者 changhao
		{
			v = bank.limitmoney;
		}
...
//			clanpoint.setWeekclanpoint(0);//周清零
	...
	public static boolean delContribution(long roleId,int delvalue,String reason,boolean showMsg){ 
		...
		if(showMsg){
			MessageMgr.psendMsgNotifyWhileCommit(roleId, 160313,Arrays.<String>asList(delvalue+""));
...
//	}
	...
	public static mkdb.Procedure sendCRequestRuneInfo(long roleid){
		return new mkdb.Procedure(){
			...
			protected boolean process() throws Exception {
				...
				int skilllevel = LiveSkillManager.getInstance().GetEnhanceLevelBySchool(roleid, school);//取得职业技能等级，作者 changhao
				if (skilllevel>0)
				{
	...
	public static boolean sendCRuneGive(long selfroleid,long targetid,int selecttype,int itemid,int itemkey,int bagtype){
		...
		if(selecttype==0){
			...
			int skilllevel = LiveSkillManager.getInstance().GetEnhanceLevelBySchool(selfroleid, school);//取得职业技能等级，作者 changhao
			if (skilllevel == 0)
			{
		...
		//通知被捐献人，有人给他捐赠物品了
		MessageMgr.psendMsgNotifyWhileCommit(targetid, 160394,Arrays.<String>asList(prop.getRolename(),item.getName()));
...
//		}
		...
		claninfo.setTotalscore(claninfo.getTotalscore() - prop.getRolezonghemaxscore()); //公会综合实力评分变化，作者 changhao
...
//		return level;
	...
	protected xbean.ClanMemberInfo joinClanMember(long roleId, xbean.Properties properties,
		...
		clanInfo.setTotalscore(clanInfo.getTotalscore() + properties.getRolezonghemaxscore()); //公会综合实力评分增加，作者 changhao
	...
	protected SOpenClan packagingToProtocol(xbean.ClanInfo clanInfo) {
		...
		sof.money = clanInfo.getMoney(); //公会资金，作者 changhao
		sof.house.putAll(clanInfo.getHouse()); //公会房子情况，作者 changhao
