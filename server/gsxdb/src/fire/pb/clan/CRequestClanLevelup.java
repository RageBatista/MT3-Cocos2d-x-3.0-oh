...
public class CRequestClanLevelup extends __CRequestClanLevelup__ {
	...
	protected void process() {
		...
		mkdb.Procedure requestfactionlevelup = new mkdb.Procedure()
		{
			...
			protected boolean process()
			{
				...
				int cost=0;//消耗
				int opid=0;//
				if (id == ClanManage.Lobby) //大厅升级，作者 changhao
				{
					...
					if (sumlevel < sum) //要求不够，作者 changhao
					{
					...
					if (money < ClanUtils.getBuildLevelupMax(clan).get(ClanManage.Lobby)) //贮备的金钱不足，作者 changhao
					{
				...
				else if (id == ClanManage.GoldBank)
				{
					...
					if (money < ClanUtils.getBuildLevelupMax(clan).get(ClanManage.GoldBank)) //贮备的金钱不足，作者 changhao
					{
						fire.pb.talk.MessageMgr.sendMsgNotify(roleid, 160224, null);
						return true;
					}
				...
				else if (id == ClanManage.DrugStone)
				{
					...
					if (money < ClanUtils.getBuildLevelupMax(clan).get(ClanManage.DrugStone)) //贮备的金钱不足，作者 changhao
					{
						fire.pb.talk.MessageMgr.sendMsgNotify(roleid, 160224, null);
						return true;
					}
				...
				else if (id == ClanManage.Hotel)
				{
					...
					if (money < ClanUtils.getBuildLevelupMax(clan).get(ClanManage.Hotel)) //贮备的金钱不足，作者 changhao
					{
						fire.pb.talk.MessageMgr.sendMsgNotify(roleid, 160224, null);
						return true;
					}
				...
				mkdb.Procedure.psendWhileCommit(clan.getMembers().keySet(), sClanLevelup); //发送给所有会员，作者 changhao
				//运营日志
	...
	public int id; // 1大厅2金库3药店4旅馆，作者 changhao
