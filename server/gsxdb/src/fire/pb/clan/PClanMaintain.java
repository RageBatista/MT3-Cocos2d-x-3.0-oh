...
public class PClanMaintain extends Procedure {
	...
	protected boolean process() throws Exception {
		...
		if (ok) //维护扣钱成功，作者 changhao
		{
			//告诉会长和副会长，作者 changhao
			Integer c = cost;
		...
		else //公会费用不足已维护 降级，作者 changhao
		{
			if (level <= 1) //解散，作者 changhao
			{	
				flag = false;					
			...
			else //降级，作者 changhao
			{
				...
				ClanManage.AddClanMoney(lobby.downcompensate, clanInfo);//降级公会资金补偿，作者 changhao
