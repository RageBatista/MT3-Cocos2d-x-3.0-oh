...
import fire.pb.school.SchoolManager;
...
public class ProfessionLeaderManager {
	...
	public boolean checkAlreadyVote(long roleid){
		...
		if(voteInfo != null)
		{
			...
			//如果上次投票时间也在voteStartTime和voteEndTime之间,说明已经投票了,不能再投，作者 changhao
	...
	public static void addDynamicServices(long roleid, long npckey,int npcid, List<Integer> result) {
		...
		School s = SchoolManager.getSchoolById(propRole.getProperties().getSchool());
		int oneleveltitle = SchoolManager.getTitleId(s, 1, propRole.getProperties().getSex());
		int twoleveltitle = SchoolManager.getTitleId(s, 2, propRole.getProperties().getSex());
		int threeleveltitle = SchoolManager.getTitleId(s, 3, propRole.getProperties().getSex());
		...
		{			
			if (ProfessionLeaderManager.getInstance().checkCanVote(System.currentTimeMillis())) //如果在投票时间，作者 changhao
			{
				//已经参加竞选 可以修改宣言，作者 changhao
			...
			if(ProfessionLeaderManager.getInstance().isInChallengePeriod(System.currentTimeMillis())) //如果在挑战时间，作者 changhao
			{							
				if (b == true) //已经挑战过了，作者 changhao
				{
					//已经参加竞选 可以修改宣言，作者 changhao
					xbean.ProfessionLeaderTicketInfo c = fire.pb.school.shouxi.ProfessionLeaderManager.getInstance().getCandInfoFromRoleid(roleid, school);
					if (c != null)
