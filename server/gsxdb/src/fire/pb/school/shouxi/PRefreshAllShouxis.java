...
public class PRefreshAllShouxis extends Procedure {
   Logger logger=Logger.getLogger(PRefreshAllShouxis.class);
	...
	protected boolean process() throws Exception
	{
		...
		{
			//从选举表中找出票数最高的人,如果没有candidate或者票数都为0，则首席弟子不变，作者 changhao
			...
			if(professionleaderroleid > 0)
			{
				try
				{
					...
					monsterBean.setMonsterid(school.getInfo().shouxiBattleNpcId);
					...
					int titleID = school.getInfo().shouxiTitleId;
					//去掉上任首席弟子的称谓，作者 changhao
					if(sxi.getRoleid() > 0)
					{
...
//					AchievementManager.giveAchievement(achievementSystem, 410501);
	...
	private void changeShouxiShape(long shouxiID,int school,Map<Integer, Integer> component) {
		...
		School s = fire.pb.school.SchoolManager.getSchoolById(school);


		java.util.List<NPC> npcs = SceneNpcManager.getInstance().getNpcByID(s.getInfo().shouxiNpcId);
		if (npcs.get(0) == null)
		{
			return;
		}
