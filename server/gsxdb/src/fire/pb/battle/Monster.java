...
public class Monster
{
	...
	/*创建NPC类型的怪物*/
	static xbean.Monster createNpcMonster(int monsterId,int level,final xbean.BattleInfo battle, double bonus, Map<Integer,Float> fixedL2attrs, xbean.HuoBanshuxing huobanInfo/*侠客资讯侠客资讯*/, int color)
	{
		...
		monster.setMonstertype(monsterConfig.getMonstertype()); //设置这个怪物的类型，作者 changhao
		monster.setSpecialtype(monsterConfig.getSpecialtype()); //设置这个怪物的特别类型，作者 liuxinhua
		monster.setSchool(monsterConfig.getSchool()); //设置这个怪物的职业，作者 liuxinhua
	...
	public static xbean.Monster createHuobanMonster(int monsterId, int level,
		...
		monster.setSchool(huobanInfo.getSchool()); // 设置这个怪物的职业，作者 liuxinhua
		// 对于怪物的造型，优先级顺序是: 外部传入 > 关联宠物造型 > 造型随机 > 造型ID
		monster.setShape(0);
...
//		}
	 ...
	 * @param huobanInfo 
	 */
	private static void initMonsterAI(xbean.Monster monster, xbean.HuoBanshuxing huobanInfo/*侠客资讯侠客资讯*/,xbean.BattleInfo battle)
	{
