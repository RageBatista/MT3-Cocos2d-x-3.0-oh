...
public class PPlayScript extends Procedure {
	...
	java.util.List<ResultItem> getScript(final xbean.BattleInfo battle, final List<fire.pb.battle.AIOperation> clientAIActions)
	{
		...
		printScript(script,battle);// FIXME 打印仅用于测试
		printAI(clientAIActions,battle);// FIXME 打印仅用于测试
		return script;
	}
	...
	private void printAI(List<fire.pb.battle.AIOperation> aiactions,xbean.BattleInfo battle )// FIXME 打印仅用于测试
	{
		if(!BattleField.logger.isDebugEnabled())
			return;
		Set<Long> roleIds = new HashSet<Long>();
