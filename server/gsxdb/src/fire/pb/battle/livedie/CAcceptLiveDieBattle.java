...
public class CAcceptLiveDieBattle extends __CAcceptLiveDieBattle__ {
	...
	protected void process() {
		...
		if (PvPHelperManager.isPvPMap(hostRole.getMapId())){
			...
			fire.pb.talk.MessageMgr.sendMsgNotify(hostid,162130,Arrays.<String>asList(guestprop.getRolename()));
			return ;
		}
		//判断下战书的玩家是否在战斗或者观战
		...
		if(hostliveDieRoleInfo.getSelecttype()==1){//组队
			...
			if(hostteam==null){
				//提示发起人，成为队长
				fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 162125,Arrays.<String>asList(guestprop.getRolename()));
				fire.pb.talk.MessageMgr.sendMsgNotify(guestid, 162121, null);
				return ;
			}
