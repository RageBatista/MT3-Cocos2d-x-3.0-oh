			...
			protected boolean process() throws Exception {
				...
				if(hostliveDieRoleInfo!=null){
					if(System.currentTimeMillis()-hostliveDieRoleInfo.getInvitationtime()<LiveDieMange.getLiveDieTime()){
						...
						if(objectid==hostliveDieRoleInfo.getObjectid()){
							//提示已经发送过162073
							fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 162073,Arrays.<String>asList(name));
						}else{
							//提示已经发送过162074
							fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 162074,Arrays.<String>asList(name));
						}
						return false;
					}
				}
				...
				if(selfid!=null){
					...
					if(otherLiveDieRoleInfo!=null&&System.currentTimeMillis()-otherLiveDieRoleInfo.getInvitationtime()<LiveDieMange.getLiveDieTime()){
						...
						fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 162076,Arrays.<String>asList(name));
						return false;
					}
				}
				//目标身上有生死战称号，不能下战书
				...
				if(geustliveDieRoleInfo!=null){
					if(System.currentTimeMillis()-geustliveDieRoleInfo.getInvitationtime()<LiveDieMange.getLiveDieTime()){
						...
						fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 162077,Arrays.<String>asList(name));
						return false;
					}
				}
				//目标是否接受了别人的战书
				...
				if(otherid!=null){
					...
					if(otherLiveDieRoleInfo!=null&&System.currentTimeMillis()-otherLiveDieRoleInfo.getInvitationtime()<LiveDieMange.getLiveDieTime()){
						if(otherLiveDieRoleInfo.getObjectid()==hostid){
							String name=new PropRole(objectid,true).getName();
							fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 162075,Arrays.<String>asList(name));
							return false;
						}else{
							String name=new PropRole(objectid,true).getName();
							fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 162077,Arrays.<String>asList(name));
...
//					MessageMgr.sendMsgNotify(hostid, 145005, null);
				...
				MessageMgr.sendSystemMsg(162066,Arrays.<String>asList(hostname,guestname));
