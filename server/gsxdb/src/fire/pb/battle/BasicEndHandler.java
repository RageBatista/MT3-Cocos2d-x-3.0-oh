...
public class BasicEndHandler extends BattleEndHandler {
	...
	protected boolean handleAward() {
		...
		if (battle.getBattletype() % 10 == 0) {
			...
			if (battle.getAreatype() == xbean.BattleInfo.AREA_BATTLEHIDE) {
				...
				sharedid = hideAreaInfo.getShared(); //得到共享战利品ID，作者 changhao
	...
	protected boolean itemAward(int lootid, int sharedid, List<Long> roleids) {
		...
		if (lootid > 0)
			super.distributeItemAward(roleids, lootid, paras); //先把必给的奖励给了，作者 changhao
		
		boolean roll = sharedid != 0 && roleids.size() > 1;
		
		if (roll) //如果需要ROLL点，作者 changhao
		{
			processRoll(sharedid, roleids, paras); //在发送roll点奖励，作者 changhao
		}
		else
		{
			if (sharedid != 0) //没有组队直接给,不走ROLL点形式，作者 changhao
			{
	...
	private void processRoll(int lootid, List<Long> roleids, Map<String, Object> paras) //处理ROLL点，作者 changhao
	{
		if (!roleids.isEmpty())
		{
			int times = RewardMgr.getInstance().getTimes(lootid);
			...
			if (rollawarditems.isEmpty()) //产生的道具，作者 changhao
			{
				return;
			}
			...
			new fire.pb.team.teammelon.PTeamRollMelon(rollawarditems, roleids, battle.getBattleid(), null).call(); //执行ROLL点，作者 changhao
		}		
	}
