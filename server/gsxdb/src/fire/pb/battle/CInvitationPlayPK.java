	...
	protected void process() {
		...
		if(hostTeam!=null){
			...
			for (Long mem : hostteamMembers) {
				...
				if (role.getLevel() < CSendInvitePlayPK.PVP_LEVEL) {
					fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 162001 ,Arrays.<String>asList(CSendInvitePlayPK.PVP_LEVEL+""));
					sendremoveTickTime(hostid);//通知客户端取消定时器
					return ;
				}
			}
		...
		if(guestteam!=null){
			...
			for (Long mem : guestteamMembers) {
				PropRole role = new PropRole(mem, true);
				if (role.getLevel() < CSendInvitePlayPK.PVP_LEVEL) {
					fire.pb.talk.MessageMgr.sendMsgNotify(hostid, 160322 ,Arrays.<String>asList(CSendInvitePlayPK.PVP_LEVEL+""));
					sendremoveTickTime(hostid);//通知客户端取消定时器
					return ;
				}
			}
