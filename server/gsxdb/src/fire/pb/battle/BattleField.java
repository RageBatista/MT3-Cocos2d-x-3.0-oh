...
//	public static final int MAX_BATTLE_ROUND = 150;
	...
	public static int getRandTargetByType(final xbean.BattleInfo battle, final int fighterId, final int targetType, boolean examdead, boolean examhide, java.util.List<Integer> filterbuffids)
	{
		...
		for (int aim : aims)
		{
			...
			boolean ok = true;
			if (filterbuffids != null) //过滤掉掉 目标带有这些BUF的，作者 changhao
			{
...
/*		别的
	...
	public static xbean.Decision aiActionToDecision(final xbean.BattleInfo battle, final Fighter fighter, AIOperation action)
	{
		...
		if(action.getOperationType() == OperationType.ACTION_SKILL&& action.getOperationId() < 1 
				&& action.getSkillSoltID().size() > 0 && fighter.getFightertype() >= xbean.Fighter.FIGHTER_PARTNER) //伙伴，作者 changhao
		{
			int ncount = 0;
			int nSoltid = action.getSkillSoltID().get(0)-1;
			float rate = (float) Math.random();
			for(float nSoltodds:action.getSkillSoltOdds()) //技能槽释放概率，作者 changhao
			{
				if(rate < nSoltodds)
				{
		...
		else if (xfighter.getFightertype() >= xbean.Fighter.FIGHTER_PARTNER)
		{
			...
			if (xmonster != null)
			{
				...
				{// 首席弟子的怪造型要跟玩家一模一样
					...
					School school = fire.pb.school.SchoolManager.getSchoolByBattleNpcId(monsterid);
					if (school != null)
					{
				...
				if(WinnerManager.isWinnerFight(battle.getConfigid())){
					if(xfighter.getBattleindex()==20){
						//并且位置为20
						School school = fire.pb.school.SchoolManager.getSchoolByFighterConfigWinnerId(battle.getConfigid());
						if (school != null)
						{
							xbean.ProfessionLeaderInfo sxi = xtable.Professionleader.select(school.getValue());
							if (sxi != null){
	...
	public static final List<Monster> getMonstersFromBattleConfig(final int battleConfId)
	{
		...
		if (battleConfig.randomPos != null) //随机站位，作者 changhao
		{
	...
	public static void processFaileMonsters(xbean.BattleInfo battle, boolean includeGhost)
	{
		try{
			...
			ExceptionHandler.handleException(e, "BattleField.recordFailedMonster");
		}
	}
	...
	public static void ConvertBattlevideo(OctetsStream stream) throws MarshalException {
		...
		if (true) {
			...
			// 战斗开始人物
			int nfigterinfo = stream.unmarshal_int();
			...
			int nround = stream.unmarshal_int();
			for (int i = 0; i < nround; i++) {
				xbean.RoundResultItems rounditem = xbean.Pod.newRoundResultItems();
				int resultitemcount = stream.unmarshal_int();
				...
				int aiactioncount = stream.unmarshal_int();
	...
	/***
	 * 需要过滤掉身上有某个buf的目标，避免释放同一个控制技能到一个目标身上，作者 changhao
	 * @参数curskill
	 * @param buffids
	 * @返回
	 */
	...
	public static OctetsStream outBattlevideoProcess(xbean.BattleInfo battle) {
		...
		// 战斗开始人物
		stream.marshal(battle.getFigterinfoonbegin().size());
		for (FighterInfo f : battle.getFigterinfoonbegin())
			f.marshal(stream);

		stream.marshal(battle.getRound());
		for (int i = 0; i < battle.getRound(); i++) {
			...
			if (roundrsultitem != null) {
				int resultitemcount = roundrsultitem.getResultitems().size();
				...
				int aiactioncount = roundrsultitem.getAiactions().size();
...
/*		尝试 {
			...
			// TODO 自动生成的 catch 块
			ExceptionHandler.handleException(e, "BattleField.debugVideo");
		}*/
		return stream;
	}
