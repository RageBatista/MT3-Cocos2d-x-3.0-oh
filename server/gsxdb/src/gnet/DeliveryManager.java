...
public class DeliveryManager extends Manager {
	...
	protected void addMkio(mkio.Mkio mkio) {
		...
		lock.lock();
		try {
			// Delivery service is single-instance, new connection replaces old one
			if (gdelivery != null) {
				logger.warn("Replacing existing delivery connection. This may indicate a connection issue.");
			}
			gdelivery = mkio;
