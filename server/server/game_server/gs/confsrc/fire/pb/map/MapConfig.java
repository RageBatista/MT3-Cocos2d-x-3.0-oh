...
public class MapConfig implements mytools.ConvMain.Checkable ,Comparable<MapConfig>{
	...
	public MapConfig(MapConfig arg){
		...
		this.needvip=arg.needvip ;
		this.isteam=arg.isteam ;
		this.isreturn=arg.isreturn ;
	}
	public void checkValid(java.util.Map<String,java.util.Map<Integer,? extends Object> > objs){
			do{
				int tmprefvalue=id;
				
				if(tmprefvalue < 1) throw new RuntimeException("MapConfig.id="+tmprefvalue+",所以不满足条件 MapConfig.id < 1");
			}while(false);
	}
	/**
	 * id
