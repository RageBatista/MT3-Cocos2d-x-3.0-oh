...
BIN_PATH="${BIN_PATH:-./proxyserver}"
CONF_FILE="${CONF_FILE:-proxy.conf}"
...
SERVICE_PORTS="${SERVICE_PORTS:-$((ZONE_NUMBER + 44000)) $((ZONE_NUMBER + 45000))}"
...
process_matches_service() {
    ...
    [ -n "$cmdline" ] || return 1
    echo "$cmdline" | grep -Eq '(^|/| )proxyserver( |$)|proxy\.conf'
}
