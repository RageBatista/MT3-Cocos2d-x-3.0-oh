...
namespace Nuclear
{
	...
	bool ComponentSprite::SetModel(const std::wstring &modelname, bool async)
	{
		...
		if (modelname != L"")
		{
			...
			m_vShaderType.assign(m_nLayerCount, 0);
			...
			for (int k = 0; k < ani->GetRegionCount(); ++k)
			{
				if (realRender)
				{
					if (ani->GetPic(layerParam.dir, layerParam.linkframe, k, pic) && pic.pBigPic && pic.pBigPic->handle != INVALID_PICTURE_HANDLE)
					{
						...
						bool hasPartTexture = false;
						if (pic.pBigPicPart && pic.pBigPicPart->loaded && pic.pBigPicPart->handle
							&& pRenderer->IsTextureExist(pic.pBigPicPart->handle))
						{
							hasPartTexture = true;
						...
						if (param.iShaderType == 3 && !hasPartTexture)
						{
							// type=2 染色模型在缺少分区贴图时必须回退普通渲染，避免异常发白/虚化。
							param.iShaderType = 0;
						}
						...
						if (!pRenderer->DrawPicture(param))
						{
							...
							if (pic.pBigPic && pic.pBigPic->handle && pic.pBigPic->loaded && !pRenderer->IsTextureExist(pic.pBigPic->handle))
							{
								needReload = true;
							}
							if (pic.pBigPicPart && pic.pBigPicPart->handle && pic.pBigPicPart->loaded && !pRenderer->IsTextureExist(pic.pBigPicPart->handle))
							{
								needReload = true;
							}
							if (needReload)
							{
								...
								bool partReady = pic.pBigPicPart && pic.pBigPicPart->loaded && pic.pBigPicPart->handle
									&& pRenderer->IsTextureExist(pic.pBigPicPart->handle);
								param.handle = pic.pBigPic ? pic.pBigPic->handle : INVALID_PICTURE_HANDLE;
								param.handlePart = partReady ? pic.pBigPicPart->handle : INVALID_PICTURE_HANDLE;
								if (param.iShaderType == 3 && !partReady)
								{
									param.iShaderType = 0;
								}
	...
	void ComponentSprite::SetDyePartIndex(int level, int part, int index)
	{
		...
		if (level < 0 || level >= (int)m_vShaderType.size()) return;
		if (part < 0 || part >= m_iPartCount) return;
		std::vector<float> defaultParam;
		defaultParam.push_back(0.0f);
		defaultParam.push_back(0.0f);
		defaultParam.push_back(2.0f);
		defaultParam.push_back(3.0f);
		defaultParam.push_back(4.0f);
		defaultParam.push_back(0.0f);
		defaultParam.push_back(0.0f);
		if (part > 0 && m_bDyeEnable <= 1)
		{
			m_vShaderType[level] &= ~(part + 1);
			SetPartParam(level, part, defaultParam);
			return;
		}
		if (index > 0)
		{
		...
		if (m_vShaderType[level] != 0 && index > 0)
		{
			...
			if (pModel && pModel->GetDyeParam(part, index - 1, dinfo) == true)
			{
			...
			{
				// 索引越界或配置缺失时回滚该分区的 shader 状态，避免残留参数导致异常渲染。
				m_vShaderType[level] &= ~(part + 1);
				SetPartParam(level, part, defaultParam);
			}
		}
