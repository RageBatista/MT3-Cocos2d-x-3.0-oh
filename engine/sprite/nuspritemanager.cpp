...
namespace Nuclear
{
	namespace
	{
		const std::wstring kFashionModelPrefix = L"fashion-";
		const std::wstring kRoleModelPrefix = L"role-";
		const std::wstring kWeaponLayerName = L"weapon";

		bool IsFashionModelName(const std::wstring& modelName)
		{
			return modelName.size() > kFashionModelPrefix.size()
				&& modelName.compare(0, kFashionModelPrefix.size(), kFashionModelPrefix) == 0;
		}

		bool IsWeaponLayerName(const std::wstring& layerName)
		{
			return layerName == kWeaponLayerName;
		}

		std::wstring ResolveWeaponSourceModel(const std::wstring& modelName)
		{
			if (!IsFashionModelName(modelName))
			{
				return modelName;
			}

			std::wstring roleName = modelName.substr(kFashionModelPrefix.size());
			while (!roleName.empty() && iswdigit(roleName[roleName.size() - 1]))
			{
				roleName.erase(roleName.size() - 1);
			}
			...
			const std::wstring candidateModelName = kRoleModelPrefix + roleName;
			const std::wstring layerConfigPath = L"/model/" + candidateModelName + L"/layerdef.lmx";
			if (!LJFM::LJFMOpen::IsFileExisting(layerConfigPath))
			{
				return modelName;
			}
		...
		bool IsOptionalFashionWeaponLayer(const std::wstring& modelName, const std::wstring& layerName)
		{
			return IsFashionModelName(modelName) && IsWeaponLayerName(layerName);
		}

		std::wstring ResolveWeaponAniUri(SpriteManager* spriteManager,
			const std::wstring& modelName,
			int layer,
			const std::wstring& componentName,
			const std::wstring& actionName)
		{
			const std::wstring layerName = spriteManager->GetLayerName(modelName, layer);
			const std::wstring sourceModelName = IsWeaponLayerName(layerName)
				? ResolveWeaponSourceModel(modelName)
				: modelName;

			return L"/model/" + sourceModelName + L"/" + layerName
				+ L"/" + componentName + L"/" + actionName + SFI_ExtName;
		}
		bool ShouldSkipActionForLayer(SpriteManager* spriteManager,
			const std::wstring& modelName,
			int layer,
			const std::wstring& actionName)
		{
			...
			const bool isRidingAction = actionName.find(L"riding_") == 0;
			if (spriteManager->GetLayerRideEnable(modelName, layer))
			{
				return !isRidingAction;
			}

			const std::wstring layerName = spriteManager->GetLayerName(modelName, layer);
			return IsWeaponLayerName(layerName) && isRidingAction;
		}
	}
	...
	bool SpriteManager::XModel::LoadBase()
	{
		...
		for (int i = 0; i < GetLayerCount(); ++i)
		{
			...
			const std::wstring layerName = GetLayerName(i);
			const bool optionalFashionWeaponLayer = IsOptionalFashionWeaponLayer(m_name, layerName);
			std::wstring xmlpath = L"";
			if (GetLayerRideEnable(i))
			{
				xmlpath = L"/model/mt_zuoqi/" + layerName + L"/" + layerName + L".lmx";
			}
			else
			{
				xmlpath = L"/model/" + m_name + L"/" + layerName + L"/" + layerName + L".lmx";
			}
			char* fileBuf = NULL;
			std::streamsize ss = 0;
			if (GetBuffFromFile(xmlpath, fileBuf, ss) != 0)
			{
				if (optionalFashionWeaponLayer)
				{
					continue;
				}
				return false;
	...
	NuclearPrefetchResult Component::PrefetchAction(const std::wstring &modelname, const std::wstring &action_name, const ActionNameSet &linked_action_names, unsigned char bDye)
	{
		...
		for (; it != ie; ++it)
		{
			...
			if ((!hardref.IsValid()) || hardref->GetState() == XAni::XPANISTATE_NOPICTURE)
			{
				...
				{
					if (ShouldSkipActionForLayer(m_pEB->GetSpriteManager(), modelname, m_layer, *it))
					{
						continue;
					}
				...
				else
				{
					...
					uri = ResolveWeaponAniUri(
						m_pEB->GetSpriteManager(),
						modelname,
						m_layer,
						(m_linkname.empty() ? m_name : m_linkname),
						*it);
		...
		if (PrefetchAction(modelname, action_name, linked_action_names, bDye) == XPPR_OK)
		{
			if (info.first == 1)//第一次Hold
			{
				...
				for (; it != ie; ++it)
				{
					...
					AniMap::iterator aniIt = m_anis.find(*it);
					if (aniIt != m_anis.end())
					{
						hardref = aniIt->second;
					}
					if (!hardref.IsValid())
					{
						if (ShouldSkipActionForLayer(m_pEB->GetSpriteManager(), modelname, m_layer, *it))
						...
						NuclearWeakRef<XAni> weakani = GetAni(modelname, *it, false, bDye);
						hardref = weakani;
					}
					if (!hardref.IsValid())
					{
						XPLOG_ERROR(L"Component::HoldAction skip invalid ani model=%s layer=%d component=%s link=%s action=%s hold=%s\n",
							modelname.c_str(),
							m_layer,
							m_name.c_str(),
							m_linkname.c_str(),
							(*it).c_str(),
							action_name.c_str());
						XPTRACE(L"Component::HoldAction skip invalid ani model=%s layer=%d component=%s link=%s action=%s hold=%s\n",
							modelname.c_str(),
							m_layer,
							m_name.c_str(),
							m_linkname.c_str(),
	...
	NuclearWeakRef<XAni> Component::GetAni(const std::wstring &modelname, const std::wstring &actname, bool async, unsigned char bDye)
	{
		...
		if (!hardref.IsValid() || hardref->GetState() == XAni::XPANISTATE_NO_XAP)
		{
			...
			if (m_pEB->GetSpriteManager()->GetLayerRideEnable(modelname, m_layer))
			{
				if (m_needDraw == -1)
				{
					if (ShouldSkipActionForLayer(m_pEB->GetSpriteManager(), modelname, m_layer, actname))
					{
						m_needDraw = 0;
						return weakani;
					}
	...
	Component* SpriteManager::GetComponent(const std::wstring &modelname, int ectype, const std::wstring &name)
	{
		...
		const std::wstring layerName = it->second->GetLayerName(ectype);
		if (IsWeaponLayerName(layerName))
		{
			const std::wstring sourceModelName = ResolveWeaponSourceModel(modelname);
			ModelMap::iterator sourceIt = m_models.find(sourceModelName);
			if (sourceIt != m_models.end())
			{
				return sourceIt->second->GetComponent(ectype, name);
			}
