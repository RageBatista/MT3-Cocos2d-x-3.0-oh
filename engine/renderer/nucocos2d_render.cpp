...
#define GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT 0x84FF
	...
	namespace
	{
		...
		bool shouldUseMapMipMap(const std::wstring& fileuri)
		{
			return fileuri.find(L"/map/") != std::wstring::npos
				|| fileuri.find(L"\\map\\") != std::wstring::npos;
		}

		float getPreferredAnisotropyLevel()
		{
			...
			if (ext == NULL || std::strstr(reinterpret_cast<const char*>(ext), "GL_EXT_texture_filter_anisotropic") == NULL)
			{
				return s_value;
			}
			...
			// 4x 已能显著改善斜向地砖清晰度，避免盲目开到硬件上限带来的带宽压力。
			s_value = maxAniso > 4.0f ? 4.0f : maxAniso;
			return s_value;
		}

		void applyPictureTextureSampling(cocos2d::Texture2D* texture, const std::wstring& fileuri)
		{
			if (texture == NULL)
			{
				return;
			...
			if (shouldUseMapMipMap(fileuri)
				&& isPowerOfTwo(texture->getPixelsWide())
				&& isPowerOfTwo(texture->getPixelsHigh()))
			{
				...
				const float anisotropy = getPreferredAnisotropyLevel();
				if (anisotropy > 1.0f)
				{
    ...
    Cocos2dRenderTarget* Cocos2dRenderTarget::CreateRenderTarget(int aIWidth, int aIHeight, NuclearTextureFormat aTFormat)
    {
        ...
        cocos2d::Texture2D::PixelFormat fmt;
        switch (aTFormat) {
        ...
        cocos2d::RenderTexture* pRT = cocos2d::RenderTexture::create(aIWidth, aIHeight, fmt, 0);        
    ...
    NuclearDisplayMode Cocos2dRenderer::GetDisplayMode() const
	{
        ...
        dismd.width = GetEngine()->m_adapter->get_scene_render_w();
        dismd.height = GetEngine()->m_adapter->get_scene_render_h();
        dismd.bitperpix = 32;
	...
	void Cocos2dRenderer::GetAvailableMultiSampleType(bool window, bool isMode16, std::vector<NuclearMultiSampleType> &types)
	{
		...
		(void)isMode16;
		types.clear();
		types.push_back(XPMULTISAMPLE_NONE);
	}
	...
	Cocos2dRenderer::~Cocos2dRenderer()
	{
		...
		for (; it != m_mapPictures.end(); ++it)
		{
			...
			for (int i = 0; i < cnt; ++i)
			{
				cocos2d::Texture2D* ifTexture = it->second.m_CTextureInfoVector[i].m_pTexture;
				delete ifTexture;
			}
		}
		m_mapPictures.clear();
	...
	bool Cocos2dRenderer::DrawBox(const NuclearFRectt &rect, NuclearColor color, NuclearFillMode fillmode, const NuclearMatrix3 *pMatrix)
	{
		...
		cocos2d::ShaderCache::getInstance()->pushShader(kCCShader_PositionColor);
	...
	bool Cocos2dRenderer::SetPictureData(PictureHandle handle, const void *data, int size, const NuclearRect* pRect)
	{
		...
		for(int i=0; i<n; i++)
		{
           ...
           
           {
                ...
                applyPictureTextureSampling(texInfo.m_pTexture, it->second.m_info.fileuri);
                delete[] buf;
                return true;
            }
		}
	...
	PictureHandle Cocos2dRenderer::LoadPictureFromNativePath(const std::string &nativepath)
    {
		...
		int64_t tick = Nuclear::GetMilliSeconds();
#endif
		cocos2d::Texture2D* d_texture = cocos2d::Director::getInstance()->getTextureCache()->addImage(nativepath.c_str());
        ...
        if (it != m_mapPictures.end()) {
            ...
            switch (pi.m_info.m_texfmt) {
                ...
                case XPIFF_PNG:
                    ...
                    break;
                case XPIFF_TGA:
                    ccfmt = cocos2d::Image::Format::TGA;
                    break;
                default:
                    ccfmt = cocos2d::Image::Format::PNG;
            }
            cocos2d::VolatileTextureMgr::addImageTexture(ti.m_pTexture, ws2s(filename).c_str(), ccfmt);
        }
#endif   
#endif        
    }
...
//===========================================
...
#if CC_ENABLE_CACHE_TEXTURE_DATA
#ifdef TEST_CACHE_MEM
            cocos2d::VolatileTextureMgr::addETCImage(d_texture,data,size,0);
#endif
#endif
#ifdef XP_PERFORMANCE
		...
		if (CC_SWAP_INT32_BIG_TO_HOST(header->version) == 0x50565203)
		{
			...
			d_texture->initWithPVRTCData(((unsigned char *)data) + (sizeof(ccPVRv3TexHeader) + header->metadataLength), 0, fileinfo.ImageFileFormat == XPIFF_PVR4 ? 4 : 2, true, CC_SWAP_INT32_LITTLE_TO_HOST(header->width), fileinfo.ImageFileFormat == XPIFF_PVR4 ? cocos2d::Texture2D::PixelFormat::PVRTC4 : cocos2d::Texture2D::PixelFormat::PVRTC2);
			...
			{
				cocos2d::VolatileTextureMgr::addPVRTCImage(d_texture, data, size);
			}
#endif       
#endif
        ...
        if (pAH->sig == 0x11111111)
		{
            ...
            d_texture = new cocos2d::Texture2D();
            d_texture->initWithATCData(((unsigned char *)data) + 4*sizeof(unsigned int), 0, 8, true, pAH->width, pAH->height,cocos2d::Texture2D::PixelFormat::ATC_EXPLICIT_ALPHA);
            CTextureInfo TextureInfo;
            CPicInfo PicInfo;
...
#if CC_ENABLE_CACHE_TEXTURE_DATA
 #ifdef TEST_CACHE_MEM            
            cocos2d::VolatileTextureMgr::addAtcImage(d_texture,data,size );
#endif
#endif
#ifdef XP_PERFORMANCE
			int deltaT = int(Nuclear::GetMilliSeconds() - tick);
...
#endif
        cocos2d::Texture2D* d_texture = NULL;
        cocos2d::Image::Format ccfmt = cocos2d::Image::Format::PNG;
        switch (fileinfo.ImageFileFormat)
		{
            ...
            case XPIFF_TGA:
				...
				break;
			case XPIFF_WEBP:
				ccfmt = cocos2d::Image::Format::WEBP;
				break;
            default:
                ccfmt = cocos2d::Image::Format::PNG;
        }
        ...
        if(image->initWithImageData((const unsigned char*)data, size))
        {
            ...
            if(image->hasAlpha())
            {
                pixfmt = cocos2d::Texture2D::PixelFormat::RGBA8888;
            }else
            {
                pixfmt = cocos2d::Texture2D::PixelFormat::RGB888;
            }
 ...
 #ifdef TEST_CACHE_MEM  
            if (bCache) {
                cocos2d::VolatileTextureMgr::addCCImage(d_texture, image);
                image->release();
            }      
#endif
#endif      
	...
	bool Cocos2dRenderer::LoadCCImageFromMem(cocos2d::Image* pImage, NuclearTextureFormat texfmt, const void *data, int size)
	{
		if (!pImage)
		{
			return false;
	...
	PictureHandle Cocos2dRenderer::LoadPictureFromCCImage(cocos2d::Image* image, NuclearTextureFormat texfmt, NuclearPictureInfo *pPicInfo/* = NULL*/, NuclearPoolType pooltype/* = XPPOOL_MANAGED*/, bool bCache/* = true*/, PictureHandle handle/* = 0*/)
	{
		...
		cocos2d::Texture2D* d_texture = new cocos2d::Texture2D();
	...
	void Cocos2dRenderer::ResetAllTextureUName()
    {
        ...
        for (; it != m_mapPictures.end(); it++) {
            ...
            for (; tx != textureVec->end(); tx++) {
                cocos2d::Texture2D* texture = tx->m_pTexture;
	...
	bool Cocos2dRenderer::DrawPicture(const DrawPictureParam& param)
	{
		...
		if (param.handlePart > 0)
		{
			...
			// 分区贴图在高负载场景下可能尚未就绪或被回收，此时降级为主贴图渲染，避免整层丢失。
			if (itPart == m_mapPictures.end())
				itPart = m_mapPictures.end();
		}
