...
#ifdef ANDROID
#include <android/log.h>
#define MT3_ENGINE_TRACE(...) __android_log_print(ANDROID_LOG_ERROR, "MT3Trace", __VA_ARGS__)
#elif defined(WIN32) && (defined(_DEBUG) || defined(DEBUG))
static void MT3Win32EngineTraceToFile(const char* fmt, ...)
{
	FILE* fp = NULL;
...
#define MT3_ENGINE_TRACE(...) MT3Win32EngineTraceToFile(__VA_ARGS__)
#else
#define MT3_ENGINE_TRACE(...)
#endif
...
XPLOG_INFO(L"  动画管理器初始化成功\n");
	...
	bool Engine::Run(const EngineParameter &ep) 
	{
...
#endif
		MT3_ENGINE_TRACE("Engine::Run enter app=%p initCount=%d", ep.pApp, ep.nAppInitStepCount);
		...
		{
			MT3_ENGINE_TRACE("Engine::Run FileIO init failed");
...
#ifndef WIN7_32
		MT3_ENGINE_TRACE("Engine::Run return true");
		return true;
#endif

		MT3_ENGINE_TRACE("Engine::Run return Exit()");
		return Exit();
	}	
	...
	void Engine::OnIdle() 
	{ 
		static int sOnIdleCount = 0;
		++sOnIdleCount;
		if (sOnIdleCount <= 20 || sOnIdleCount == 60 || sOnIdleCount == 180 || sOnIdleCount == 600)
		{
			MT3_ENGINE_TRACE("Engine::OnIdle #%d state=%d step=%d appInit=%d bg=%d", sOnIdleCount, m_InitState, m_nInitStep, m_nAppInitCount, gIsInBackground() ? 1 : 0);
		}
		if (m_InitState < 2 && gIsInBackground())
		{
			MT3_ENGINE_TRACE("Engine::OnIdle skip background state=%d step=%d", m_InitState, m_nInitStep);
			return;
		}
            ...
            case 0:
                {
                    ...
                    int initStep = m_nInitStep;
                    MT3_ENGINE_TRACE("Engine::Init step %d begin", initStep);
                    if (!Init(m_nInitStep++))
                    {
                        MT3_ENGINE_TRACE("Engine::Init step %d fail nextStep=%d", initStep, m_nInitStep);
                        sprintf(buffer,"Engine_OnIdle_InitState_%d_InitStep_%d_fail",m_InitState,m_nInitStep);
                    }
                    else
                    {
                        MT3_ENGINE_TRACE("Engine::Init step %d success state=%d nextStep=%d sprStep=%d", initStep, m_InitState, m_nInitStep, m_nSprStepCount);
                    }
		...
		case 1:
			if (m_nInitStep < m_nAppInitCount)
			{
				...
				MT3_ENGINE_TRACE("Engine::AppOnInit step %d/%d begin", m_nInitStep, m_nAppInitCount);
				int appInitStep = m_nInitStep;
				if (!m_pApp->OnInit(m_nInitStep++))
				{
					MT3_ENGINE_TRACE("Engine::AppOnInit step %d fail nextStep=%d", appInitStep, m_nInitStep);
                    sprintf(buffer, "Engine_OnIdle_InitState_%d_InitStep_%d_fail",m_InitState,m_nInitStep);
                    assert(false);
				}
				MT3_ENGINE_TRACE("Engine::AppOnInit step %d success nextStep=%d", appInitStep, m_nInitStep);
                std::wstring wsw = s2ws(buffer);
                MHSD_UTILS::flurryEvent(wsw);
				break;
			}
			MT3_ENGINE_TRACE("Engine::OnIdle init finished state 1 -> 2");
			m_InitState = 2;
			//不用break了
		case 2:
			if (sOnIdleCount <= 20 || sOnIdleCount == 60 || sOnIdleCount == 180 || sOnIdleCount == 600)
			{
				MT3_ENGINE_TRACE("Engine::MainLoop begin onIdle=%d", sOnIdleCount);
			}
			MainLoop();
			break;
		}
	...
	{
		if (m_adapter) {
			return m_adapter->get_ui_logic_w();
		}
	...
	{
		if (m_adapter) {
			return m_adapter->get_ui_logic_h();
		}
