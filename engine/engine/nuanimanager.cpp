...
static void MT3AniDiagTrace(const char* fmt, ...)
{
	char buffer[1024];
	va_list args;
	va_start(args, fmt);
...
#define MT3_DIAG_ANI_LOG(...) MT3AniDiagTrace(__VA_ARGS__)
#else
#define MT3_DIAG_ANI_LOG(...)
#endif
...
namespace Nuclear
{
	...
	{
#if defined(MT3_RUNTIME_DIAG_ENABLE)
		int diagBeforeTotal = (int)m_rsrcmap.size();
		int diagHardRefOne = 0;
		int diagStateNull = 0;
		int diagStateNoXap = 0;
		int diagStateNoPicture = 0;
		int diagStateBasePic = 0;
		int diagStatePicture = 0;
		int diagFreePicByTime = 0;
		int diagFreePicImmediate = 0;
		int diagErase = 0;
		for (ResourceMap::iterator statIt = m_rsrcmap.begin(), statIe = m_rsrcmap.end(); statIt != statIe; ++statIt)
		{
			NuclearHardRef<XAni>& statRef = *statIt->second;
			if (statRef.GetHardRefCount() == 1)
			{
				++diagHardRefOne;
			}
			switch (statRef->GetState())
			{
		...
		MT3_DIAG_ANI_LOG("MT3_DIAG_ANI event=gc now=%d before=%d after=%d hardRefOne=%d stateNull=%d stateNoXap=%d stateNoPicture=%d stateBasePic=%d statePicture=%d freePicByTime=%d freePicImmediate=%d erased=%d",
			nowtick,
			diagBeforeTotal,
			(int)m_rsrcmap.size(),
			diagHardRefOne,
	...
	void AniManager::GarbageCollectionAll(bool bImmediately)
	{
		...
		MT3_DIAG_ANI_LOG("MT3_DIAG_ANI event=gc_all immediate=%d before=%d after=%d erased=%d",
			bImmediately ? 1 : 0,
			diagBeforeTotal,
			(int)m_rsrcmap.size(),
			diagErase);
