...
#ifdef IGNORE_EXPORT
#undef IGNORE_EXPORT
#define NUCLEAR_RESTORE_IGNORE_EXPORT 1
#endif
#include "SimpleAudioEngine.h"
#ifdef NUCLEAR_RESTORE_IGNORE_EXPORT
#define IGNORE_EXPORT
#undef NUCLEAR_RESTORE_IGNORE_EXPORT
#endif
...
namespace Nuclear
{
	...
	do{\
		point.x *= scale;\
		point.y *= scale;\
		point.x *= ((float)pEngine->m_adapter->get_ui_logic_w()/(float)pEngine->m_adapter->get_display_w());\
		point.y *= ((float)pEngine->m_adapter->get_ui_logic_h()/(float)pEngine->m_adapter->get_display_h());\
    ...
    void EngineLayer::draw(void)
    {
        ...
        float rh = pEngine->m_adapter->get_scene_render_h();
        float rw = pEngine->m_adapter->get_scene_render_w();
        if (rw <= 0.f || rh <= 0.f)
        {
            rw = lw;
            rh = lh;
        ...
        // Keep camera center in logic space to avoid world clipping/drift when scene render density is higher.
        float midx = lw*0.5f;
        float midy = lh*0.5f;
