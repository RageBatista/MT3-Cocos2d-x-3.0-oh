...

{
	...
	// Keep visual baseline aligned with packaged defaults when frameSize.txt is missing.
	int w = 1280;
	int h = 720;
	...
	if (fp)
	{
		...
		char *context = NULL;
		const char *delims = "\xEF\xBB\xBF xX,\t\r\n";
		int parsedW = 0;
		int parsedH = 0;
		char *p = strtok_s(buffer, delims, &context);
		if (p != NULL)
		{
			parsedW = atoi(p);
			p = strtok_s(NULL, delims, &context);
			if (p != NULL)
			{
				parsedH = atoi(p);
			}
	...
	if (!bIsPoint)
	{
		...
		SendMessage(glfwGetWin32Window(eglView->getWindow()), WM_SETICON, TRUE, (LPARAM)icon_dk);//��ͼ��
	}
	else
	{
		HICON icon_free = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON2));
		SendMessage(glfwGetWin32Window(eglView->getWindow()), WM_SETICON, TRUE, (LPARAM)icon_free);//��ͼ��
	}
