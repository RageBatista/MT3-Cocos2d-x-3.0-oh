...
const char* DeviceInfo::sGetOSVersion()
{
    ...
    DeviceInfo::s_strOSVersion = std::string([version UTF8String]);
...
{
    NSString *model = [IOSDeviceInfo getSysInfoByName:"hw.machine"];
    DeviceInfo::s_strProductModel = std::string([model UTF8String]);
...
const char* DeviceInfo::sGetDeviceID()
{
    ...
    DeviceInfo::s_strDeviceID = std::string([identifierForVendor UTF8String]);
...
const char* DeviceInfo::sGetProvider()
{
    ...
    if (carrier) {
        ...
        if (carrierName) {
            DeviceInfo::s_strProvider = std::string([carrierName UTF8String]);
        }
    }
...
void MT3SystemInfo::sSendSystemInfo()
{
    ...
    mapSystemInfo.insert(std::map<std::wstring, std::wstring>::value_type(L"PlatType", SHARE_String2Wstring("2")));
