...
void ComponentsObject::SetSpriteComponent(eSpriteComponentType type,int baseid,bool forcechange)
{
	...
	{
		oldHorseID = GetSpriteComponent(eSprite_Horse);
	}
	...
	if (type == eSprite_Horse)
	{
		...
		if (GetSprite() && oldHorseID != GetSpriteComponent(eSprite_Horse))
		{
...
{
	ComponentInfoMap::const_iterator horseIt = m_ComponentInfo.find(eSprite_Horse);
	if (horseIt != m_ComponentInfo.end() && horseIt->second != 0)
		return gGetActionName(type,m_iActionID,true,horseIt->second);
