	...
	int ct_type, const std::string& gip, const std::string& gport, const std::string& extparam )
{
	...
	param.mStrChildChannel = extparam;
	{
		...
		const std::wstring wHost = StringCover::to_wstring(host);
		const std::wstring wPort = StringCover::to_wstring(port);
		SDLOG_INFO(L"[LoginFlow] NetConnection ctor userLen=%d hasComma=%d host=%ls port=%ls serverid=%d loginType=%d connectType=%d channelLen=%d extLen=%d",
			static_cast<int>(user.length()), user.find(",") != std::string::npos ? 1 : 0,
			wHost.c_str(), wPort.c_str(), serverid, type, ct_type, channelLen, static_cast<int>(extparam.length()));
	}
	...
	m_login = FireNet::GetNetSystem()->CreateConnector(param, this);
	m_login->Start();
	SDLOG_INFO(L"[LoginFlow] NetConnection started loginPtr=%d", m_login ? 1 : 0);
}
...
{
	SDLOG_INFO(L"[LoginFlow] OnAutoSuccess userid=%u zoneid=%d aid=%d algorithm=%d accountLen=%d",
		userid, zoneid, aid, algorithm, static_cast<int>(m_sAccount.length()));
