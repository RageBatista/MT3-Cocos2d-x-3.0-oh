...
namespace MT3
{
	...
	public:
		...
		virtual void onLoginSuccess(SFNativeOnlineUser* user, const char* remain) {
			...
			{
#ifdef ANDROID
				LOGE("SFNativeOnlineLoginCallbackImpl onLoginSuccess user is null");
#endif
				return;
			}
			const char* productCode = user->productCode != NULL ? user->productCode : "";
			const char* channelId = user->channelId != NULL ? user->channelId : "";
			std::string encodedChannelUserId = MT3::ChannelManager::urlencode(user->channelUserId != NULL ? user->channelUserId : "");
			std::string encodedToken = MT3::ChannelManager::urlencode(user->token != NULL ? user->token : "");
#ifdef ANDROID
			LOGE("SFNativeOnlineLoginCallbackImpl onLoginSuccess productCode=%s,channelId=%s,channelUserId=%s,tokenLen=%d",
				productCode, channelId, encodedChannelUserId.c_str(), static_cast<int>(encodedToken.length()));
			...
			// {
				...
				gGetLoginManager()->SetSessionKey(s2ws(encodedToken));
				char aAccount[128] = "";
				snprintf(aAccount, sizeof(aAccount), "%s,%s", encodedChannelUserId.c_str(), strMacAddress.c_str());
				...
				gGetLoginManager()->SetYJAppId(s2ws(productCode));
				gGetLoginManager()->SetYJChannelId(s2ws(channelId));
				gGetLoginManager()->SetYJUserId(s2ws(encodedChannelUserId));
				gGetLoginManager()->SetYJToken(s2ws(encodedToken));
	...
	int ChannelManager::StartBuyYuanbao(int64_t sn, std::wstring& name, int goodid, int goodnum, int price, int zoneid)
	{
        ...
        char cpinfo[512];
		snprintf(cpinfo, sizeof(cpinfo), "%d|%s|%lld|%s|%s|%s|",gGetLoginManager()->getServerID(), ws2s(gGetLoginManager()->GetChannelId()).c_str(), GetMainRoleDataNumValue<int64_t>("roleid"),StringCover::to_string(gGetLoginManager()->GetIp()).c_str(),StringCover::to_string(gGetLoginManager()->GetDeviceid()).c_str(),StringCover::int64_tToString(sn).c_str());
		std::string strFoodID = "";
		...
		char aserverid[128];
        snprintf(aserverid, sizeof(aserverid),"%d",gGetLoginManager()->getServerID());
...
#else
        char aprise[128];
		snprintf(aprise, sizeof(aprise), "%d", sellpricenum * 100);
		...
		char cbi[512];
		snprintf(cbi, sizeof(cbi), "%d|%s|%lld|%s|%s|88|%s|%s|",
	...
	std::string ChannelManager::getClipboardText()
	{
			...
			"()Ljava/lang/String;"))
		{
			...
			if (text != NULL)
			{
				...
				const char* chars = methodInfo.env->GetStringUTFChars(text, &isCopy);
				if (chars != NULL)
				{
					str = chars;
					methodInfo.env->ReleaseStringUTFChars(text, chars);
				}
				methodInfo.env->DeleteLocalRef(text);
			}
		}
	#endif
		return str;
