...
WinSDK::WinSDK()
{
	...
	m_strPlatformId = "";
	//m_strLoginUrl = "http://192.168.29.151:9001/ClientGame/Login.aspx?"; // �������Ե�ַ
	m_strLoginUrl = "http://billing.locojoy.com/api/nosdk/login/locojoy/ClientGameWeb.ashx?";
	//m_strChargeUrl = "http://192.168.29.151:9001/api/nosdk/pay/locojoy/ClientGameWeb.ashx?"; // �������Ե�ַ
...
void WinSDK::openLoginUrl()
{
	...
	snprintf(md5Data, sizeof(md5Data), "%s%s%s", MT3_GameKey, MT3_Version, strData.c_str());
...
void WinSDK::onLoginSuccess(std::string& platformid, std::string& session)
{
	...
	char aAccount[128] = "";
	snprintf(aAccount, sizeof(aAccount), "%s@@%s,%s", m_strPlatformId.c_str(), strChannel.c_str(), strMacAddress.c_str());

	gGetLoginManager()->SetSessionKey(StringCover::to_wstring(strSession));
	...
	if (gGetLoginManager()->getEnterMainStatus() == eEnterMainStatus_None)
		gGetLoginManager()->setEnterMainStatus(eEnterMainStatus_LoginSuccess); // SDK��½�ɹ�
	else {
		gGetLoginManager()->setEnterMainStatus(eEnterMainStatus_GamePlaying);
		cocos2d::CCScriptEngineManager::sharedManager()->getScriptEngine()->executeGlobalFunctionWithBooleanData("SelectServerEntry_EnableClick", true); // ����ѡ�����水ť�ɵ��
	}
