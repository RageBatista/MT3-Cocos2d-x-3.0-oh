...
void COleContainer::TrackBrowserEventConnection(IConnectionPoint* pConnectionPoint, DWORD dwCookie)
{
	ResetBrowserEventConnection();
	_pEventConnectionPoint = pConnectionPoint;
	_dwEventCookie = dwCookie;
}

void COleContainer::ResetBrowserEventConnection()
{
	if (_pEventConnectionPoint)
	{
		if (_dwEventCookie != 0)
		{
			_pEventConnectionPoint->Unadvise(_dwEventCookie);
			_dwEventCookie = 0;
		}
		_pEventConnectionPoint->Release();
		_pEventConnectionPoint = NULL;
	}
}
...
, m_bShowScroll(true)
{
	...
	if (SUCCEEDED(hrOle))
	{
		_bOleInitialized = true;
	}
    HRTEST_SE( hrOle,L"Ole初始化错误");
...
COleContainer::~COleContainer(void)
{
	...
	if (_pInPlaceObj)
	{
		_pInPlaceObj->InPlaceDeactivate();
