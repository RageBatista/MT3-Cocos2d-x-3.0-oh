...
void onLogined()
{
    ...
    loginManager->SetSessionKey(channelPlatformGetSessionId());

    LOGD("CAndroidLoginDialog::onLogined 2");
    LOGD("[SelectServerDiag] AndroidLoginDialog::onLogined before ToServerChoose areaLen=%d serverLen=%d",
        static_cast<int>(gGetLoginManager()->GetSelectArea().length()),
        static_cast<int>(gGetLoginManager()->GetSelectServer().length()));
