...
class LoginManager : public cocos2d::CCObject, public CSingleton<LoginManager>
{
public:
	LoginManager();
	~LoginManager();
public:
	...
	const std::wstring& GetSessionKey();
	const std::wstring& GetPassword(); // Deprecated: login session key compatibility wrapper.
	const std::wstring& GetChannelId();
	const std::wstring& GetCurChannelId();
	int GetHostType();
	void SetAccountInfo(const std::wstring& account);
	void SetSessionKey(const std::wstring& sessionKey);
	void SetPassword(const std::wstring& password); // Deprecated: login session key compatibility wrapper.
	...
	void RegisterAccount(const std::string& account, const std::string& password, const std::string& invitecode, const std::string& captcha);
    void LoginAgain();
    void SaveAccount();
	bool isAutoLogin();
    void Clear();
