...
#ifdef WIN7_32
	#ifdef NoPack
		return  SHARE_String2Wstring(CFileUtil::GetWin32LooseResourceRoot() + "/replay/");
	#else
		return  SHARE_String2Wstring(CFileUtil::GetWin32PackedResourceRoot() + "/replay/");
	#endif
#else
...
bool BattleReplayManager::UnCompressReplayFile()
{
	...
	//CopyFileByFS(m_wsDstFileNameFullCompress, m_wsDstFileNameFullUnCompress);//��ʱ����
