...
namespace
{
	const float c_select_effect_visual_base_size = 84.0f; // 11065 caihong visible alpha bbox.
	...
	class ScopedExternalRenderPass
	{
	public:
		explicit ScopedExternalRenderPass(CEGUI::Renderer* renderer)
			: m_renderer(renderer)
		{
			if (m_renderer)
			{
				m_renderer->SaveXPRenderState();
			}
		}

		~ScopedExternalRenderPass()
		{
			if (m_renderer)
			{
				m_renderer->RestoreXPRenderState();
			}
		}

	private:
		ScopedExternalRenderPass(const ScopedExternalRenderPass&);
		ScopedExternalRenderPass& operator=(const ScopedExternalRenderPass&);
	...
	if (pWnd != NULL)
	{
		...
		for (; it != m_mapUIEffect.end(); ++it)
		{
			if (it->second == pWnd)
			{
				...
				Nuclear::NuclearLocation sceneEffectPos = ConvertUIToSceneLocation(effectPos);
				printf("location x = %d, y = %d\n", sceneEffectPos.x, sceneEffectPos.y);
				pEffect->SetLocation(sceneEffectPos);
...
void GameUImanager::RenderWindowSprite(XPRenderEffect* pRenderEffect)
{
	if (pRenderEffect)
	{
		ScopedExternalRenderPass externalRenderPass(m_pCEGUICocos2DRender);
		...
		if (pRenderEffect->m_bClip)
		{
			glScissor(d_ScissorBox[0], d_ScissorBox[1], d_ScissorBox[2], d_ScissorBox[3]);
			if (scissorEnable)
				glEnable(GL_SCISSOR_TEST);
			else
...
void GameUImanager::UpdateEffectBoundsPos(Nuclear::IEffect* pEffect, CEGUI::Window* pWnd)
{
	...
	if (pEffect)
	{
		pEffect->SetLocation(ConvertUIToSceneLocation(Nuclear::NuclearLocation(x, y)));
	}
...
Nuclear::IEffect* GameUImanager::AddUIEffect(CEGUI::Window* pWnd, const std::wstring& strEffectName, bool bCycle, int x, int y, bool clip)
{
	if (pWnd != NULL)
	{
		...
		if (NULL != pEffect)
		{
			...
			pEffect->SetLocation(ConvertUIToSceneLocation(effectPos));
...
void HandleAddSelectEffectToItemCell(CEGUI::Window* wnd, bool isSelected)
{
	if (isSelected)
	{
		...
		float scale = wnd->getPixelSize().d_width / c_select_effect_visual_base_size;
...
void GameUImanager::DrawSysMsgEffect()
{
	...
	{
		ScopedExternalRenderPass externalRenderPass(CEGUI::System::getSingleton().getRenderer());
...
void GameUImanager::DrawFPS()
{
	...
	if (pDirector->isDisplayStats())
	{
		...
		snprintf(buffer, sizeof(buffer), "FPS:%.1f,Mem:%.2fM,Thread:%d", curFps, workSize, nThreadExcCounter);
...
void GameUImanager::Draw()
{
	...
	{
		m_pCEGUICocos2DRender->ProcessPendingTextures();
	}
...
void GameUImanager::DrawPerformance()
{
	...
	snprintf(buffer, sizeof(buffer), "ManagedTextureCount : %d", rp.m_iManagedTextureCount);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffff00);

	snprintf(buffer, sizeof(buffer), "ManagedTextureMemory : %d M", rp.m_iManagedTextureMemory / (1024 * 1024));
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	...
	snprintf(buffer, sizeof(buffer), "AverageTextureLayer : %d, %d", iTmp1, iTmp2);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffeebb00);
	...
	snprintf(buffer, sizeof(buffer), "AverageReadSpeed  : %d.%d M/s", iTmp1, iTmp2);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffeebb00);

	snprintf(buffer, sizeof(buffer), "DrawPrimitiveUp : %d	/Frame", rp.m_iDrawPrimitiveUPCountPerFrame);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffff00);

	snprintf(buffer, sizeof(buffer), "DrawPrimitive : %d  /Frame", rp.m_iDrawPrimitiveCountPerFrame);
	trect.top += fheight;
	trect.bottom += fheight;
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffff00);

	snprintf(buffer, sizeof(buffer), "DrawPicture : %d  /Frame", rp.m_iDrawPictureCountPerFrame);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffff00);

	snprintf(buffer, sizeof(buffer), "TotalActiveParticles  : %d", pRenderer->GetParticleManager()->GetTotalNumActiveParticles());
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffff00);
	...
	snprintf(buffer, sizeof(buffer), "TotalLoadTime : %d	/Frame", sTotalLoadTimePerFram);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xff0000ff);

	snprintf(buffer, sizeof(buffer), "LoadXmlTime : %d /Frame", sLoadXmlTimePerFrame);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xff00ff00);

	snprintf(buffer, sizeof(buffer), "LoadTextureTime : %d /Frame", sLoadTextureTimePerFrame);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffff0000);

	snprintf(buffer, sizeof(buffer), "GameUpdateTime : %d ms/s", rp.m_iUpdateTimePerSecond);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffffff);

	snprintf(buffer, sizeof(buffer), "GameTicketsTime : %d ms/s", rp.m_iGameTickTimePerSecond);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffffff);

	snprintf(buffer, sizeof(buffer), "LuaTicketsTime : %d ms/s", rp.m_iLuaTickTimePerSecond);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffffff);

	snprintf(buffer, sizeof(buffer), "GameRenderTime : %d ms/s", rp.m_iRenderTimePerSecond);
	trect.top += fheight;
	trect.bottom += fheight;
	outText = s2ws(buffer);
	pRenderer->DrawText(outText.c_str(), strlen(buffer), &trect, 0xffffffff);
...
void GameUImanager::DrawUIState()
{
	...
	if (gGetGameApplication()->IsShowUIState())
	{
		...
		char buffer[256];
		snprintf(buffer, sizeof(buffer), "layout count =%d window count=%d last window =%s", stats.size(), windowCount, lastWindowName);
		...
		while (itStat != stats.end())
		{
			...
			if (wnd)//&&wnd->isVisible(false))
			{
				...
				if (time < 1000)//小于1�?
				{
					snprintf(buffer, sizeof(buffer), "%clife time<1s mem=%.2fM name=%s", isShow, memSize, layoutName);
				}
				else if (time < 60 * 1000)//小于1分钟.
				{
					snprintf(buffer, sizeof(buffer), "%clife time=%ds mem=%.2fM minute name = %s", isShow, time/(1000), memSize, layoutName);
				}
				else if (time < 60 * 60 * 1000)//小于1小时
				{
					snprintf(buffer, sizeof(buffer), "%clife time=%dm mem=%.2fM hour name=%s", isShow, time/(1000*60), memSize, layoutName);
				}
				texts.push_back(buffer);
			}
			++itStat;
...
showWebView(sWidthAndroid / 2, sHeightAndroid / 2, (int)widthFinal, (int)heightFinal);
#elif CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
	int logicWidth = Nuclear::GetEngine()->GetLogicWidth();
	int logicHeight = Nuclear::GetEngine()->GetLogicHeight();
	int sWidthAndroid = Nuclear::GetEngine()->GetScreenWidth();
...
void GameUImanager::InjectChar(const char* pStr)
{
	if (CEGUI::System::getSingletonPtr() != NULL && pStr != NULL)
	{
		...
		CEGUI::System& guiSystem = CEGUI::System::getSingleton();
		CEGUI::Window* pTarget = guiSystem.getKeyboardTargetWindow();
		bool bHandled = guiSystem.injectChar(pStr);
		std::wstring wTargetName = pTarget ? StringCover::to_wstring(pTarget->getName().c_str()) : L"nil";
		SDLOG_INFO(L"[IME][InjectChar] target=%ls targetNull=%d handled=%d textLen=%d firstByte=%d", wTargetName.c_str(), pTarget ? 0 : 1, bHandled ? 1 : 0, (int)strlen(pStr), (int)(unsigned char)pStr[0]);
#else
		std::wstring wstr = StringCover::to_wstring(pStr);

		CEGUI::System& guiSystem = CEGUI::System::getSingleton();
		CEGUI::Window* pTarget = guiSystem.getKeyboardTargetWindow();
		bool bHandled = guiSystem.injectChar(wstr);
		std::wstring wTargetName = pTarget ? StringCover::to_wstring(pTarget->getName().c_str()) : L"nil";
		SDLOG_INFO(L"[IME][InjectChar] target=%ls targetNull=%d handled=%d textLen=%d firstByte=%d", wTargetName.c_str(), pTarget ? 0 : 1, bHandled ? 1 : 0, (int)strlen(pStr), (int)(unsigned char)pStr[0]);
#endif       
	}
...
bool GameUImanager::DetachIME()
{
	...
	{
		SDLOG_INFO(L"[IME][DetachIME] skip delegate=nil");
		return false;
	}

	if (!m_bAttachIME)
	{
		SDLOG_INFO(L"[IME][DetachIME] skip attached=0");
		return false;
	}

	SDLOG_INFO(L"[IME][DetachIME] begin");
	if (m_pUIIMEDelegate->detachWithIME())
	{
		m_bAttachIME = false;
		m_pUIIMEDelegate->setInputText(NULL);
		SDLOG_INFO(L"[IME][DetachIME] end ret=1");
		return true;
	}

	SDLOG_INFO(L"[IME][DetachIME] end ret=0");
	return false;
}
...
void GameUImanager::AttachIME(const CEGUI::String& str)
{
	if (m_pUIIMEDelegate == NULL)
	{
		SDLOG_INFO(L"[IME][AttachIME] skip delegate=nil");
		return;

	}
	CEGUI::Window* pTarget = NULL;
	if (CEGUI::System::getSingletonPtr() != NULL)
	{
		pTarget = CEGUI::System::getSingleton().getKeyboardTargetWindow();
	}
	std::wstring wTargetName = pTarget ? StringCover::to_wstring(pTarget->getName().c_str()) : L"nil";
	SDLOG_INFO(L"[IME][AttachIME] begin textLen=%d target=%ls targetNull=%d attached=%d", (int)str.length(), wTargetName.c_str(), pTarget ? 0 : 1, m_bAttachIME ? 1 : 0);
	m_pUIIMEDelegate->setInputText(str.c_str());

	if (m_pUIIMEDelegate->attachWithIME())
	{
		m_bAttachIME = true;
		SDLOG_INFO(L"[IME][AttachIME] end ret=1");
	}
	else
	{
		SDLOG_INFO(L"[IME][AttachIME] end ret=0");
	}
}
...
//#endif
	...
	std::wstring wTargetName = pTarget ? StringCover::to_wstring(pTarget->getName().c_str()) : L"nil";
	SDLOG_INFO(L"[IME][OnClickEditArea] textLen=%d target=%ls targetNull=%d", (int)str.length(), wTargetName.c_str(), pTarget ? 0 : 1);
	AttachIME(str);
...
std::string GameUImanager::getClipboard()
{
	...
	memcpy(url, pData, length); url[length] = '\0';
	::GlobalUnlock(hData);
	::CloseClipboard();
	url[length] = 0;
	return url;
