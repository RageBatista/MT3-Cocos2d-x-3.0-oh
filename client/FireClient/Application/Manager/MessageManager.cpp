...
namespace
{
int MakeLuaRegistryFunctionRef(lua_State* state, int toluaFuncRef)
{
	if (!state || toluaFuncRef == 0 || toluaFuncRef == LUA_NOREF)
		return LUA_NOREF;

	toluafix_get_function_by_refid(state, toluaFuncRef);
	if (!lua_isfunction(state, -1))
	{
		lua_pop(state, 1);
		toluafix_remove_function_by_refid(state, toluaFuncRef);
		return LUA_NOREF;
	}

	int registryRef = luaL_ref(state, LUA_REGISTRYINDEX);
	toluafix_remove_function_by_refid(state, toluaFuncRef);
	return registryRef;
}
}
...
{
	int okRegistryRef = MakeLuaRegistryFunctionRef(state, okfunc);
	int cancelRegistryRef = MakeLuaRegistryFunctionRef(state, cancelfunc);
	CEGUI::LuaFunctor oklf(state, okRegistryRef, okselfIndex);
	CEGUI::LuaFunctor cancellf(state, cancelRegistryRef, cacelselfIndex);
