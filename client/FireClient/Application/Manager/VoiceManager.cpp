...
void GetTextManager::getTextFromXunFei(const char* uuid, const std::string& spxData, int recordTime, int voiceChanelid)
{
	...
	char parText[256] = {};
#if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32
	snprintf(parText, sizeof(parText), "mac=%s&appid=%s", DeviceInfo::sGetNoColonMacAddress(), getXunFeiAppId().c_str());
#else
	snprintf(parText, sizeof(parText), "mac=%s&appid=%s", DeviceInfo::sGetMacAddress(), getXunFeiAppId().c_str());
#endif
	...
	snprintf(paramText, sizeof(paramText), "%d", voiceChanelid);
	oper->mVetParam.push_back(paramText);
...
void GetTextManager::requestTocken()
{
	...
	char text[256] = {};
#if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32
	snprintf(text, sizeof(text), "mac=%s&appid=%s", DeviceInfo::sGetNoColonMacAddress(), getXunFeiAppId().c_str());
#else
	snprintf(text, sizeof(text), "mac=%s&appid=%s", DeviceInfo::sGetMacAddress(), getXunFeiAppId().c_str());
#endif
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
...
void SendVoiceManager::sendVoiceToServer(const char* uuid, const std::string& spxData, unsigned int recordTime)
{
	...
	char timeText[256] = {};
	snprintf(timeText, sizeof(timeText), "\"sec\":%.1f,", recordTime * 0.001f);
	text += timeText;
#if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32
...
std::string createUUID()
{
	...
	SYSTEMTIME sm;
	GetSystemTime(&sm);
	snprintf(uuid, sizeof(uuid), "%s_%d_%d_%d_%d_%d_%d_%d", (unsigned char*)DeviceInfo::sGetNoColonMacAddress(),
		sm.wYear, sm.wMonth, sm.wDay, sm.wHour, sm.wMinute, sm.wSecond, sm.wMilliseconds);
#else
	time_t timep;
	time(&timep);
	struct tm* p = gmtime(&timep);
	snprintf(uuid, sizeof(uuid), "%s_%d_%d_%d_%d_%d_%d_%d", (unsigned char*)DeviceInfo::sGetMacAddress(),
