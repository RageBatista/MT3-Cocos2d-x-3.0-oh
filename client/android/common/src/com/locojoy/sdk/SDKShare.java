...
public class SDKShare extends OnekeySharePage implements Callback {
	...
	{
		if (m_Context == null) {
			Log.e(TAG, "initOnekeyShare skipped: context is null");
			return;
		}
	...
	public static void doOnekeyShare(int ePlatform, int eType,
		...
		if (mine == null || mine.m_Context == null) {
			Log.e(TAG, "doOnekeyShare skipped: SDKShare is not initialized");
			return;
		}
		if (ePlatform <= 0 || ePlatform > PlatformName.length) {
			Log.e(TAG, "doOnekeyShare skipped: invalid platform index " + ePlatform);
			return;
		}

		String platformName = PlatformName[ePlatform - 1];
	...
	private static String copyImg(String imgPath)
	{
		...
		try
		{
			...
			if (!outFile.exists()) {
				...
				try {
					in = GameApp.getApp().getAssets().open(imgPath);
