...
public class FileDownloader {
	...
	private static final boolean MT3_RUNTIME_DIAG_ENABLE = false;

	private static void diagUpdate(String message) {
		if (MT3_RUNTIME_DIAG_ENABLE) {
			Log.d("MT3Diag", "MT3_DIAG_UPDATE " + message);
		}
	}

	static boolean DownloadOneFile(final String url, final String dst, boolean notify) {
		for (int i = 0; i < 3; i++) {
			diagUpdate("event=attempt index=" + (i + 1) + " notify=" + (notify ? 1 : 0));
			boolean ret = DownloadOneFileIMP(url, dst, notify);
			diagUpdate("event=attempt_result index=" + (i + 1) + " result=" + (ret ? 1 : 0));
			if (ret)
				return true;
	...
	static boolean DownloadOneFileIMP(String src, String dst, boolean notify) {
		...
		try {
			...
			if (totalsize == -1) {//获取失败
				diagUpdate("event=size_query_failed");
				return false;
			}
			...
			diagUpdate("event=file_prepare total=" + totalsize + " resume=" + size + " notify=" + (notify ? 1 : 0));
			URL url = new URL(src);
			...
			diagUpdate("event=http_status status=" + status + " resume=" + size);
			if (status == 404) {
			...
			cachedfile = new RandomAccessFile(dstFile, "rw");
			cachedfile.seek(size);
			int length = 0;
			byte[] buffer = new byte[1024];
			while ((length = inputstream.read(buffer)) > 0) {
				cachedfile.write(buffer, 0, length);
//				output.flush();
//				if (notify) {
//					size += length;
//
...
//				}
			...
			diagUpdate("event=file_done total=" + totalsize + " finalLength=" + dstFile.length());
		} catch (Exception e) {
			diagUpdate("event=file_error type=" + e.getClass().getSimpleName());
