...
public class GameApp extends Cocos2dxActivity {
	...
	private static final int COLLECT_INFO_FTP_PORT = 21;
	private static final String COLLECT_INFO_FTP_USER = "collect_infoUSER";
	private static final String COLLECT_INFO_FTP_PASSWORD = "TRx87dx@S0DXx8dC";
	...
	private static final int REQUEST_CODE_STARTUP_PERMISSIONS = 200;
	...
	private static InputStream openClientSettingStream() {
		try {
			File rootCfg = new File(getAssetsOutDir() + "/res/2863654426");
			if (rootCfg.exists()) {
				return new FileInputStream(rootCfg);
			}

			File cacheCfg = new File(getCacheOutDir() + "/res/2863654426");
			if (cacheCfg.exists()) {
				return new FileInputStream(cacheCfg);
			}

			if (getApp() != null) {
				return getApp().getAssets().open("res/2863654426");
			}
		} catch (Exception e) {
			Log.w("GameApp", "open client setting failed", e);
		}
		return null;
	}

	private static String readClientSettingValue(String key) {
		InputStream ins = null;
		try {
			ins = openClientSettingStream();
			if (ins == null) {
				return "";
			}
			...
			Log.w("GameApp", "read client setting failed: " + key, e);
			return "";
		} finally {
			if (ins != null) {
				try {
					...
					Log.w("GameApp", "close client setting failed", e);
				}
			}
		}
	}

	private static String normalizeResourceUpdateUrl(String url) {
		if (url == null) {
			return "";
		}
		...
		int fragmentIndex = ret.indexOf('#');
		if (fragmentIndex >= 0) {
			ret = ret.substring(0, fragmentIndex);
		}
		...
		if ((lower.endsWith(".html") || lower.endsWith(".json") || lower.endsWith(".txt")) && lastSlash >= 0) {
			return ret.substring(0, lastSlash + 1);
		}
	...
	public static String getResourceUpdateUrl() {
		String serverListUrl = readClientSettingValue("HttpServerListUrl");
		String ret = normalizeResourceUpdateUrl(serverListUrl);
		if (ret.length() == 0) {
			ret = normalizeResourceUpdateUrl(readClientSettingValue("HttpServerAddressPlatForm"));
		}
		Log.d("GameApp", "getResourceUpdateUrl=" + ret);
		return ret;
	}
...
//					}
	...
	private void requestStartupPermissionsIfNeeded() {
		...
		if (!missingPermissions.isEmpty()) {
			requestPermissionsCompat(missingPermissions.toArray(new String[missingPermissions.size()]), REQUEST_CODE_STARTUP_PERMISSIONS);
		}
	}
	...
	private boolean hasPermissionCompat(String permission) {
		try {
			Method method = Class.forName("android.app.Activity").getMethod("checkSelfPermission", String.class);
	...
	private void requestPermissionsCompat(String[] permissions, int requestCode) {
		try {
			Method method = Class.forName("android.app.Activity").getMethod("requestPermissions", String[].class, int.class);
			method.invoke(this, new Object[] { permissions, Integer.valueOf(requestCode) });
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
//		}
	...
	protected void onDestroy() {
		...
		if (mIsBatteryReceiverRegistered) {
			try {
				unregisterReceiver(batteryLevelRcvr);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				mIsBatteryReceiverRegistered = false;
			}
		}
		removeWebView();
		if (m_InitHandler != null) {
			m_InitHandler.removeCallbacksAndMessages(null);
			m_InitHandler = null;
		}
		if (m_UpdateHandler != null) {
			m_UpdateHandler.removeCallbacksAndMessages(null);
...
//		}
	...
	protected void onResume() {
		...
		mIsBatteryReceiverRegistered = true;
	...
	// -----------------------------------notify jni start ------------------
	private static void sendUpdateMessage(Message smsg) {
		Handler updateHandler = m_UpdateHandler;
		if (updateHandler != null && smsg != null) {
			updateHandler.sendMessage(smsg);
		}
	}
	...
	public static void uploadDumpFile() {
		new Thread() {
			...
			public void run() {
				...
				ftp.setLoginInfo(getHttpShareUrl(), COLLECT_INFO_FTP_PORT, COLLECT_INFO_FTP_USER, COLLECT_INFO_FTP_PASSWORD);	// "120.26.89.140"
