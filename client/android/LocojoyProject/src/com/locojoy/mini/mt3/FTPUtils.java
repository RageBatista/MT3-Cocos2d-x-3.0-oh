...
public class FTPUtils {
	...
	private static String normalizeFTPHost(String url) {
		if (url == null) {
			return "";
		}
		...
		int questionPos = host.indexOf('?');
		if (questionPos >= 0) {
			host = host.substring(0, questionPos);
		}
		if (host.startsWith("[")) {
	...
	public boolean openConnect() {
		try {
			// 连接至服务器
			String ftpHost = normalizeFTPHost(this.FTPUrl);
			if (ftpHost.length() == 0) {
				return false;
			}
			ftpClient.connect(ftpHost, this.FTPPort);
