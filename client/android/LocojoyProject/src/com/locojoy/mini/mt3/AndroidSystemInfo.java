...
public class AndroidSystemInfo {
	...
	// 鑾峰彇绯荤粺鎬籆PU浣跨敤鏃堕棿
    private static String[] splitCpuInfoLine(String load)
    {
        if (load == null)
        {
            return null;
        ...
        String[] cpuInfos = load.split("\\s+");
        return cpuInfos.length > 0 ? cpuInfos : null;
    }

    private static long parseCpuInfoValue(String[] cpuInfos, int index)
    {
        if (cpuInfos == null || index < 0 || index >= cpuInfos.length)
        {
            return 0L;
        }
        try
        {
            return Long.parseLong(cpuInfos[index]);
        }
    ...
    private static long getTotalCpuTime()
    {
        ...
        try
        {
            ...
            reader.close();
            cpuInfos = splitCpuInfoLine(load);
        }
        ...
        long totalCpu = parseCpuInfoValue(cpuInfos, 2)
                + parseCpuInfoValue(cpuInfos, 3) + parseCpuInfoValue(cpuInfos, 4)
                + parseCpuInfoValue(cpuInfos, 6) + parseCpuInfoValue(cpuInfos, 5)
                + parseCpuInfoValue(cpuInfos, 7) + parseCpuInfoValue(cpuInfos, 8);
        return totalCpu;
    }
    ...
    private static long getAppCpuTime()
    {
        ...
        long appCpuTime = parseCpuInfoValue(cpuInfos, 13)
                + parseCpuInfoValue(cpuInfos, 14) + parseCpuInfoValue(cpuInfos, 15)
                + parseCpuInfoValue(cpuInfos, 16);
        return appCpuTime;
    }
