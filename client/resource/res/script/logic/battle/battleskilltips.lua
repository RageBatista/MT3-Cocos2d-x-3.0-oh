...
BattleSkillTips = {}
setmetatable(BattleSkillTips, Dialog)
BattleSkillTips.__index = BattleSkillTips

local _instance
function BattleSkillTips.getInstance()
	if not _instance then
		_instance = BattleSkillTips:new()
		_instance:OnCreate()
	end
	return _instance
end

function BattleSkillTips.getInstanceAndShow()
	if not _instance then
		_instance = BattleSkillTips:new()
		_instance:OnCreate()
	else
...
function BattleSkillTips.getInstanceNotCreate()
	return _instance
end

function BattleSkillTips.DestroyDialog()
	if _instance then
...
function BattleSkillTips.ToggleOpenClose()
	if not _instance then
		_instance = BattleSkillTips:new()
		_instance:OnCreate()
	else
...
function BattleSkillTips.GetLayoutFileName()
	return "skilltipsdialog.layout"
end

function BattleSkillTips:new()
	local self = {}
	self = Dialog:new()
	setmetatable(self, BattleSkillTips)
	return self
end

function BattleSkillTips:OnCreate()
...
function BattleSkillTips:SetSkill(aSkillID)
    if aSkillID == 0 then
		return
	end
	...
	elseif SkillType == eSkillType_Pet then        
		...
		self.m_pSkillDesc:Clear()
		self.m_pSkillDesc:AppendParseText(CEGUI.String(skillBase.littledes))
...
function BattleSkillTips:AppendSkillDescLine(ParseText)
...
function BattleSkillTips.showAndSetSkill(aSkillID)
    local dlg = BattleSkillTips.getInstanceAndShow()
    dlg:SetSkill(aSkillID)
end
return BattleSkillTips
