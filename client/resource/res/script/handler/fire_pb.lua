...
local p = require "protodef.fire.pb.http.SResponsePost"
function p:process()
     HttpManager.getInstance():RecvData(self.retvalue,self.dataid)
end
...
local function ProcessServerIdResponse(serverid)
    -- Set ServerID to android.lua
    local LuaAndroid = require "android"
    LuaAndroid.serverid = serverid
    ...
    elseif Config.TRD_PLATFORM == 1 and Config.MOBILE_ANDROID == 0 and Config.CUR_3RD_PLATFORM == "this" then
        MT3.ChannelManager:CommonInterface(1, serverid)
    end
end

local p = require "protodef.fire.pb.sserveridresponse"
function p:process()
    ProcessServerIdResponse(self.serverid)
end

local gnetserveridresponse = require "protodef.gnet.serveridresponse"
function gnetserveridresponse:process()
    local serverid = tonumber(gGetLoginManager():getServerID()) or 0
    local sid = tonumber(tostring(self.serverid))
    if sid then
        serverid = sid
    end
    ProcessServerIdResponse(serverid)
end
...
function sgametime:process()
    ...
    if gLastServerTime ~= 0 then     --���ǵ�һ�η�������  ��������Ƿ���ȷ
