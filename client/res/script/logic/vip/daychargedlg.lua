...
DayChargeDialog = {}
setmetatable(DayChargeDialog, Dialog)
DayChargeDialog.__index = DayChargeDialog

local _instance

function DayChargeDialog.getInstance()
	if not _instance then
		_instance = DayChargeDialog:new()
		_instance:OnCreate()
	end
	return _instance
end

function DayChargeDialog.getInstanceAndShow()
	if not _instance then
		_instance = DayChargeDialog:new()
		_instance:OnCreate()
	else
...
function DayChargeDialog.DestroyDialog()
	if _instance then
...
function DayChargeDialog:OnClose()
	Dialog.OnClose(self);
	getmetatable(self)._instance = nil;
end

function DayChargeDialog.GetLayoutFileName()
	return "daycharge.layout"
end

function DayChargeDialog.getInstanceNotCreate()
	return _instance
end


function DayChargeDialog:new()
	local self = {}
	self = Dialog:new()
	setmetatable(self, DayChargeDialog)
	return self
end

function DayChargeDialog:getChargeItem()
	-- case eHttpShareUrl:
	-- case eHttpInfoUrl:
	-- case eHttpChatUrl:
    ...
    local actionname = "getChargeItemDay"
    CTipsManager:ServerPost(action,param,actionname)
end
function DayChargeDialog:OnCreate()
    Dialog.OnCreate(self)
    SetPositionScreenCenter(self:GetWindow())
	local winMgr = CEGUI.WindowManager:getSingleton()
	self:GetCloseBtn():removeEvent("Clicked")
	self:GetCloseBtn():subscribeEvent("Clicked", DayChargeDialog.DestroyDialog, nil)
	
    self.scrollable = winMgr:getWindow("daychargedlg/bg/scrollable") --背景

	self.cells = {}
    self:getChargeItem()

end
function DayChargeDialog:changeCell(param)
...
function DayChargeDialog:addchargecell(index,data)
    ...
    ImageButton:subscribeEvent("Clicked", DayChargeDialog.handleLingquButtonClick, self)
...
function DayChargeDialog:handleLingquButtonClick(args)
    ...
    local eHttpShareUrl = GetServerInfo():getHttpAdressByEnum(eHttpShareUrl)
    local action = eHttpShareUrl.."/api/charge_award/receiveday"
    local param = {}
    param["account"] = gGetLoginManager():GetAccount()
    param["roleid"] = gGetDataManager():GetMainCharacterID()
	param["serverid"] = tostring(gGetLoginManager():getServerID())
    ...
    local actionname = 'ReceiveDayCharge'
