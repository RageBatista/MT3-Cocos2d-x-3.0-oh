...
local function bindDialogEvent(wnd, eventName, obj, handler)
...
function Dialog:OnCreate(parentwindow,nameprefix)
	...
	if not self.m_pMainFrame then
		...
		--LogInfo("lua load layout "..nameprefix..fileName)
	    self.m_pMainFrame = winMgr:loadWindowLayout(fileName, nameprefix)
	end

	if not self.m_pMainFrame then 
	    LogErr("layout load error --> " .. tostring(fileName))
		return
	end
...
--]]
	...
	if closeBtn then 
		bindDialogEvent(closeBtn, "Clicked", self, self.HandleCloseBtnClick);
	end

	bindDialogEvent(self.m_pMainFrame, "Shown", self, self.HandleDialogOpen);
	bindDialogEvent(self.m_pMainFrame, "Hidden", self, self.HandleDialogClose);
	bindDialogEvent(self.m_pMainFrame, "DestructStart", self, self.HandleDialogClose);
--[[
	if self.m_pMainFrame:GetCloseWndEffect() ~= CEGUI.CloseWndEffect_None then
		bindDialogEvent(self.m_pMainFrame, "CloseWndEffectEnd", self, self.HandleCloseEffectEnd)
	end
--]]
...
function Dialog:HandleDialogOpen(args)
	local wnd = self and self.GetWindow and self:GetWindow()
    if wnd and wnd:isModalAfterShow() then
...
function Dialog:HandleCloseEffectEnd(args)
	if not self or not self.m_pMainFrame or not self.GetWindow then return true end
...
function Dialog:HandleCloseBtnClick(args)
	if self and self.DestroyDialog then
		self:DestroyDialog()
	end
	return true
end
