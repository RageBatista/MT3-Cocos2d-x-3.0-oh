...
function Openui.OpenUI(nUIId, nParam1, nParam2)
    ...
    elseif Openui.eUIId.xiulianjineng_20 == nUIId then
        ...
        if nLvl >= recordOpen.needlevel then
            -- FIXME:新功能开启表  现在新功能开启那块代码有问题判断方法有问题 暂时这样处理等新功能开启完成再修改  wjf
    ...
    --elseif Openui.eUIId.anyemaxituan_62 == nUIId then--暗黑马戏团
	...
	elseif Openui.eUIId.fabaojinjie == nUIId then
		require "logic.workshop.workshopaq".getInstanceAndShow()
	---/======================法宝进阶	
	elseif Openui.eUIId.fabaonizhuan == nUIId then
		require "logic.workshop.zhuangbeichongzhu".getInstanceAndShow()
	---/======================法宝逆转
	elseif Openui.eUIId.qilingtzdh == nUIId then
		require "logic.workshop.Attunement".getInstanceAndShow()
	---/======================器灵套装点化
	elseif Openui.eUIId.qilingkg == nUIId then
		require "logic.workshop.superronglian".getInstanceAndShow()
	---/======================器灵开孔
	elseif Openui.eUIId.qilingxq == nUIId then
		require "logic.workshop.zhuangbeifumo".getInstanceAndShow()
	---/======================器灵镶嵌
	elseif Openui.eUIId.zuoqixl == nUIId then
		require "logic.workshop.workshopxilian".getInstanceAndShow()
	---/======================坐骑洗炼	
	elseif Openui.eUIId.tejidingzhi == nUIId then
		require "logic.workshop.tejidingzhi".getInstanceAndShow()
	---/======================特技定制	
	elseif Openui.eUIId.cwzbdazao == nUIId then
	    require("logic.shengsizhan.jingmaihecheng_hs1").getInstanceAndShow()
	---/======================宠装打造	
	elseif Openui.eUIId.lyshc1 == nUIId then
	    require("logic.libaoduihuan.lyshc1")
        lyshc1.getInstanceAndShow()
    elseif Openui.eUIId.jinyi == nUIId then
        debugrequire"logic.ranse.charactershizhuangdlg".getInstanceAndShow() 
	---/======================����ʯ�ϳ�	
	end
end
